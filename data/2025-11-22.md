...
Result: 归因分数过度强调最终推理步骤（尤其在错误生成中），结构化CoT提示显著提高高资源拉丁语系语言的准确性，受控扰动降低模型准确性和归因一致性。

Conclusion: 思维链提示在多语言鲁棒性和解释透明度方面存在局限性，需要改进多语言场景下的推理可靠性和可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 现有心理理论基准主要关注错误信念任务和非对称信息推理，忽略了信念之外的其他心理状态以及丰富的非语言沟通方式。
...
<details>
  <summary>Details</summary>
Motivation: 现有TOD基准将复杂指令过度简化为简单的意图、槽位和API调用配置，无法反映真实世界中需要严格遵循包含复杂约束的流程指令的场景。

Method: 基于高质量ABCD数据集构建指令文档和对话数据，将细粒度约束和动作流程表述为多级条件-动作指令语句，设计三个任务全面评估LLM的指令遵循能力。

Result: 构建了包含复杂流程指令的基准数据集，并设计了系统性的评估框架来测试LLM在多轮TOD中的指令遵循能力。

Conclusion: TOD-ProcBench填补了现有基准的空白，为系统评估LLM在复杂指令遵循能力方面提供了重要工具，并研究了多语言设置和指令格式对性能的影响。
...
TL;DR: LIARS' BENCH是一个包含72,863个谎言和诚实回答的测试基准，涵盖4个开源模型和7个数据集，用于评估现有谎言检测技术的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型谎言检测技术通常在狭窄场景中验证，无法捕捉模型可能生成的各种谎言类型。

Method: 创建LIARS' BENCH测试基准，包含不同原因和目标的谎言类型，评估三种黑白盒谎言检测技术。

Result: 现有技术系统性地无法检测某些类型的谎言，特别是在仅凭对话记录无法确定模型是否说谎的场景中。

Conclusion: LIARS' BENCH揭示了现有技术的局限性，为谎言检测的进步提供了实用的测试基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法使用可处理的代理模型（如HMM）来近似延续概率，但这些代理模型通常上下文感知能力较弱，导致查询质量下降。需要一种既能利用神经上下文信息又能保持计算效率的方法。

Method: LTLA将基础语言模型用于丰富前缀编码，与固定可处理代理模型配对计算精确的延续概率。通过单次批量HMM更新处理所有下一个token候选，并仅让代理模型的潜在状态先验依赖于LM的隐藏表示，同时保持代理解码器固定以实现跨前缀的计算重用。

Result: LTLA获得了比无条件HMM更高的条件似然，能够为视觉语言模型近似延续分布（独立HMM无法编码视觉上下文），在受控生成任务中以可比较的流畅度提高了约束满足度，且推理开销最小。

Conclusion: LTLA通过有效结合神经上下文和可处理代理模型，在保持计算效率的同时显著提升了受控语言生成的质量和约束满足能力。
...
TL;DR: GPT-5在数学、物理、天文学、计算机科学、生物学和材料科学等多个科学领域展示了加速研究的能力，特别是在数学领域产生了四个经过验证的新结果，证明了AI与人类科学家合作的价值。


<details>
  <summary>Details</summary>
Motivation: 许多科学家对前沿AI的能力认识不足，本文旨在通过具体案例展示GPT-5如何帮助科学家在研究中取得新进展，并阐明AI与人类合作的有效模式。

Method: 通过多个学科领域的短案例研究，记录科学家与GPT-5的互动过程，展示AI如何加速研究并指出其局限性。

Result: GPT-5在多个科学领域产生了具体的研究进展，特别是在数学领域产生了四个经过人类作者仔细验证的新结果，能够帮助数学家解决先前未解决的问题。

Conclusion: 尽管这些贡献范围有限，但鉴于前沿AI的快速发展速度，其影响深远，展示了AI与人类科学家合作的重要价值。
...
TL;DR: 提出基于集成学习的提示优化框架ELPO，通过投票机制和多样化搜索方法提升复杂任务中的提示优化效果


<details>
  <summary>Details</summary>
Motivation: 现有自动提示优化方法通常采用单一模型或算法，在处理复杂任务时性能受限，需要更准确和鲁棒的优化方案

Method: 基于集成学习思想，采用投票机制，结合共享生成策略和不同搜索方法来寻找更优提示，并提出了更高效的生成和搜索算法
...
TL;DR: 本文提出了一种新的Token-Selective PEFT方法，质疑传统PEFT对所有位置索引进行修改的必要性，并通过实验证明选择性修改部分位置索引可以提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 传统PEFT方法对所有位置索引都进行修改，作者质疑这种做法的必要性，认为可能造成资源浪费甚至负面影响。

Method: 提出Token-Selective PEFT方法，通过选择函数S仅对部分位置索引应用PEFT修改，而不是对所有索引进行修改。
...
TL;DR: SemanticCite是一个AI驱动的引文验证系统，通过全文分析验证引文准确性，提供四类分类（支持、部分支持、不支持、不确定）来捕获细微的声明-来源关系，并支持大规模引文验证。


<details>
  <summary>Details</summary>
Motivation: 学术文献面临语义引文错误、AI生成的幻觉参考文献以及传统引文格式无法指明具体支持章节的问题，需要准确验证引文来确保科学传播的有效性。

Method: 结合多种检索方法与四类分类系统，通过微调的轻量级语言模型进行全文源分析，提供详细推理和相关文本片段。

Result: 微调的轻量级语言模型在性能上可与大型商业系统相媲美，但计算需求显著降低，使大规模引文验证变得可行。
...
Method: 开发自适应稀疏有向语义图构建算法，捕捉方向性语义依赖并自动剪除负干扰连接；通过层次抽象利用潜在语义结构信息，将SeSE定义为最优语义编码树的结构熵；扩展SeSE以量化长文本生成中单个声明的不确定性。
...
Method: 提出SDA框架，基于用户定义的对齐指令动态重分布模型输出概率。该方法轻量、资源高效，可独立使用或与训练策略集成，支持个性化偏好对齐。

Result: 在8个不同规模和来源的开源LLMs上测试，SDA在帮助性、无害性和诚实性三个维度上分别平均提升64.4%、30%和11.5%，显示出良好的泛化能力。

Conclusion: SDA是一种有效且通用的对齐框架，无需训练即可显著提升LLMs与人类意图的对齐性能，适用于多种模型和应用场景。
...
TL;DR: 提出自重写框架，通过模型重写自身推理文本来改进内部推理过程质量，解决传统强化学习仅关注最终正确性的局限性。
...
Method: 采用选择性重写方法，仅对模型一致正确的"简单"样本进行重写，保持GRPO的所有原始奖励信号；在单个批次中编译重写和原始生成，保持强化学习算法的可扩展性。

Result: 在准确率-长度权衡方面，自重写方法在显著缩短推理长度(-46%)的同时提高了准确率(+0.6)；在内部推理质量方面，自重写获得显著更高的LLM评判分数(+7.2)，成功缓解了内部推理缺陷。

Conclusion: 自重写框架有效提升了推理模型的内部推理质量，在保持强化学习可扩展性的同时解决了传统单边奖励的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 习语和比喻语言在口语和书面语中占很大比例，但大型语言模型在这方面仍然存在困难。虽然大语料库似乎能解决所有NLP问题，但习语和比喻语言继续困扰着LLMs。

Method: 使用现有习语和比喻语言数据集获取组合习语列表，从大型语料库中检索上下文序列，创建一个大规模潜在习语数据集和两个人工标注的确切习语数据集，用于评估预训练语言模型。
...
TL;DR: 该论文分析了自然语言解释（rationales）的充分性指标，发现其不能很好地反映模型性能提升，并探讨了rationales在token分类和注意力正则化中的影响。


<details>
  <summary>Details</summary>
Motivation: 现有的rationales充分性指标对模型性能提升的洞察有限，需要更深入地理解rationales信息对模型性能的影响机制。

Method: 通过将充分性与两种建模范式关联：token分类能力和通过注意力正则化在输入中融入rationales的能力。

Result: 发现高度信息化的rationales不一定有助于正确分类；充分性反而捕捉了非rationalized上下文的分类影响；融入rationales信息可以提升跨域分类但结果不一致；充分性与token分类无关。

Conclusion: rationales的复杂性表明需要开发能够系统捕捉这类信息的指标，值得进一步研究。
...
TL;DR: 提出了MinerU-HTML，一种基于语言模型的HTML到文本提取方法，显著优于传统启发式方法，并构建了AICC语料库，证明提取质量对下游模型性能有重要影响。


<details>
  <summary>Details</summary>
Motivation: 现有网页语料库主要依赖启发式提取器，这些方法难以保留文档结构并经常破坏结构化元素。作者假设提高提取质量对下游性能的影响可能与积极过滤策略一样重要。

Method: 将内容提取重新定义为序列标注问题，使用0.6B参数的语言模型解决。采用两阶段格式化流程，先明确分类语义元素，再转换为Markdown格式。

Result: 在7,887个标注网页的基准测试中，MinerU-HTML达到81.8% ROUGE-N F1，显著优于Trafilatura的63.6%，结构化元素保留率极高（代码块90.9%，公式94.0%）。基于此构建的AICC语料库在13个基准测试中平均准确率达到50.8%，优于传统方法。

Conclusion: HTML提取是网页语料库构建中关键但常被低估的组成部分，基于模型的提取方法具有内在可扩展性，而启发式方法改进路径有限。
...
Method: 使用3种机器学习分类器和3种深度学习模型，基于141万篇英文新闻文章数据集，每篇文章有194个语言特征，将专家评级的579个新闻网站按中位数分为低质量与高质量两类。
...
TL;DR: 本文研究了基于Transformer的语言模型如何处理习语表达，通过电路发现和分析技术识别了习语处理的计算模式，发现了"习语头"和"增强接收"现象。
...
Method: 使用改进的路径修补算法进行电路发现，识别和分析"习语头"（在多个习语中频繁激活的注意力头）以及"增强接收"（由于早期处理导致的习语token间增强注意力）现象。

Result: 发现习语处理具有独特的计算模式，识别出专门的"习语头"，观察到习语token间存在增强的注意力机制（"增强接收"），这些机制帮助模型平衡计算效率和鲁棒性。

Conclusion: 研究结果为理解Transformer处理非组合性语言提供了见解，并为理解更复杂语法结构的处理机制指明了途径。
...
TL;DR: Arctic-Extract是一个最先进的结构化数据提取模型，专为从扫描或数字商务文档中提取问答、实体和表格数据而设计，具有轻量级部署特性。
...
Method: 采用先进的训练协议，优化模型架构以实现轻量化，模型大小仅6.6 GiB，可在A10 GPU等有限资源设备上运行。

Result: 模型在A10 GPU上可处理多达125页A4文档，在文档理解任务中表现出色，达到最先进性能水平。

Conclusion: Arctic-Extract成功实现了高性能与轻量化部署的平衡，为资源受限环境下的文档处理提供了实用解决方案。
...
TL;DR: TurkColBERT是首个针对土耳其语检索的全面基准测试，比较了密集编码器和延迟交互模型。结果显示延迟交互模型在参数效率方面表现优异，比密集编码器小3-5倍但性能更好，ColmmBERT-base-TR在特定领域任务上mAP提升达+13.8%。MUVERA+重排序比PLAID快3.33倍且mAP相对提升+1.7%。


<details>
  <summary>Details</summary>
Motivation: 神经信息检索系统在高资源语言中表现出色，但在形态丰富的低资源语言如土耳其语中研究不足。密集双编码器目前主导土耳其语IR，但保留词元级表示进行细粒度匹配的延迟交互模型尚未系统评估。

Method: 采用两阶段适应流程：先在土耳其语NLI/STS任务上微调英语和多语言编码器，然后使用在MS MARCO-TR上训练的PyLate将其转换为ColBERT风格的检索器。评估了10个模型在5个土耳其BEIR数据集上的表现。

Result: 延迟交互模型表现出强大的参数效率：1.0M参数的colbert-hash-nano-tr比600M的turkish-e5-large密集编码器小600倍，但保留了其平均mAP的71%以上。比密集编码器小3-5倍的延迟交互模型显著优于它们；ColmmBERT-base-TR在特定领域任务上mAP提升达+13.8%。MUVERA+重排序比PLAID快3.33倍，提供+1.7%相对mAP增益。

Conclusion: TurkColBERT为土耳其语检索建立了首个全面基准，证明延迟交互模型在参数效率和性能方面优于密集编码器。MUVERA实现了低延迟检索，ColmmBERT-base-TR在MUVERA下达到0.54毫秒查询时间。局限性包括依赖中等规模数据集和翻译基准，可能无法完全反映真实世界的土耳其语检索条件。
...
TL;DR: 基于LLM激活状态预测文本体裁，使用Mistral-7B和两个数据集，通过scikit-learn分类器实现高达98%和71%的F1分数，证明浅层学习模型可以从LLM中推断文本体裁。
...
TL;DR: 该论文挑战了ASR评估中过度依赖词错误率(WER)的标准，发现WER等现有指标与临床影响相关性差，提出了基于LLM的自动化评估框架来替代专家临床评估。


<details>
  <summary>Details</summary>
Motivation: 当前ASR在临床对话中部署增多，但标准评估仍主要依赖WER，需要验证这些指标是否真正反映转录错误对临床实践的影响。

Method: 建立专家标注的黄金标准基准，使用GEPA程序化优化LLM作为评判者(Gemini-2.5-Pro)，复制专家临床评估。

Result: WER和现有指标与临床风险标签(无影响、最小影响、显著影响)相关性差；优化的LLM评判者达到90%准确率和0.816 Cohen's κ，表现与人类专家相当。
...
TL;DR: 提出了一种无需人工标注训练数据的词义消歧方法，利用统计语言模型作为消歧器，将符号NLU系统生成的候选含义转换为可区分的自然语言替代项，通过LLM选择给定语境下的合适解释。
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG系统依赖LLM将图像总结成文本进行预处理，导致上下文信息和视觉细节丢失，影响下游检索和问答性能。

Method: 在金融财报电话会议基准上评估了6个LLM模型和2个多模态嵌入模型，比较文本分块检索和直接多模态嵌入检索两种方法。

Result: 直接多模态嵌入检索在mAP@5上绝对提升13%（相对提升32%），在nDCG@5上绝对提升11%（相对提升20%），且能产生更准确和事实一致的答案。

Conclusion: LLM总结在预处理过程中引入信息损失，而直接多模态嵌入能保留视觉上下文用于检索和推理，是更优的多模态RAG解决方案。
...
TL;DR: Nemotron Elastic是一个框架，通过单一父模型嵌入多个嵌套子模型，实现多尺度推理LLM的零成本部署，相比从头训练模型家族可节省360倍成本。


<details>
  <summary>Details</summary>
Motivation: 传统训练多尺度语言模型成本高昂，需要为每个不同规模的模型单独训练。现有压缩方法虽然降低成本，但仍需数百亿token的训练开销。

Method: 采用混合Mamba-Attention架构，通过端到端训练的路由器和两阶段训练课程，实现权重共享的嵌套子模型零成本提取。引入组感知SSM弹性化、异质MLP弹性化、归一化MSE层重要性评估和知识蒸馏等技术。

Result: 在Nemotron Nano V2 12B模型上应用，仅用110B训练token同时生成9B和6B模型，相比从头训练节省360倍成本，相比最先进压缩技术节省7倍成本。嵌套模型在准确率上达到或超过最先进水平。

Conclusion: 该方法实现了恒定部署内存的多合一推理模型，解决了模型家族部署的内存开销问题，为多尺度LLM部署提供了高效解决方案。
...
TL;DR: 提出UniFit，一个基于多模态大语言模型的通用虚拟试穿框架，通过语义对齐模块和两阶段训练策略解决语义鸿沟和数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本指令的多任务VTON框架存在语义鸿沟和复杂场景数据稀缺两大挑战，需要构建更通用的解决方案。

Method: 引入MLLM引导的语义对齐模块(MGSA)，通过多模态输入和可学习查询实现语义对齐；采用两阶段渐进训练策略和自合成流程。

Result: UniFit支持多种VTON任务（多服装、模型到模型试穿等），并在广泛实验中达到最先进性能。

Conclusion: UniFit通过MLLM驱动的语义对齐和高效训练策略，成功构建了通用的VTON框架，解决了现有方法的局限性。
...
TL;DR: EfficientSAM3通过渐进式分层蒸馏方法将SAM3的能力转移到轻量级模型，实现高效的设备端概念分割和跟踪。


<details>
  <summary>Details</summary>
Motivation: SAM3的统一架构（共享视觉骨干、DETR风格检测器、密集内存跟踪器）在设备端使用成本过高，需要开发更高效的模型。
...
Conclusion: EfficientSAM3能够在保持与教师模型行为高度一致的同时，实现设备端概念分割和跟踪，提供了高效的解决方案。
...
TL;DR: 提出了四种新颖的扩展方法来改进基于模型的6D物体姿态估计，包括动态非均匀密集采样、多假设推理、迭代优化和遮挡增强训练，在遮挡场景下显著提升了精度和速度。


<details>
  <summary>Details</summary>
Motivation: 现有方法在遮挡情况下容易出错，且错误会在多阶段处理中传播，导致性能下降。需要解决遮挡带来的挑战，提高对未见物体的泛化能力。

Method: 1) 动态非均匀密集采样策略，聚焦可见区域计算；2) 多假设推理机制，保留多个置信度排序的姿态候选；3) 迭代优化逐步提升精度；4) 遮挡增强训练提升鲁棒性和泛化能力。

Result: 在ICBIN数据集上精度提升超过5%，BOP基准数据集上提升超过2%，推理速度提升约3倍。

Conclusion: 提出的方法有效解决了遮挡场景下的6D姿态估计问题，显著提升了精度和效率，并提出了新的加权可见性评估指标来减少现有评估协议的偏差。
...
TL;DR: 提出了一种自动深度生成方法和手动随机退化技术，为历史地图生成合成训练数据，解决了历史地图标注数据稀缺的问题，并通过语义分割任务验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 历史地图的自动化分析面临标注数据稀缺的挑战，而合成训练数据往往缺乏真实性和多样性。本文旨在通过生成逼真的合成历史地图来解决这一问题。

Method: 采用自动深度生成方法和手动随机退化技术，将原始历史地图的制图风格转移到矢量数据上，生成具有视觉不确定性和噪声的合成历史地图。

Result: 生成的训练数据集在自构建图卷积网络上进行了领域自适应语义分割测试，验证了数据引导方法的有效性。

Conclusion: 该方法能够有效解决历史地图训练数据稀缺问题，为同质地图语料库的分析任务提供可行的数据增强方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决仓库自动化、拣选等场景中，现有方法在灵活性、精度和实用性方面的不足：基于模型的方法需要精确CAD模型且泛化差；无模型方法在挑战条件下易失败；类别级方法过于通用且忽略环境先验。
...
TL;DR: 提出一种两阶段自改进范式，通过让VLM生成详细文本推理，然后基于这些自生成的推理进行微调，最后进行监督微调，显著提升领域特定视频分类性能。
...
Result: 在多个数据集上的实验表明，该方法显著优于直接监督微调，验证了自生成推理作为有效且标注高效的VLM适应范式。

Conclusion: 自生成推理是一种有效的、标注高效的方法，可用于将视觉语言模型适应到领域特定的视频分析任务中。
...
TL;DR: 提出了MGLL框架，通过多粒度对比学习改进医学图像的多标签和跨粒度对齐，在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: CLIP等现有方法专注于单标签、单粒度对齐，但在医学成像等复杂领域中，图像通常对应多个高级标签和不同粒度的标注，需要更有效的多粒度对齐方法。

Method: MGLL框架利用结构化多标签监督，整合不同粒度的文本描述，引入带点约束的软标签监督，并使用平滑KL散度确保跨粒度一致性，可作为即插即用模块。

Result: 在构建的大规模多粒度数据集上预训练，并在多个数据集上评估，MGLL在下游任务中优于其他最先进方法。

Conclusion: MGLL通过多粒度对比学习有效提升了医学图像的多标签和跨粒度对齐能力，为复杂领域的视觉语言模型提供了有效的解决方案。
...
TL;DR: 提出了一种从3D心脏超声体积中自动选择标准2D视图的方法，结合深度学习视图分类器和基于解剖标志的启发式规则，重建标准超声心动图视图，验证准确率达96%。


<details>
  <summary>Details</summary>
Motivation: 传统心脏超声依赖2D视频，而3D超声心动图具有更好的图像质量，但医生习惯使用2D视图进行诊断。需要一种方法既能利用3D扫描的速度优势，又能让医生以熟悉的2D格式解读数据。

Method: 应用深度学习视图分类器和基于解剖标志的下游启发式规则，结合心脏病专家提供的启发式方法，从3D超声体积中重建标准2D超声心动图视图。

Result: 三位心脏病专家盲法评估显示96%的准确率（来自2家医院的1,600个视频）。提取的2D视频能够使用AI模型检测心脏异常，并生成临床级的心脏解剖测量。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频场景图生成系统是封闭的前馈管道，无法融入人类指导；而可提示分割模型缺乏语义或关系推理。需要统一视觉提示与空间、时间和语义理解的交互框架。
...
TL;DR: InfoCLIP通过信息论方法改进CLIP在开放词汇语义分割中的微调，避免过拟合和模态对齐退化，利用互信息目标压缩像素-文本对齐并最大化预训练与微调模型间的对齐知识传递。
...
Method: 提出InfoCLIP，基于互信息设计两个目标：压缩预训练CLIP的像素-文本模态对齐以减少噪声；最大化预训练CLIP与微调模型间对齐知识的互信息以传递适合分割的局部语义关系。
...
Result: 在三个外部数据集上的验证显示，所提出的多任务方法相比基线在分割任务上取得了统计学显著改进（p<0.001）：UDIAT（Dice=0.81 vs 0.59）、BUSI（0.66 vs 0.56）、BUS-UCLM（0.69 vs 0.49）。在UDIAT数据集上达到了最先进的分割性能。

Conclusion: 所提出的一致性正则化方法有效缓解了多任务学习中的破坏性干扰，显著提升了乳腺超声肿瘤分割任务在外部验证中的泛化性能，并在UDIAT数据集上实现了最先进的性能。
...
Method: 检测引导的生成式框架，提取ROI级特征并使用BART生成器预测层次化类别和属性token，支持属性条件识别。

Result: 在大型电商数据集和开源数据集上实验表明，该方法显著优于基于相似度的流程和多阶段分类系统。

Conclusion: 该方法实现了更强的细粒度识别和更一致的统一推理，为视觉语义理解提供了有效解决方案。
...
TL;DR: 提出FADS方法，通过聚类采样构建人口统计学平衡的演示样本，在无需微调的情况下改善多模态大语言模型在医学图像推理中的公平性。
...
Method: 提出公平感知演示选择(FADS)，通过聚类采样构建人口统计学平衡且语义相关的演示样本，作为轻量级、无需调优的替代方案。
...
TL;DR: 本文提出了一种基于图表示的方法，通过利用样本间关系来改进长尾分布视觉数据集中的OOD检测性能，显著降低了误报率并提高了尾类分类准确率。


<details>
  <summary>Details</summary>
Motivation: 在长尾分布数据集中进行OOD检测面临挑战，现有方法在尾类ID数据上表现不佳，导致高误报率和低尾类分类准确率。

Method: 使用预训练模型特征空间初始化图结构，引入高斯化处理激活层分布偏差，然后使用图卷积网络精炼图表示以获得适合长尾OOD检测的特征空间。

Result: 在CIFAR10-LT、CIFAR100-LT和ImageNet-LT三个基准测试中，该方法在FPR和尾类ID分类准确率方面大幅优于现有最先进方法。

Conclusion: 图表示方法能有效提升长尾分布数据集中的OOD检测性能，特别在尾类样本上表现出色。
...
TL;DR: 提出了一种序列级优化框架，生成可在整个行走视频中保持有效的可打印对抗纹理，用于衣物和帽子，在数字和物理环境中都能持续隐藏人体检测。
...
Method: 将产品图像映射到UV空间并转换为紧凑的调色板和控制点参数化，使用物理人体-衣物管道模拟运动、多角度视角、布料动力学和光照变化，通过期望变换目标和时序加权优化控制点。

Result: 实验显示该方法具有强大稳定的隐蔽性，对视角变化具有高鲁棒性，且具有优异的跨模型可迁移性。物理打印的衣物在室内外录制中实现了可靠的检测抑制。

Conclusion: 该方法证明了在真实世界中生成可打印对抗纹理的可行性，能够有效对抗人体检测系统。
...
TL;DR: 提出了一种用于单步图像超分辨率的混合秩（MoR）架构，将稀疏混合专家（MoE）集成到真实图像超分辨率中，通过细粒度专家划分策略和退化感知路由机制实现自适应计算分配。


<details>
  <summary>Details</summary>
Motivation: 现有的真实图像超分辨率方法主要依赖通过LoRA微调预训练扩散模型，但这些密集模型难以自适应捕捉复杂真实世界退化样本的异质特征，也无法在同等计算预算下实现输入间的知识共享。

Method: 1) 将LoRA中的每个秩视为独立专家的细粒度专家划分策略；2) 利用CLIP嵌入和预定义正负文本对计算相对退化分数的退化估计模块；3) 包含零专家槽和退化感知负载均衡损失的动态专家激活机制。

Result: 综合实验验证了该框架的有效性和最先进的性能表现。

Conclusion: MoR架构成功解决了真实图像超分辨率中自适应特征捕捉和计算效率的问题，通过稀疏MoE和退化感知路由实现了优异的性能。
...
TL;DR: 提出了一种基于深度学习的激光散斑模式材料分类方法，用于激光切割过程的监控和控制，解决了激光颜色变化时的分类问题，在多种材料上实现了高精度分类。


<details>
  <summary>Details</summary>
Motivation: 激光切割过程中产生大量粉尘、烟雾和气溶胶，对环境和工人健康构成风险。散斑传感虽能实时监控切割过程和识别材料类型，但传统方法在激光颜色变化时可能失效。

Method: 使用卷积神经网络(CNN)在激光散斑模式数据集上进行训练，识别不同材料类型。该方法专门针对激光颜色变化的情况进行了优化。

Result: 训练集准确率达98.30%，验证集准确率达96.88%。在30种不同材料的3000张新图像上评估，F1分数达到0.9643。
...
Method: 提出课程引导框架，为每个真实姿态(教师)生成多组不同扰动水平的学生视图，通过渐进式课程安排逐步解锁更高扰动级别，结合深度相关性和协同正则化进行训练。

Result: 实验结果表明CuriGS在各种合成和真实稀疏视图场景中，在渲染保真度和几何一致性方面均优于现有最先进方法。

Conclusion: CuriGS通过课程学习和学生视图增强策略，有效解决了稀疏视图3D高斯泼溅重建中的核心挑战，实现了高质量的稀疏视图合成。
...
<details>
  <summary>Details</summary>
Motivation: 解决卫星遥感在农业监测中的空间分辨率限制问题，特别是针对现代微地块管理系统的需求，通过跨模态学习弥补卫星与无人机感知之间的差距。

Method: 使用在美国玉米带五个不同地点收集的约84个杂交玉米品种的共配准卫星-无人机图像对数据集，训练学习传感模态间细粒度光谱空间对应关系的模型。

Result: 从卫星输入生成的无人机类似表示在多个下游任务（包括产量和氮预测）中始终优于真实卫星图像。

Conclusion: 跨模态对应学习具有连接卫星和无人机农业监测的潜力，能够有效提升卫星图像在精细农业应用中的性能。
...
TL;DR: 提出了一个利用大语言模型解决食品识别中领域偏移、长尾分布和细粒度分类挑战的框架，通过生成食品标题和成分，将文本和图像投影到共享嵌入空间进行对齐。


<details>
  <summary>Details</summary>
Motivation: 解决食品识别中的三个主要挑战：训练数据与真实环境图片的领域偏移问题、长尾数据分布以及不同类别食品间细微视觉差异导致的分类困难。

Method: 首先使用LLM解析食品图像生成标题和成分，然后将生成的文本和不同领域的食品图像投影到共享嵌入空间以最大化配对相似度，最后使用对齐后的多模态特征进行识别。

Result: 在两个食品数据集上，该方法在长尾分布、领域适应和细粒度分类方面均优于现有专门方法。

Conclusion: 该框架通过简单而有效的方式，利用LLM生成文本信息并与图像特征对齐，成功解决了食品识别中的多个关键挑战。
...
Method: 基于对注意力模式的分析，提出AMS-KV策略：优先存储关键尺度和局部尺度的KV，通过尺度间相似性分析识别缓存需求层，优化KV缓存利用。

Result: 相比基线方法，AMS-KV将KV缓存使用减少84.83%，自注意力延迟降低60.48%，并能在基线模型内存不足时实现更大批次的稳定扩展。

Conclusion: AMS-KV通过自适应KV缓存策略有效解决了视觉自回归模型中的内存瓶颈问题，显著提升了模型的可扩展性和计算效率。
...
TL;DR: LiSTAR是一个直接在传感器原生几何上操作的4D LiDAR生成世界模型，通过混合柱面-球面表示和时空注意力机制，实现了高保真、可控的4D LiDAR数据合成，在重建、预测和条件生成方面达到最先进性能。
...
Method: 提出LiSTAR模型，采用混合柱面-球面表示来减少量化伪影，使用基于射线中心的时空注意力变换器来捕捉稀疏时间数据中的复杂动态，并提出4D点云对齐体素布局和离散掩码生成框架实现可控合成。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频推理分割方法依赖监督微调，泛化能力有限且缺乏显式推理过程，需要一种能适应分布外场景并具有明确推理能力的新方法。

Method: 采用三阶段架构：1)分层文本引导帧采样模拟人类注意力；2)推理模型生成空间线索和显式推理链；3)使用SAM2和XMem进行分割传播，并引入任务难度感知机制自适应控制推理长度。
...
TL;DR: SpectralTrain是一个通用的、架构无关的训练框架，通过结合课程学习和基于PCA的光谱下采样，在显著降低计算成本的同时提高高光谱图像分类的学习效率。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像分类通常涉及大规模数据和计算密集型训练，限制了深度学习模型在实际遥感任务中的部署。需要一种能提高学习效率的通用训练方法。

Method: 集成课程学习与基于PCA的光谱下采样，逐步引入光谱复杂性同时保留关键信息，支持各种经典和最先进的模型架构。

Result: 在三个基准数据集上的实验显示，训练时间减少2-7倍，精度损失较小，在云分类等气候相关遥感任务中表现出潜力。

Conclusion: 训练策略优化是HSI模型中架构设计的有效补充，SpectralTrain框架在保持性能的同时显著提高了训练效率。
...
TL;DR: Rad-GS是一个基于4D雷达-相机融合的SLAM系统，使用3D高斯作为可微空间表示，适用于公里级户外环境重建。


<details>
  <summary>Details</summary>
Motivation: 解决传统基于相机或LiDAR的3D高斯方法在户外大尺度环境中的局限性，探索使用4D毫米波雷达进行稳健的户外建图。

Method: 结合原始雷达点云与多普勒信息，通过几何增强点云指导同步图像中的动态物体掩码；利用非同步图像帧全局优化3D高斯表示；采用全局八叉树结构和定向高斯基元管理策略。

Result: 在公里级真实环境重建中验证了系统潜力，性能与基于相机或LiDAR的传统3D高斯方法相当。

Conclusion: 证明了使用4D毫米波雷达进行稳健户外大尺度场景重建的可行性。
...
TL;DR: T2T-VICL是一个用于视觉语言模型的跨任务视觉上下文学习框架，通过生成文本提示描述不同视觉任务间的差异，在多个跨任务场景中取得优异表现。
...
Method: 提出T2T-VICL全协作流程，包括设计机制生成和选择能隐式描述不同低层视觉任务差异的文本提示，构建首个跨任务VICL数据集，并提出结合感知评分推理和传统评估指标的新型推理框架。

Result: 在九个跨任务场景中取得顶级结果，在另外十个场景中取得次优表现，突破了视觉语言模型中跨任务VICL的边界。
...
TL;DR: 提出了一种改进4D高斯泼溅方法，通过椭圆误差聚类和分组4D高斯泼溅技术，显著提升了动态场景重建的准确性和时间一致性。
...
Method: 采用椭圆误差聚类和误差校正泼溅添加来精确定位动态区域并初始化拟合泼溅，同时使用分组4D高斯泼溅技术提高泼溅与动态对象映射的一致性。通过将渲染误差分类为缺失颜色和遮挡类型，并基于跨视图颜色一致性进行反向投影或前景分割校正。
...
TL;DR: DCS-LDM提出了一种新的视觉生成范式，通过构建层次化、尺度无关的潜在空间，将信息复杂度与尺度解耦，支持固定潜在表示下任意分辨率和帧率的解码，实现灵活的计算质量权衡。
...
Method: 构建层次化、尺度无关的潜在空间，通过多级标记建模样本复杂度，支持在固定潜在表示下解码到任意分辨率和帧率，采用渐进式粗到细生成范式。
...
TL;DR: VTinker是一个新颖的视频帧插值方法，通过引导流上采样和纹理映射来解决高分辨率运动估计中的模糊和伪影问题，在VFI任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 传统基于光流的视频帧插值方法在低分辨率预测双向光流后使用高倍率上采样，会导致光流边缘模糊或马赛克，且无法充分捕捉高分辨率下的精细像素运动，导致插值帧出现重影和不连续问题。

Method: 提出VTinker管道，包含两个核心组件：引导流上采样(GFU)使用输入帧作为引导来缓解双线性上采样光流的模糊细节；纹理映射生成中间代理帧，从输入帧选择清晰纹理块映射到代理上，通过重建模块生成最终插值帧。

Result: 大量实验表明VTinker在视频帧插值任务中实现了最先进的性能。

Conclusion: VTinker通过引导流上采样和纹理映射有效解决了高分辨率视频帧插值中的运动估计问题，显著减少了重影和不连续伪影。
...
TL;DR: 提出PiN-CLIP方法，通过特征空间的正向激励噪声抑制虚假捷径，提升AI生成图像检测的泛化能力，在42个生成模型的数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展使得真实和合成图像难以区分，现有AI生成图像检测方法在分布外泛化方面存在挑战，主要由于训练时利用了虚假捷径。

Method: 提出PiN-CLIP，联合训练噪声生成器和检测网络，通过交叉注意力融合视觉和类别语义特征构建正向激励噪声，在特征空间注入噪声微调视觉编码器。

Result: 在包含42个不同生成模型的合成图像开放世界数据集上，平均准确率比现有方法提升5.4个百分点，达到新的最先进性能。

Conclusion: PiN-CLIP通过抑制捷径敏感方向、放大稳定取证线索，能够提取更鲁棒和泛化的伪影表示，有效解决了AI生成图像检测的泛化问题。
...
TL;DR: 提出一种预训练的退化表示学习模块和分层终止机制，用于解决压缩视频质量增强中的盲QP问题和计算效率问题。


<details>
  <summary>Details</summary>
Motivation: 现有压缩视频质量增强方法依赖已知QP参数，但在实际场景中QP可能未知；现有盲方法只能捕获全局退化信息，缺乏空间细节；现有方法在不同QP下使用统一架构，忽略了不同压缩级别的计算需求差异。

Method: 使用预训练的退化表示学习模块从视频内容中解耦和提取高维多尺度退化表示来指导伪影去除；引入分层终止机制，根据压缩级别动态调整伪影减少阶段的数量。

Result: 在QP=22时，相比最先进的盲方法PSNR提升了110%（从0.31 dB到0.65 dB）；分层终止机制在QP=22时的平均推理时间比QP=42减少了一半。

Conclusion: 提出的方法在盲压缩视频质量增强任务中显著提升了性能，同时通过动态调整计算资源提高了效率。
...
TL;DR: SR3D是一个用于室内点云实时3D目标检测的新框架，通过空间优先最优传输分配和排名感知自适应自蒸馏来解决训练-推理差距问题，在ScanNet V2和SUN RGB-D数据集上表现出色且保持实时速度。


<details>
  <summary>Details</summary>
Motivation: 解决点云3D目标检测中训练与推理之间的差距问题，这种差距源于训练时缺乏空间可靠性和排名感知，与推理时基于排名的预测选择相冲突，阻碍了模型学习与推理行为对齐的表征。

Method: SR3D包含两个组件：1）空间优先最优传输分配，动态强调位置良好且空间可靠的样本；2）排名感知自适应自蒸馏方案，通过自蒸馏范式自适应注入排名感知。

Result: 在ScanNet V2和SUN RGB-D数据集上的广泛实验表明，SR3D有效弥合了训练-推理差距，在准确率上显著优于先前方法，同时保持实时速度。
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS SLAM系统缺乏开放词汇语义理解能力，而集成语言特征面临高维特征存储开销大、静态模型缺乏适应性等问题。

Method: 使用场景自适应编码器-解码器将高维语言嵌入压缩到紧凑的16维特征空间，实现语言引导的剪枝策略和基于语言的闭环检测。

Result: 在保持渲染质量的同时，将高斯数量减少60%以上，实现15 FPS的实时性能，具有竞争力的建图质量和跟踪精度。

Conclusion: LEGO-SLAM成功解决了3DGS SLAM中语言特征集成的问题，实现了实时开放词汇语义映射，为高级机器人交互提供了基础。
...
TL;DR: 本文提出了多模态交互欺骗评估(MIDA)任务和数据集，评估了12个最先进的多模态大语言模型在识别社交欺骗方面的能力，发现现有模型表现不佳，并提出了Social Chain-of-Thought推理和Dynamic Social Epistemic Memory模块来改进性能。
...
Method: 引入了MIDA任务和包含同步视频文本数据的新数据集；评估了12个最先进的开源和闭源MLLMs；设计了Social Chain-of-Thought推理流程和Dynamic Social Epistemic Memory模块。

Result: 基准测试显示显著的性能差距：即使是GPT-4o等强大模型也难以可靠地区分真假；分析失败模式表明模型无法有效将语言与多模态社交线索相结合，缺乏建模他人知识、信念或意图的能力。
...
Method: RGE方法首先让模型基于指令执行结构化推理生成，然后在推理展开后提取表示，结合对比训练

Result: 在MMEB基准测试中，推理引导的条件化使多模态检索性能比非推理基线提升4.9%

Conclusion: 显式推理能有效增强嵌入质量，RGE方法成功将MLLMs的推理能力融入嵌入提取过程
...
TL;DR: PPCL是一个专门为扩散变换器设计的可插拔剪枝框架，通过层间蒸馏实现参数减少50%，性能损失小于3%。
...
Result: 在多个多模态扩散变换器模型上，PPCL实现了参数量减少50%，关键客观指标下降小于3%，保持了高质量图像生成能力。

Conclusion: PPCL在保持高质量图像生成的同时实现了更高的压缩比，非常适合资源受限环境。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于网格的认知地图方法依赖离散化栅格表示，限制了模型在细粒度空间推理方面的能力。为了克服这一局限性，需要开发能够进行定量空间计算的连续表示方法。

Method: 提出Video2Layout框架，使用连续对象边界坐标来量化对象间物理距离和对象大小。包含两个核心阶段：监督微调阶段从AI2THOR模拟器构建高质量数据集学习视觉输入到精确边界坐标的映射；强化微调阶段增强模型的真实世界泛化能力。

Result: 在QVS-Bench和主流空间推理基准测试中，V2LO-7B模型相比基于网格地图训练的模型平均提升了4.92%，验证了方法的优越性。

Conclusion: 连续边界坐标表示能够有效提升多模态大语言模型的空间推理能力，解决了自然语言描述空间关系时的固有模糊性问题，为空间智能发展提供了新方向。
...
TL;DR: Simba框架将点云补全中的逐点变换回归重新定义为分布学习问题，结合对称先验和扩散模型的生成能力，避免了实例特定记忆化，同时捕捉鲁棒的几何结构。


<details>
  <summary>Details</summary>
Motivation: 解决点云补全中同时保持输入细粒度细节和全局结构完整性的挑战，克服现有回归方法容易过拟合、记忆实例特定变换以及对输入噪声高度敏感的问题。

Method: 将逐点变换回归重新定义为分布学习问题，集成对称先验与扩散模型的生成能力，采用分层Mamba架构实现高保真上采样。

Result: 在PCN、ShapeNet和KITTI基准测试上的广泛实验验证了该方法的最先进性能。

Conclusion: Simba框架通过分布学习方法有效解决了点云补全中的过拟合和噪声敏感性问题，实现了优异的细节保持和结构完整性。
...
TL;DR: 提出LNG-SWR方法，通过层间噪声引导和小波重构实现医学图像分割的鲁棒性，无需牺牲干净精度且计算开销低


<details>
  <summary>Details</summary>
Motivation: 临床部署需要分割模型在分布偏移和扰动下保持稳定，但现有对抗训练方法存在干净-鲁棒性权衡问题，训练成本高，限制了医学影像中的可扩展性

Method: 在训练时向多个层注入小噪声学习频率偏置先验，然后应用先验引导的选择性小波重构进行频率自适应：抑制噪声敏感频带、增强方向结构和形状线索、稳定边界响应

Result: 在CT和超声数据集上，LNG-SWR在干净Dice/IoU上获得一致提升，显著减少强攻击下的性能下降；与对抗训练结合可获得额外增益

Conclusion: LNG-SWR为医学图像分割提供了一条简单、有效且工程友好的鲁棒性路径，适用于对抗性和标准训练机制
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在多模态任务上的成功，其部署效率问题日益突出。现有方法仅能延迟EOS token的出现来隐式延长输出，缺乏稳定性和可控性，无法直接最大化输出token长度作为显式优化目标。

Method: 采用两阶段框架：1) 对抗性提示搜索，使用强化学习策略自动识别能诱导LLM产生冗长输出的对抗提示；2) 视觉对齐扰动优化，在输入图像上构建对抗样本，最大化扰动图像视觉嵌入与对抗提示嵌入的相似度。

Result: 在四个流行视觉语言模型上的综合实验表明，该方法在有效性、效率和泛化能力方面具有显著优势。

Conclusion: 提出的VTIA攻击能够有效诱导视觉语言模型产生冗长文本输出，为解决模型部署效率问题提供了新的视角。
...
TL;DR: EvoVLA是一个自监督的视觉语言动作框架，通过三个组件解决长时程机器人操作中的阶段幻觉问题：阶段对齐奖励、基于姿态的对象探索和长时程记忆，在Discoverse-L基准上比最强基线提升10.2个百分点，达到69.2%成功率。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型存在阶段幻觉问题，代理利用粗糙评估信号来走捷径完成多步骤任务，报告高进度但未真正完成任务。

Method: 1. 阶段对齐奖励：使用三元组对比学习和Gemini生成的困难负样本来防止视觉捷径；2. 基于姿态的对象探索：在相对对象-夹爪姿态而非原始像素上建立好奇心；3. 长时程记忆：使用选择性上下文保留和门控融合来稳定扩展执行过程中的内在塑造。

Result: 在Discoverse-L长时程操作基准上，EvoVLA比最强基线（OpenVLA-OFT）平均任务成功率提高10.2个百分点，达到69.2%；样本效率提高1.5倍；阶段幻觉从38.5%降至14.8%。真实世界部署在物理机器人上达到54.6%平均成功率，优于OpenVLA-OFT 11个百分点。
...
Method: 采用Mamba-Transformer混合骨干网络，结合状态空间模型的效率和注意力机制的表达能力，提出TransV模块将视觉token转移压缩到指令token中。

Result: 在多个基准测试中与最先进模型竞争，同时能处理超过10,000帧的时长视频，并分析了混合模型的注意力行为。
...
TL;DR: 本文提出RF-CLIP方法，通过模拟人类注意力分散-重新聚焦行为，过滤CLIP中的干扰token，提升开放词汇语义分割的像素级多模态对齐能力。
...
Method: 提出RF-CLIP训练免费方法，识别并过滤由维度特定过度激活产生的干扰token，重新将注意力聚焦到目标区域。

Result: 在八个基准测试中达到最先进性能，同时保持高推理效率。

Conclusion: 通过模拟人类注意力重新聚焦机制，可以有效提升CLIP在密集预测任务中的多模态对齐粒度。
...
Conclusion: SurvAgent通过分层思维链增强的多智能体系统，成功解决了现有方法的局限性，为临床可解释的生存预测提供了有效解决方案。
...
TL;DR: Mantis是一个新颖的Vision-Language-Action框架，通过解耦视觉预测来提升模型性能，在LIBERO基准测试中达到96.7%的成功率，超越了现有基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型直接预测高维视觉状态会分散模型容量并带来高昂训练成本，而压缩视觉状态会导致信息瓶颈，同时忽视语言监督会导致理解和推理能力不足。

Method: 提出解耦视觉预测(DVF)，使用元查询和扩散Transformer头将视觉预测从主干网络解耦，通过残差连接提供当前视觉状态，使元查询自动捕捉描述视觉轨迹的潜在动作。

Result: 在LIBERO基准测试中达到96.7%的成功率，收敛速度快，在真实世界评估中超越π₀.₅模型，在指令跟随、泛化到未见指令和推理能力方面表现优异。
...
TL;DR: 提出DSLGA模型解决无监督域自适应可见光-红外行人重识别问题，通过两阶段方法处理域间和域内模态差异，在多个设置下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有VI-ReID算法在公共数据集上表现优异，但由于公共数据与现实数据存在差异，在实际应用中效果不佳。需要解决无监督域自适应问题，将知识从公共数据迁移到现实数据而不需要新样本标注。

Method: 提出两阶段DSLGA模型：1)预训练阶段使用域共享学习策略(DSLS)缓解域间模态差异；2)微调阶段使用渐进对齐策略(GAS)通过簇到整体的方式处理域内跨模态对齐挑战。

Result: 大量实验表明该方法在多种设置下显著优于现有的VI-ReID域自适应方法，甚至优于一些有监督方法。

Conclusion: DSLGA模型有效解决了UDA-VI-ReID中的域间和域内模态差异问题，提出的CMDA-XD测试方法为训练和测试不同模型提供了新基准。
...
TL;DR: TwiG框架首次实现了在视觉生成过程中交织文本推理，通过零样本提示、监督微调和强化学习三种策略，使文本推理与视觉生成协同进化，产生更具上下文感知和语义丰富的视觉输出。


<details>
  <summary>Details</summary>
Motivation: 现有视觉生成方法只在生成前（预规划）或生成后（后优化）进行文本推理，缺乏在生成过程中的多模态实时交互。

Method: 提出TwiG框架，在视觉内容逐步生成时交织文本推理，指导即将生成的局部区域并反思已合成内容。研究了零样本提示、监督微调（TwiG-50K数据集）和强化学习（TwiG-GRPO策略）三种实现方法。
...
<details>
  <summary>Details</summary>
Motivation: 人类器官由相互连接的子结构组成，其几何形状和空间关系相互制约。大多数深度学习方法将这些部分独立处理，导致解剖学上不合理的重建结果。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单模态扰动，忽视了影响具身推理和决策的根本问题——跨模态错位，而VLA模型的对抗鲁棒性在真实多模态和黑盒条件下尚未被充分探索
...
Conclusion: 具身VLA模型对多模态扰动高度敏感，跨模态错位攻击特别有效，强调了在真实部署中增强多模态鲁棒性的必要性
...
TL;DR: 提出ICCE方法，通过聚类集成策略改进无监督图像分类性能，在多个基准数据集上达到最先进水平，首次在ImageNet上超过70%准确率


<details>
  <summary>Details</summary>
Motivation: 现有无监督图像分类方法要么将表示学习和聚类结合在迭代框架中，要么仅关注聚类而忽略表示学习。本文旨在通过聚类集成策略提升聚类性能
...
TL;DR: 提出SwiTrack框架，通过三个专用流处理跨模态目标跟踪，包括视觉编码器、NIR门控适配器和一致性轨迹预测模块，在RGB-NIR跟踪中实现最先进性能。
...
Method: 部署三个专用流：RGB帧由视觉编码器处理，NIR帧通过NIR门控适配器与视觉编码器结合进行细化，无效模态使用一致性轨迹预测模块估计目标运动。还包含动态模板重建和相似性对齐损失。

Result: 在最新基准测试中实现最先进性能，精度率和成功率分别提升7.2%和4.3%，同时保持65帧/秒的实时跟踪。

Conclusion: SwiTrack通过状态切换框架重新定义了跨模态目标跟踪，能够有效提取模态特定特征并缓解目标漂移问题，在RGB-NIR跟踪中表现出色。
...
TL;DR: 提出了一种基于MLP骨干网络的新方法，通过残差连接和Memory-Block组件，从稀疏传感器输入生成完整全身动作，在移动HMD上达到72FPS并显著减少预测误差。


<details>
  <summary>Details</summary>
Motivation: 现有AR/VR系统主要通过HMD和控制器追踪头部和手部，导致3D全身重建不完整，需要从有限传感器输入生成完整全身动作。

Method: 使用多层感知机（MLP）骨干网络，增强残差连接和Memory-Block组件。Memory-Block用可训练代码向量表示缺失传感器数据，结合先前时间实例的稀疏信号提高时间一致性。采用多任务学习框架学习鲁棒表示。

Result: 实验表明该方法优于现有最先进基线，显著减少预测误差，在移动HMD上达到72FPS，改善了精度-运行时间权衡。

Conclusion: 该方法能够从稀疏传感器输入有效生成完整、平滑的全身动作，为AR/VR应用提供沉浸式体验。
...
TL;DR: TetraSDF是一个精确的解析网格化框架，用于处理带有四面体位置编码的ReLU MLP表示的SDF，通过保持全局连续分段仿射结构实现精确等值面提取。


<details>
  <summary>Details</summary>
Motivation: 现有方法在提取神经SDF的零等值面网格时存在挑战：基于采样的方法引入离散化误差，而连续分段仿射解析方法仅适用于普通ReLU MLP。
...
Conclusion: TetraSDF提供了一种实用且内存高效的精确解析网格化解决方案，能够生成与学习等值面高度一致的网格。
...
TL;DR: 提出基于视觉门控生成变换器（VGGT）的快速时空场景理解框架，支持接近实时的辅助导航应用，通过滑动窗口处理图像流来克服内存需求，实现3D场景表示的连续更新。


<details>
  <summary>Details</summary>
Motivation: 开发高效的时空场景理解系统，支持辅助导航等实时应用，解决VGGT高内存需求的问题，实现3D场景的持续更新和语义理解。

Method: 使用滑动窗口处理图像流并对齐子图，利用VGGT跟踪头将2D语义实例掩码聚合为3D对象，存储时间戳和实例级身份以实现时间一致性和环境变化检测。

Result: 在知名基准和定制辅助导航数据集上评估，结果表明该框架适用于现实世界场景。
...
<details>
  <summary>Details</summary>
Motivation: 杂草检测是精准农业的关键环节，准确的物种识别能让农民选择性施用除草剂，符合可持续农业管理需求。
...
<details>
  <summary>Details</summary>
Motivation: 考虑到移动设备部署的优势（数据隐私、无需网络连接、响应更快），将3DGS优化到移动GPU上具有重要价值。

Method: 开发了针对2D纹理缓存优化的新颖排序算法，改进了变量布局设计，并对3DGS算法的其他步骤进行了加速优化。

Result: Texture3dgs在排序和整体3D场景重建上分别实现了4.1倍和1.7倍的加速，同时内存使用减少了1.6倍。
...
<details>
  <summary>Details</summary>
Motivation: 尽管视觉基础模型在各种下游任务中表现出强大的泛化能力，但其表示通常被下采样14x/16x（如ViT），限制了在像素级应用中的直接使用。现有的特征上采样方法依赖于数据集特定的重新训练或繁重的隐式优化，限制了可扩展性和泛化性。

Method: 通过简单的每图像优化学习各向异性高斯核，结合空间和范围线索，有效桥接高斯泼溅和联合双边上采样。学习的核作为通用、边缘感知的算子，可在架构和模态间无缝迁移。

Result: 在224x224图像上仅需约0.419秒运行时间，在语义分割、深度估计以及深度和概率图上采样方面达到最先进性能。

Conclusion: Upsample Anything提供了一个高效、通用的解决方案，能够精确重建特征、深度或概率图的高分辨率表示，解决了现有方法在可扩展性和泛化性方面的限制。
...
TL;DR: 将稀疏自编码器重新解释为主题模型，提出SAE-TM框架，通过SAE学习可重用的主题原子，在文本和图像数据集上生成更连贯的主题，并应用于跨模态的大规模主题分析。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器在嵌入分析中的作用和实用价值存在争议，需要新的视角来理解其本质和潜力。

Method: 将潜在狄利克雷分配扩展到嵌入空间，推导出SAE作为最大后验估计器；提出SAE-TM框架：训练SAE学习主题原子，将其解释为下游数据的词分布，无需重新训练即可合并成任意数量主题。

Result: SAE-TM在文本和图像数据集上比强基线生成更连贯的主题，同时保持多样性；成功分析了图像数据集的主题结构和日本木版画的主题随时间变化。

Conclusion: SAE是跨模态大规模主题分析的有效工具，为理解嵌入空间提供了新的理论框架。
...
TL;DR: BioBench是一个生态视觉基准测试，解决了ImageNet-1K在科学图像评估中的局限性，通过整合9个公开任务、4个生物分类界和6种采集模态的310万张图像，为生态学计算机视觉提供更可靠的评估标准。


<details>
  <summary>Details</summary>
Motivation: ImageNet-1K线性探测准确率作为视觉表示质量的默认代理指标，在科学图像上不再能有效预测性能，需要开发专门针对生态学领域的可靠基准测试。

Method: BioBench统一了9个公开的、应用驱动的任务，涵盖4个生物分类界和6种采集模态（无人机RGB、网络视频、显微图像、现场和标本照片、相机陷阱帧），总计310万张图像，提供Python API进行数据下载和轻量级分类器训练。

Result: 在46个现代视觉模型检查点上，ImageNet top-1准确率仅能解释生态学任务中34%的方差，并且在75%以上准确率的模型中错误排名30%。BioBench提供了生态学计算机视觉的新信号。

Conclusion: BioBench为生态学计算机视觉提供了新的评估标准，并为在任何领域构建可靠的AI-for-science基准测试提供了模板方法。
...
Result: NaTex在纹理一致性和对齐方面显著优于先前方法，并展现出强大的泛化能力，能够用于材质生成、纹理优化、部件分割和纹理化等各种下游应用。

Conclusion: NaTex通过原生3D纹理生成的新范式，有效解决了传统2D多视图方法的局限性，在纹理质量和对齐精度方面取得了显著提升，同时具备良好的扩展性和应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 当前混合方法虽然性能强劲但计算成本高，限制了在实时场景中的实用性。需要开发既高效又有效的解决方案。

Method: 集成三个可解释先验：1）自适应白平衡缓解波长相关颜色衰减；2）小波增强块进行多频带分解捕获全局结构和细节纹理；3）梯度感知模块使用可学习门控Sobel算子显式保护边缘结构。

Result: 在基准数据集上的广泛实验表明，WWE-UIE以显著更少的参数和FLOPs实现了竞争性的恢复质量，支持资源受限平台上的实时推理。消融研究和可视化验证了各组件贡献。

Conclusion: WWE-UIE提供了一个紧凑高效的解决方案，在保持高质量水下图像恢复的同时实现了实时性能，为资源受限环境中的实际应用铺平了道路。
...
TL;DR: ChangeDINO是一个用于光学建筑变化检测的多尺度Siamese框架，通过融合轻量级骨干网络和冻结DINOv3特征，结合空间-光谱差异transformer解码器和可学习形态学模块，在多个公开基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的遥感变化检测方法主要依赖变化图标注，未充分利用非变化区域的语义信息，导致在光照变化、离天底视角和标签稀缺情况下鲁棒性不足。

Method: 提出端到端多尺度Siamese框架，融合轻量级骨干网络和冻结DINOv3特征构建语义丰富的特征金字塔；使用空间-光谱差异transformer解码器利用多尺度绝对差异作为变化先验；采用可学习形态学模块优化上采样logits以恢复清晰边界。
...
<details>
  <summary>Details</summary>
Motivation: 现有FSR方法受限于固定上采样尺度和对输入尺寸变化的敏感性，需要一种更灵活的人脸超分辨率方法。

Method: 使用2D深度特征、局部相对坐标和上采样比例预测RGB值；局部频率估计模块捕获高频纹理；全局坐标调制模块利用先验人脸结构知识。

Result: 定量和定性评估表明ARASFSR在各种输入尺寸和上采样尺度下都优于现有最先进方法。

Conclusion: ARASFSR方法在任意分辨率和尺度下都能稳健地进行人脸超分辨率，解决了现有方法的局限性。
...
TL;DR: 该研究提出了一种基于师生架构的弱监督学习方法，用于无人机遥感中的地形分类任务，仅使用30%标注数据就能同时提升mIoU和时序一致性。
...
Result: 仅使用30%标注数据，该方法同时提高了mIoU和时序一致性，确保地形物体的稳定定位。

Conclusion: 提出的框架克服了传统时序一致性训练在航空任务中的缺陷，为无人机遥感地形分类提供了有效的弱监督解决方案。
...
Conclusion: 该方法提供了一种在全局LiDAR坐标系中实现精确、无漂移相机定位的有效解决方案，适用于机器人和扩展现实应用。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中RGB-D数据由于硬件限制和校准漂移难以严格对齐，现有方法在不对齐场景下性能下降。

Method: 使用多阶匹配机制（零阶、一阶、二阶匹配）从不对齐RGB中自适应检索相关信息，并通过多阶聚合策略选择性融合特征。
...
<details>
  <summary>Details</summary>
Motivation: 现有离线签名验证方法依赖整体特征进行配对比较，忽视了细粒度差异的重要性。基于transformer的骨干网络可能会自然模糊局部细节，影响验证性能。

Method: 提出DetailSemNet模型，采用局部结构匹配方法，引入细节语义集成器，通过特征解缠和重缠来增强细节并扩展判别语义。

Result: 在离线签名验证的领先基准测试中，模型始终优于最新方法，以明显优势达到最先进结果，并在跨数据集测试中表现出卓越的泛化能力。

Conclusion: 局部结构匹配不仅提高了性能，还增强了模型的可解释性，结合泛化能力和可解释性，显著增强了DetailSemNet在实际应用中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于CLIP的CZSL方法主要依赖图像编码器的全局语义表示，但该表示能力有限且无法完全解缠属性和对象语义。
...
Conclusion: CAMS通过提取视觉特征的语义特征并在多维空间中进行语义解缠，有效提升了组合零样本学习的泛化能力。
...
TL;DR: 提出了一种新的运动捕捉基本单元——刚性体标记(RBM)，通过稀疏6自由度数据和基于测地线损失的深度学习回归模型，实现了实时高保真运动捕捉。
...
Method: 引入刚性体标记(RBM)作为新的运动捕捉单元，开发基于深度学习的回归模型，在测地线损失下直接估计SMPL参数。

Result: 端到端方法在性能上与基于优化的方法相当，但计算量减少了一个数量级以上，在AMASS数据集上实现了最先进的姿态估计精度。

Conclusion: 结合稀疏6自由度RBM和流形感知测地线损失，为图形学、虚拟现实和生物力学中的实时运动捕捉提供了实用且高保真的解决方案。
...
Method: 基于相机内外参，将各图像的3D点投影到共享圆柱体建立邻域关系，使用非学习的空间注意力机制聚合跨图像特征

Result: 在DDAD和nuScenes数据集上评估，相比现有方法提高了深度估计的跨视图一致性和整体精度
...
TL;DR: 提出了两种结合视觉变换器和图神经网络的图基分割模型，用于改善腹腔镜胆囊切除术中肝囊解剖结构的识别，在遮挡、长距离依赖和精细几何结构分割方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 准确识别肝囊解剖结构对于预防腹腔镜胆囊切除术并发症至关重要。深度学习模型在处理遮挡、长距离依赖和罕见结构的精细几何特征方面存在困难。

Method: 提出了两种分割模型：1）使用静态k最近邻图和GCNII的模型，实现稳定的长距离信息传播；2）使用动态可微分图生成器和GAT的模型，支持自适应拓扑学习。两种模型都在Endoscapes-Seg50和CholecSeg8k基准上进行了评估。

Result: 提出的方法在mIoU上比现有最佳基线提高了7-8%，在mDice上提高了6%，特别是在薄、罕见和安全关键结构上产生了解剖学上一致的预测。

Conclusion: 图基分割方法提高了手术场景分割的性能和解剖一致性。通过结合ViT的全局上下文和图基关系推理，模型提高了可解释性和可靠性，为通过精确识别关键解剖特征实现更安全的腹腔镜和机器人辅助手术铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 现有的少样本分割方法主要从支持图像中挖掘参考作为元指导，但由于视觉表示中的类内变化，从支持图像提取的元信息无法为未训练类别提供准确指导。本文认为支持图像的参考可能并非必需，关键在于为训练和未训练类别提供无偏的元指导。

Method: 设计了语言驱动的属性泛化（LDAG）架构，包含多属性增强（MaE）模块和多模态属性对齐（MaA）模块。MaE通过大语言模型生成目标类的多个详细属性描述，构建精炼的视觉-文本先验指导；MaA实现属性文本与视觉特征的跨模态交互。

Result: 实验表明，所提出的方法明显优于现有方法，并实现了新的最先进性能。

Conclusion: 通过利用语言描述而非仅依赖视觉支持图像，可以构建更鲁棒和无偏的少样本分割支持策略，有效解决类内变化带来的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有垃圾检测数据集缺乏对垃圾容器监控的专门标注，特别是在垃圾车拍摄图像中的溢出容器跟踪问题研究不足，限制了实际物流应用。

Method: 构建包含垃圾和垃圾容器的城市场景数据集，为三个任务提供基准模型，并提出启发式跟踪方法和利用几何先验的模型无关分割框架。
...
TL;DR: VLA-Pruner是一种专门针对Vision-Language-Action模型的token剪枝方法，通过双层级重要性标准和自适应token选择策略，在保持计算效率的同时提升VLA模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLM token剪枝方法仅基于语义显著性选择token，忽视了VLA模型的双系统特性（高层语义理解和低层动作执行），导致丢弃对动作生成关键的信息，显著降低VLA性能。

Method: 提出双层级重要性标准：视觉-语言预填充注意力用于语义相关性，通过时间平滑估计的动作解码注意力用于动作重要性。基于此标准设计自适应双层级token选择策略。

Result: 实验表明VLA-Pruner在多种VLA架构和多样化机器人任务中实现了最先进的性能。

Conclusion: VLA-Pruner通过考虑VLA模型的双系统特性和机器人操作的时间连续性，有效解决了现有token剪枝方法在VLA应用中的局限性。
...
TL;DR: LLaVA³通过多视角2D图像提升VLM的3D场景理解能力，无需微调，在3D视觉问答和语言定位任务上超越现有2D方法。
...
Method: 受立体派画家启发，通过多视角3D重建生成物体的全方位视觉表示，为VLM描述3D场景，仅使用多视角2D图像且无需微调。

Result: 在3D视觉问答和3D语言定位任务上的广泛实验表明，该方法优于之前的2D基础VLM解决方案。

Conclusion: LLaVA³通过创新的全方位视觉表示方法，有效提升了VLM的3D场景理解能力，为3D多模态学习提供了新思路。
...
TL;DR: FastSurfer-CC是一个高效、全自动的胼胝体形态测量框架，能够自动识别矢状面切片、分割胼胝体和穹窿，生成厚度剖面和分区，并提取8个形状指标用于统计分析。
...
Result: FastSurfer-CC在各项任务中优于现有专门工具，并在亨廷顿病研究中检测到当前最先进方法未能发现的显著差异。
...
Method: 使用标准的多对象潜在变换器架构进行未来帧预测，但修改该架构以融入深度和点流信息。在合成和真实世界数据集上进行受控评估，不仅使用基于外观的指标，还使用对象掩码上的Wasserstein距离来有效衡量预测的运动分布。

Result: 研究发现，当预测模型辅助以点流和深度信息时，在遮挡场景下表现更好，并且相比不使用这些模态的模型，能够预测更准确的背景运动。

Conclusion: 通过整合点流和深度信息，视频预测模型在遮挡场景下的性能得到显著提升，证明了这些额外模态对于处理复杂视频预测任务的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 当前微机器人姿态估计方法严重依赖高质量显微镜图像数据集，但这些数据集获取困难且成本高昂。现有数字孪生系统难以复现复杂的光学显微镜现象。

Method: 将基于波动光学的物理渲染和深度对齐集成到生成对抗网络中，合成高保真显微镜图像。

Result: 相比纯AI驱动方法，结构相似性指数提高35.6%，保持实时渲染速度(0.022秒/帧)。使用合成数据训练的姿势估计器达到93.9%/91.9%(俯仰/滚转)准确率。

Conclusion: 该框架能够泛化到未见过的姿势，无需额外训练数据即可实现数据增强和鲁棒的微机器人姿态估计。
...
Conclusion: 利用图像采集时间信息可以有效改进乳腺DCE-MRI肿瘤分割，增强模型在不同数据集上的泛化性能。
...
TL;DR: 提出一种联合室内场景建图和天花板相机注册的新方法，通过移动代理的RGB-D相机和同步的CMC视频，在统一框架中同时完成两个任务并提升性能。
...
Method: 使用配备头戴RGB-D相机的移动代理遍历场景，同步采集CMC视频。通过相关时间戳对齐轨迹，构建因子图联合优化自我相机位姿、场景布局和CMC位姿。

Result: 实验结果表明该方法不仅能有效完成两个任务，还能联合提升它们的性能，为下游位置感知应用提供可靠工具。

Conclusion: 提出的方法为室内场景建图和CMC注册提供了一个统一的解决方案，显著提高了注册精度和效率。
...
TL;DR: 提出了一个专门用于拳击击打检测和分类的全面、高质量视频数据集，包含6,915个拳击片段，分为6种拳击类型，来自20个YouTube训练视频，涉及18名运动员。


<details>
  <summary>Details</summary>
Motivation: 由于动作的动态性和非结构化特性以及录制环境的变化，开发鲁棒的数据集一直是计算机视觉分析格斗运动的主要瓶颈。

Method: 从20个公开的YouTube训练视频中提取6,915个高质量拳击片段，手动分割和标注以确保精确的时间边界和类别一致性，涵盖广泛的运动风格、摄像机角度和运动员体型。

Result: 创建了一个包含6种不同拳击类型的全面数据集，专门用于支持实时视觉动作识别研究，特别是在低资源和无约束环境中。

Conclusion: 通过提供具有多样化拳击示例的丰富基准，这项贡献旨在加速拳击及相关领域中运动分析、自动化教练和性能评估的进展。
...
TL;DR: 本文提出SemCLIP方法，通过结合释义和否定，改进CLIP模型的对比损失函数，使用LLM生成的训练三元组（原始、释义和否定文本）来增强模型对语义变换的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP模型在处理否定和释义文本时表现不稳定，因为否定会以最小词汇变化彻底改变含义，而释义可能产生完全不同但含义相同的表达，这对提升视觉-语言模型的对齐效果提出了挑战。

Method: 提出SemCLIP方法：1）评估释义和否定的组合效果；2）设计新的对比损失函数考虑释义和否定；3）使用LLM生成的训练三元组（原始、释义、否定文本）训练CLIP类模型。

Result: 在CC-Neg基准测试中，图像检索准确率从68.1%提升到78.1%；在Sugarcrepe++基准测试中表现优于仅用否定文本训练的模型；在下游零样本分类任务中全面优于CLIP。

Conclusion: SemCLIP能够显著提升模型对语义变换的鲁棒性，在保持CLIP性能的同时，有效增加与否定文本的嵌入距离，改善模型对复杂语义的理解能力。
...
TL;DR: 提出了一种鲁棒的多摄像机体操运动员跟踪方法，通过结合体操领域知识（运动员3D中心位于预设垂直平面）和级联数据关联策略，在交叉视角检测不足时使用射线-平面相交生成共面3D轨迹候选，解决了体操场馆中摄像机数量有限和检测失败带来的跟踪挑战。


<details>
  <summary>Details</summary>
Motivation: 体操运动员跟踪面临独特挑战：场馆空间限制导致摄像机数量有限；光照变化、背景、服装和遮挡导致多摄像机检测在某些视角失效，通常只能从两个相对视角获得有效检测。这些因素使得传统多摄像机三角测量难以准确确定运动员的3D轨迹。

Method: 结合体操领域知识，利用运动员3D中心在大部分表演时间内位于预设垂直平面的特点，采用射线-平面相交生成共面3D轨迹候选。提出新颖的级联数据关联范式：当交叉视角检测充足时使用三角测量生成3D轨迹候选，不足时使用射线-平面相交。共面候选用于补偿不确定轨迹，最小化跟踪失败。

Result: 通过广泛实验验证了方法的鲁棒性，在挑战性场景中优于现有方法。配备该跟踪方法的体操评分系统已成功应用于最近的体操世锦赛，并获得国际体操联合会的高度认可。

Conclusion: 该方法通过结合领域知识和级联数据关联策略，有效解决了体操运动员跟踪中的特殊挑战，在实际应用中表现出优异的性能和鲁棒性。
...
TL;DR: 本文分析了局部和全局光流计算方法，重点关注Horn-Schunck算法，实现了多分辨率版本，并研究了在不同图像条件下的运动估计效果。


<details>
  <summary>Details</summary>
Motivation: 研究局部方法（如Lucas-Kanade）和全局方法（如Horn-Schunck）在光流计算中的理论和实践表现，探索结合多分辨率策略来提升运动估计的准确性和收敛性。

Method: 实现Horn-Schunck算法的多分辨率版本，使用双线性插值和延拓操作，对比分析局部和全局方法在不同图像条件下的性能。

Result: 多分辨率Horn-Schunck算法通过结合双线性插值和延拓操作，提高了光流计算的准确性和收敛速度，在不同图像条件下表现出更好的运动估计效果。

Conclusion: 全局方法结合多分辨率策略能够有效提升光流计算的性能，Horn-Schunck算法在多分辨率框架下展现出更好的准确性和鲁棒性，适用于各种图像条件下的运动分析。
...
TL;DR: 提出一个两阶段检测框架，通过对比学习提取图像嵌入特征，结合k-NN分类器在少样本学习范式下实现合成图像检测，显著提升检测准确率和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 生成式AI快速发展导致合成图像越来越难以与真实内容区分，传统检测方法依赖定期重新训练，在新型生成模型快速迭代的背景下计算不可行且操作不实用。

Method: 第一阶段使用监督对比学习的视觉深度学习模型提取图像嵌入特征，训练时故意保留部分生成器架构以测试跨生成器泛化能力；第二阶段使用k-NN分类器在嵌入空间进行少样本学习，仅需每个类别150张图像。

Result: 在少样本学习条件下，平均检测准确率达到91.3%，比现有方法提升5.2个百分点；在源归属任务中，AUC和OSCR分别提升14.70%和4.27%。

Conclusion: 该框架为实现鲁棒、可扩展的取证归属系统提供了重要进展，能够适应不断演变的生成AI环境，无需进行耗尽的重新训练。
...
TL;DR: EOGS++是基于3D高斯泼溅的卫星图像重建方法，直接在原始高分辨率全色数据上运行，无需外部预处理，通过光流技术将束调整嵌入训练过程，提升相机姿态估计精度。


<details>
  <summary>Details</summary>
Motivation: 扩展地球观测高斯泼溅框架，针对卫星图像开发更高效的重建方法，避免依赖外部预处理和优化工具，同时提升重建质量和几何精度。

Method: 使用原始高分辨率全色数据，在训练过程中嵌入基于光流的束调整，引入早停机制和TSDF后处理，改进相机姿态估计和几何重建。

Result: 在IARPA 2016和DFC2019数据集上，EOGS++在重建质量和效率方面达到最先进性能，建筑重建的MAE误差从1.33降至1.19，优于原始EOGS和其他基于NeRF的方法。

Conclusion: EOGS++成功提升了卫星图像3D重建的质量和效率，保持了高斯泼溅的计算优势，为地球观测应用提供了更有效的解决方案。
...
TL;DR: VARiant通过权重共享的子网络架构，在保持VAR模型质量的同时大幅降低内存消耗和加速推理。


<details>
  <summary>Details</summary>
Motivation: VAR模型的多尺度生成导致累积KV缓存占用大量内存，限制了实际部署。研究发现VAR模型存在尺度-深度不对称依赖：早期尺度对网络深度极度敏感，而后期尺度对深度减少具有鲁棒性。

Method: 从30层VAR网络中通过等距采样选择多个子网络（16层到2层），早期尺度使用完整网络处理，后期尺度使用子网络。采用渐进式训练策略解决权重共享带来的优化冲突。
...
Method: 设计了紧凑而富有表达力的主干网络确保可扩展性，精心构建了混合成本聚合模块，并提出了在百万级数据上的三阶段训练策略来有效弥合模拟到现实的差距。

Result: 在四个广泛使用的真实世界基准测试中排名第一，准确性与最先进的非先验精确方法相当甚至超越，同时计算成本不到1%。

Conclusion: 该研究证明了超轻量模型能够提供强大的泛化能力，为高效立体匹配设立了新标准。
...
Result: 在临床研究中，医生对其准确性和效率分别评分为4.3/5和4.6/5；在2141名儿童数据集上达到0.79召回率和0.82 AUC，人体测量RMSE显著降低；跨数据集结果显示使用人口统计匹配知识库可获得高达25%的召回率提升和3.5厘米RMSE减少。
...
Method: 提出点图编码3D坐标到结构化2D网格的方法，设计视图到场景对齐策略转移2D先验，引入POMA-JEPA架构确保多视图几何一致性，构建ScenePoint数据集进行大规模预训练。

Result: POMA-3D成为专业和通用3D理解的强大骨干网络，支持3D问答、具身导航、场景检索和具身定位等任务，仅使用几何输入即可实现。

Conclusion: POMA-3D通过点图方式成功解决了3D表示学习中的预训练先验和数据稀缺问题，为3D场景理解提供了有效的新途径。
...
Method: 使用师生蒸馏范式，通过LoRA约束的子空间更新，在低秩解码器空间中擦除特定表示，同时保持全局解剖理解。包括强遗忘阶段和温和恢复阶段。

Result: 在ISIC分割中，遗忘集IoU从0.875降至0.509，保留集和验证集性能保持竞争力；在ISIC分类中，遗忘集准确率从87.0%降至64.1%，保留集准确率从83.9%提升至90.6%。

Conclusion: 基于LoRA的子空间遗忘为医学图像分析提供了负责任、可控且可逆的遗忘途径，能够在保留关键性能的同时忘记敏感样本或结构。
...
TL;DR: 利用生成式AI技术合成标注烟雾数据集，通过无监督域适应方法解决烟雾分割中的合成与真实数据差距问题，并集成风格迁移、GAN和图像抠图等技术提升合成数据真实性。


<details>
  <summary>Details</summary>
Motivation: 早期野火检测对减少大规模损害至关重要，但烟雾检测的大规模标注数据集稀缺限制了深度神经网络的应用潜力。

Method: 使用生成式AI合成标注烟雾数据集，探索无监督域适应方法进行烟雾分割，集成风格迁移、GAN和图像抠图技术提升合成数据真实性。

Result: 提出的方法能够有效缩小合成数据与真实数据之间的域差距，为更准确和可扩展的野火检测模型铺平道路。

Conclusion: 生成式AI和域适应技术的结合为解决烟雾检测数据稀缺问题提供了有效途径，有望显著提升野火早期检测的准确性。
...
TL;DR: 提出了SAM2S模型，通过构建最大的手术视频分割基准SA-SV，并引入DiveMem记忆机制、时序语义学习和抗模糊学习，显著提升了手术交互式视频分割的长期跟踪性能和零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决手术视频分割中交互式分割模型（如SAM2）面临的领域差距和长期跟踪限制问题，需要构建专门的手术基准并改进模型以适应手术场景的特殊需求。

Method: 1) 构建SA-SV基准数据集；2) 提出SAM2S模型，包含DiveMem多样记忆机制、时序语义学习和抗模糊学习三个关键组件；3) 在SA-SV上进行微调。

Result: SAM2S在SA-SV基准上达到80.42平均J&F分数，比原始SAM2提升17.10分，比微调SAM2提升4.11分，同时保持68FPS实时推理和强零样本泛化能力。

Conclusion: SAM2S通过专门的手术基准和针对性改进，显著提升了手术视频分割的长期跟踪性能和泛化能力，为计算机辅助手术提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的目标检测面临长尾分布挑战，即大量类别只有少量实例。这种类别不平衡会导致检测模型偏向频繁类别，在稀有类别上性能下降。

Method: 采用两阶段Faster R-CNN架构，改进Balanced Group Softmax框架缓解类别不平衡；探索度量学习生成良好分离的特征嵌入，并使用k-NN方法提升分类性能。

Result: 在LVISv1数据集上达到24.5%的mAP，超越了之前的24.0%基准，特别是在稀有类别上表现出色。

Conclusion: 提出的方法在长尾目标检测中有效，通过平衡分类和度量学习显著提升了稀有类别的检测性能。
...
TL;DR: 提出自适应引导上采样(AGU)方法，通过多参数优化同时提升低光图像的多个质量特性，如降噪和锐化，能够在实时处理中生成高质量图像。


<details>
  <summary>Details</summary>
Motivation: 传统引导图像方法在处理低光图像时效果不佳，因为低光图像噪声高、亮度低，缺乏足够的特征用于引导传输。

Method: 基于引导图像方法，通过多参数优化学习低光图像与明亮图像特征之间的关联，仅需少量样本图像对进行机器学习。

Result: 实验证明AGU在低光图像处理场景中优于现有最先进方法，能够实时处理低质量、低分辨率输入并生成高质量图像。

Conclusion: AGU方法有效解决了低光图像引导上采样的挑战，通过多特征学习实现了显著的质量提升。
...
Conclusion: SAM 3D突破了3D数据限制，在自然图像中的3D物体重建方面取得了显著进展，将发布代码、模型权重、在线演示和新的挑战性基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D高斯扩散模型由于大量高斯基元导致去噪和后续处理耗时，生成速度慢且沿采样轨迹的可扩展性有限。

Method: 提出轻量级选择器模型评估潜在高斯基元质量，实现早期轨迹缩减；引入实例掩码去噪在每一步去噪过程中修剪冗余背景区域的高斯基元。

Result: 实验表明TRIM显著提高了3D生成的效率和质量。

Conclusion: TRIM在不影响输出质量的前提下，有效加速了3D高斯扩散模型的推理过程，支持推理时的可扩展性。
...
TL;DR: 提出了一种解决3D层次语义分割中多层级冲突和类别不平衡问题的新框架，包含主分支和辅助分支，通过解耦架构和双向监督机制提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D层次语义分割方法存在两个关键问题：多层级优化冲突和跨层级类别不平衡，导致模型性能受主要类别主导。

Method: 采用后期解耦框架，使用多个解码器配合粗到细的层次引导和一致性；引入基于语义原型的双向监督机制，学习类别判别特征。

Result: 在多个数据集和骨干网络上实现了最先进的3D层次语义分割性能，核心组件可作为即插即用模块提升现有方法。

Conclusion: 所提方法有效解决了3D层次语义分割中的多层级冲突和类别不平衡问题，具有优异的性能和泛化能力。
...
TL;DR: 提出了一种新颖的教师引导剪枝框架，将知识蒸馏与重要性评分估计紧密结合，实现一次性全局剪枝，减少计算开销
...
Method: 在重要性评分计算中利用教师模型的梯度信号，识别对任务性能和知识转移都关键的参数，采用一次性全局剪枝策略，剪枝后使用稀疏感知再训练恢复精度

Result: 在CIFAR-10、CIFAR-100和TinyImageNet等图像分类基准测试中，该方法在高稀疏度下实现了最小性能损失，优于EPG和EPSD等先进基线方法
...
<details>
  <summary>Details</summary>
Motivation: 对Cambrian-S提出的空间超感知能力进行批判性分析，验证其基准测试是否真正衡量了空间认知能力，以及其推理方法是否利用了基准中的捷径。

Method: 1. 提出NoSense基线方法，仅使用词袋SigLIP模型，几乎完全丢弃时间结构；2. 在VSC基准上设计VSC-Repeat验证实验，将视频与自身拼接1-5次，观察模型表现变化。

Result: 1. NoSense在VSR基准上达到95%准确率，表明该基准无需空间认知即可解决；2. VSC-Repeat实验中，Cambrian-S的平均相对准确率从42%降至0%，证明其依赖基准中的捷径（房间从不重复访问）。

Conclusion: 当前VSI-Super基准无法可靠衡量空间超感知能力，Cambrian-S的推理方法通过利用基准捷径而非真正的空间认知来提高性能。
...
TL;DR: PartUV是一个基于部件分解的UV展开方法，通过结合语义部件分解和几何启发式方法，在保持低扭曲的同时生成更少、部件对齐的图表。


<details>
  <summary>Details</summary>
Motivation: 现有UV展开方法在处理AI生成网格时表现不佳，这些网格通常噪声多、凹凸不平且条件差，导致图表高度碎片化和边界不理想，影响下游任务。

Method: 基于PartField部件分解方法，PartUV在自上而下的递归框架中结合高层语义部件分解和新颖的几何启发式方法，确保每个图表的扭曲低于用户指定阈值，同时最小化图表总数。

Result: 在四个不同数据集上的评估显示，PartUV在图表数量和接缝长度方面优于现有工具和最新神经方法，达到可比较的扭曲度，在挑战性网格上具有高成功率。

Conclusion: PartUV提供了一种有效的部件对齐UV展开解决方案，能够处理AI生成网格等挑战性数据，并支持部件特定的多瓦片打包等新应用。
...
TL;DR: TriDiff-4D是一个基于扩散模型的4D生成管道，通过三平面重定位生成高质量、时间一致的4D虚拟形象，支持骨架驱动的任意长度4D生成，显著提升了生成效率和视觉质量。


<details>
  <summary>Details</summary>
Motivation: 解决现有4D生成方法在时间几何一致性、感知伪影、运动不规则性、计算成本和动态控制方面的局限性，满足对高保真可控4D虚拟形象的需求。
...
Conclusion: TriDiff-4D通过从大规模3D和运动数据集中学习3D结构和运动先验，在时间一致性、运动准确性、计算效率和视觉保真度方面表现出色，为4D虚拟形象生成提供了有效的解决方案。
...
TL;DR: SceneDesigner是一个用于精确灵活的多物体9自由度姿态控制的方法，通过分支网络、CNOCS映射表示、两阶段训练策略和分离物体采样技术，解决了现有方法在复杂多物体场景中控制性不足和质量下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在同时控制多个物体的9D姿态（位置、大小和方向）方面存在局限性，控制性不足且生成质量下降，无法实现全面的多物体姿态控制。

Method: 1. 在预训练基础模型上加入分支网络；2. 提出CNOCS映射表示，从相机视角编码9D姿态信息；3. 构建ObjectPose9D数据集；4. 采用两阶段训练策略，第二阶段使用强化学习在重平衡数据上微调；5. 推理时使用分离物体采样技术；6. 集成用户特定个性化权重。

Result: 广泛的定性和定量实验表明，SceneDesigner在控制性和质量方面显著优于现有方法。

Conclusion: SceneDesigner通过创新的网络架构、数据表示和训练策略，成功实现了准确灵活的多物体9自由度姿态控制，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 随着生成视频模型（如Veo-3）显示出令人惊讶的零样本推理能力，需要系统可靠的评估方法来衡量这些模型的推理性能。
...
Conclusion: V-ReasonBench提供了一个统一可重现的框架来测量视频推理能力，旨在支持开发具有更可靠、与人类对齐推理技能的模型。
...
TL;DR: 本文提出了Video-Next-Event Prediction (VNEP)任务，将视频作为下一事件预测的回答模态，并开发了VANS模型，通过强化学习将视觉语言模型与视频扩散模型对齐，生成动态视频回答。


<details>
  <summary>Details</summary>
Motivation: 视频具有展示物理世界信息的能力，而语言模型在视频生成方面主要局限于娱乐应用。作者发现将视频作为下一事件预测的回答模态具有未充分利用的潜力，能够提供更直观和定制化的程序性学习和创造性探索答案。

Method: 提出了VANS模型，使用强化学习（Joint-GRPO）将视觉语言模型(VLM)与视频扩散模型(VDM)对齐。VLM生成准确且易于可视化的字幕，VDM基于这些字幕和输入视觉上下文生成视频。还构建了VANS-Data-100K专用数据集。

Result: 在程序和预测基准测试中，VANS在视频事件预测和可视化方面都达到了最先进的性能。

Conclusion: VNEP任务将视频作为回答模态具有重要价值，VANS模型通过强化学习有效解决了该任务的挑战，为程序性学习和创造性探索提供了更直观的视频生成解决方案。
...
TL;DR: 提出了一种让视觉语言模型自动在快速思考和慢速思考模式间切换的强化学习方法，通过输出长度判断任务难度，在保持高性能的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型主要追求冗长的推理链，导致计算成本过高，而人类会根据问题复杂度自动切换思考模式来高效分配认知资源。

Method: 两阶段方法：第一阶段根据模型输出长度标注数据为快速或慢速思考模式；第二阶段使用GRPO强化学习训练模型发展双模式思考能力。

Result: DualMindVLM模型显著超越基础模型，性能与最先进视觉推理模型相当，同时保持极高的token效率。

Conclusion: 简单的强化学习方法能够有效训练视觉语言模型自动切换思考模式，在保持高性能的同时大幅提升计算效率。
...
TL;DR: EvoLMM是一个自进化框架，通过两个协作代理（提议者和解决者）在无监督条件下提升大型多模态模型的推理能力，仅使用原始训练图像就能在数学推理基准上获得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有大型多模态模型的训练仍依赖人工标注数据或外部奖励模型，限制了自主性和可扩展性。本文旨在完全无监督地提升模型推理能力。

Method: 提出EvoLMM框架，从单一骨干模型实例化两个协作代理：提议者生成多样化的图像相关问题，解决者通过内部一致性解决这些问题，形成持续自我奖励的学习过程。

Result: 以Qwen2.5-VL为基础模型，EvoLMM在ChartQA、MathVista和MathVision等多模态数学推理基准上实现了约3%的稳定提升，仅使用原始训练图像。

Conclusion: EvoLMM提供了一个简单有效的基线方法，为完全无监督的自改进大型多模态模型研究铺平了道路。
...
Method: 提出NoPo-Avatar方法，仅从图像重建化身，完全不需要姿态输入，避免了噪声姿态估计对重建结果的影响。

Result: 在THuman2.0、XHuman和HuGe100K数据集上的实验表明，在无真实姿态的实际设置中优于现有基线，在有真实姿态的实验室设置中也能达到可比结果。

Conclusion: NoPo-Avatar通过消除对测试时人体姿态的依赖，提高了重建方法的实用性和鲁棒性，在噪声姿态环境下表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法主要关注从头开始训练模型，而当前最先进的视觉方法越来越多地基于大型预训练自监督模型。本文研究如何为这类预训练模型蒸馏出最优的数据集来训练线性分类器。

Method: 提出Linear Gradient Matching方法，优化合成图像使其通过预训练特征提取器后，在线性分类器中产生的梯度与真实数据产生的梯度相似。

Result: 该方法生成的合成数据在所有真实图像基线方法上表现更优，且具有跨预训练模型的泛化能力，例如使用DINO骨干网络蒸馏的数据集可以训练出具有竞争力的CLIP线性分类器。
...
TL;DR: 一个端到端的垂直集成研究原型系统，集成了288个NorthPole神经推理加速卡，提供115 peta-ops的4位整数精度计算能力和3.7 PB/s内存带宽，在30kW功耗下可同时运行3个80亿参数模型实例。


<details>
  <summary>Details</summary>
Motivation: 开发一个可扩展、高效的云端推理服务系统，用于企业AI应用中的智能工作流部署，支持现有数据中心环境。

Method: 采用垂直集成的端到端方法，结合288个NorthPole神经推理加速卡、离线训练算法、高性能运行时栈和容器化推理流水线。

Result: 系统在18台2U服务器中实现115 peta-ops计算能力，功耗仅30kW，可同时运行3个80亿参数模型实例，支持28个用户，每个用户的令牌间延迟为2.8ms。

Conclusion: 该系统具有可扩展性、模块化和可重构性，支持多种模型规模和上下文长度，适合在企业数据中心环境中部署智能工作流。
...
### [137] [Optimizing Communication in Byzantine Agreement Protocols with Slim-HBBFT](https://arxiv.org/abs/2511.15957)
*Nasit S Sony,Xianzhong Ding*

Main category: cs.DC

TL;DR: Slim-HBBFT是一个原子广播协议，通过仅考虑部分节点的请求来优化通信复杂度，相比传统异步拜占庭协议降低了O(n)倍的通信开销。


<details>
  <summary>Details</summary>
Motivation: 传统异步拜占庭协议需要广播所有节点的请求，通信开销大，当请求重复时这种开销不合理。需要设计更高效的协议。

Method: 设计了优先可证明广播(P-PB)协议，只为选定的节点生成广播证明，并基于此构建Slim-HBBFT原子广播协议。

Result: Slim-HBBFT协议在通信复杂度上比传统协议优化了O(n)倍，同时满足异步公共子集协议的安全性要求。

Conclusion: Slim-HBBFT通过选择性处理节点请求，在保证安全性的同时显著降低了通信开销，适用于请求重复的场景。
...
### [138] [Efficient Chromosome Parallelization for Precision Medicine Genomic Workflows](https://arxiv.org/abs/2511.15977)
...
<details>
  <summary>Details</summary>
Motivation: 大规模基因组工作流在处理每个样本数十到数百GB数据时，会出现高内存峰值、密集磁盘I/O和内存不足错误。静态资源分配方法难以处理不同染色体RAM需求的变异性，导致资源利用率低和运行时间长。

Method: 1. 开发符号回归模型估计每个染色体的内存消耗，引入插值偏差保守最小化过度分配；2. 提出动态调度器使用多项式回归模型自适应预测RAM使用，将任务打包视为背包问题；3. 提出静态调度器优化染色体处理顺序以最小化峰值内存同时保持吞吐量。

Result: 在模拟和真实基因组管道上的评估表明，所提方法提供了减少内存溢出和平衡线程间负载的新机制，实现了更快的端到端执行。
...
### [139] [Can Asymmetric Tile Buffering Be Beneficial?](https://arxiv.org/abs/2511.16041)
...
TL;DR: 提出了非对称瓦片缓冲(ATB)技术，通过解耦输入和输出操作数的缓冲瓦片维度，在AMD XDNA2 AI Engine上实现了最高4.54倍的GEMM性能提升


<details>
  <summary>Details</summary>
Motivation: 传统对称瓦片缓冲方法限制了GEMM计算效率，需要更灵活的瓦片缓冲策略来提升算术强度

Method: 开发非对称瓦片缓冲(ATB)技术，解耦输入和输出操作数的缓冲瓦片维度，并建立包含ATB收益和开销的性能模型

Result: 在AMD XDNA2 AI Engine上实现最高4.54倍加速，从4.8 TFLOPS提升到24.6 TFLOPS，创下XDNA2 AIE性能新纪录

Conclusion: ATB是实用且高效的GEMM优化技术，通过解耦瓦片维度显著提升算术强度，适用于现代AI工作负载
...
### [140] [Mitigating Shared Storage Congestion Using Control Theory](https://arxiv.org/abs/2511.16177)
*Thomas Collignon,Kouds Halitim,Raphaël Bleuse,Sophie Cerf,Bogdan Robu,Éric Rutten,Lionel Seinturier,Alexandre van Kempen*

Main category: cs.DC

TL;DR: 提出基于控制理论的自适应方法，通过动态调节客户端I/O速率来缓解HPC系统中的I/O拥塞问题，提高性能稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统I/O栈优化方法通常针对特定工作负载且需要专业知识，难以通用化；在共享HPC环境中，资源拥塞会导致性能不可预测、运行缓慢和超时问题。

Method: 采用控制理论方法，利用少量运行时系统负载指标动态调节客户端I/O速率，在多节点集群中实现控制器。

Result: 实验结果表明，该方法有效缓解I/O拥塞，总运行时间最多减少20%，降低了尾延迟，同时保持稳定的性能表现。
...
TL;DR: SpecActor通过动态解耦推测和动态Best-of-N推测方法，在大型语言模型后训练中实现了1.3-1.7倍的加速，解决了推测执行在大批量配置下的效率问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型后训练中的rollout阶段占据了大部分训练时间，传统的推测解码方法在大批量执行时效率低下，需要解决GPU计算效率最大化和推测准确性的问题。
...
Conclusion: SpecActor通过创新的推测执行方法，有效加速了LLM后训练中的rollout阶段，解决了大批量配置下的效率瓶颈问题。
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习在处理数十亿参数的大语言模型时面临严重的通信开销和内存限制，需要高效的传输和处理方案。

Method: 采用消息量化和容器/文件流两种关键技术：量化减少消息大小，流技术实现高效内存管理。

Result: 这些技术显著提升了联邦学习与大语言模型的鲁棒性和效率，在实际应用中表现更好。

Conclusion: NVIDIA FLARE的通信能力增强为联邦学习在大语言模型场景下的实际部署提供了可行解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统分布式算法假设节点遵循协议，但理性节点可能为了增加期望效用而偏离协议。标准MIS算法依赖诚实随机性或唯一标识符，但在理性设置中，代理可能操纵随机性，仅依赖标识符可能导致不公平。

Method: 提出了两种算法，基于效用模型，代理寻求局部正确解并对所选解有偏好。算法中的随机性通过邻居节点间的成对交互生成，视为简单游戏，单个节点无法单方面影响结果。

Result: 证明在算法执行的每个阶段，给定任何历史，假设其他节点遵循算法，没有代理能通过单边偏离增加期望效用。当所有节点遵循协议时，每个节点都有正概率加入MIS，最终输出正确的MIS。在温和附加假设下，两种算法高概率在O(log n)轮内终止。

Conclusion: 所提算法在理性分布式设置中提供了强于颤抖手完美均衡的保证，确保了公平性和正确性，同时保持了高效的时间复杂度。
...
<details>
  <summary>Details</summary>
Motivation: 存内计算在机器学习加速中很重要，但现有系统难以有效利用稀疏性，缺乏统一的稀疏DNN工作负载建模方法。

Method: 开发CIMinus框架，提供组件级能耗分析和整体工作负载延迟评估，并在当代CIM架构上验证。

Result: 验证了CIMinus在当代CIM架构上的适用性，通过两个用例展示了稀疏模式和映射策略的影响。

Conclusion: CIMinus填补了理论设计与实际实现之间的空白，为稀疏DNN在CIM系统中的优化提供了系统化建模方法。
...
TL;DR: 提出了一种混合工作流，将多模式分解建模为约束图着色问题，主要目标是最小化特征违规，次要目标是平衡各掩模上的特征数量。


<details>
  <summary>Details</summary>
Motivation: 现有的启发式回溯和SAT求解器难以同时处理复杂约束和次要目标，需要一种能同时优化主要和次要目标的多模式分解方法。

Method: 集成两个主要组件：(1) 无监督训练的GNN代理生成初始颜色预测；(2) 通过GNN启发式算法和模拟退火进行精炼，共同提升解质量和平衡性。
...
TL;DR: QueryGym是一个轻量级、可扩展的Python工具包，用于支持基于大语言模型的查询重写，提供统一的框架来实现、执行和比较不同的LLM重写方法。


<details>
  <summary>Details</summary>
Motivation: 当前LLM查询重写方法虽然能显著提升检索效果，但缺乏统一的工具包，导致公平比较、快速实验、一致基准测试和可靠部署存在困难。

Method: 提供Python API支持多种LLM方法，检索无关的接口支持Pyserini和PyTerrier等后端，集中式提示管理系统，内置BEIR和MS MARCO基准支持。

Result: 开发了一个完全开源的统一工具包，解决了LLM查询重写方法实现不一致的问题。

Conclusion: QueryGym填补了LLM查询重写工具包的空白，为研究人员提供了公平比较和快速实验的统一平台。
...
TL;DR: 论文主张LLM驱动的音乐推荐系统需要重新思考评估方法，分析了LLM如何重塑音乐推荐，并提出了结构化的成功和风险维度评估框架。


<details>
  <summary>Details</summary>
Motivation: 传统音乐推荐系统基于信息检索框架，主要关注准确性指标，难以回答"什么是好的推荐"这一根本问题。LLM的出现颠覆了这一框架，需要重新思考评估方法。

Method: 首先回顾LLM如何重塑用户建模、物品建模和自然语言推荐；然后借鉴NLP领域的评估实践；最后综合见解，提出适用于MRS的LLM提示评估框架。

Result: 提出了一个结构化的评估框架，包含成功维度和风险维度，为MRS社区提供了更新的、教学性的跨学科评估视角。

Conclusion: LLM驱动的音乐推荐系统需要新的评估范式，传统基于准确性的指标不再适用，需要综合考虑生成质量、幻觉问题、知识截止等新挑战。
...
TL;DR: 提出Prism框架，将可解释推荐系统解耦为独立的排序阶段和解释生成阶段，通过知识蒸馏使用大模型作为教师生成高质量解释知识，再由小型学生模型合成个性化解释，解决了端到端架构中的性能-效率权衡问题。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在可解释推荐系统中端到端架构导致的性能-效率权衡问题，避免排序和解释联合优化带来的次优妥协。

Method: 采用解耦框架，使用教师LLM（如FLAN-T5-XXL）作为Oracle生成高质量解释知识，然后由经过微调的学生模型（如BART-Base）专门合成个性化解释。

Result: 140M参数的Prism模型在忠实性和个性化的人类评估中显著优于11B参数的教师模型，推理速度提升24倍，内存消耗减少10倍。

Conclusion: 解耦结合定向蒸馏为高质量可解释推荐提供了高效有效的途径，证明了每个组件针对特定目标优化的优势。
...
### [149] [The Subtle Art of Defection: Understanding Uncooperative Behaviors in LLM based Multi-Agent Systems](https://arxiv.org/abs/2511.15862)
*Devang Kulshreshtha,Wanyu Du,Raghav Jain,Srikanth Doss,Hang Su,Sandesh Swamy,Yanjun Qi*

Main category: cs.MA

TL;DR: 提出了一个模拟和分析不合作行为如何破坏LLM多智能体系统稳定性的框架，包括基于博弈论的行为分类和动态模拟流程，实验显示不合作行为可在1-7轮内导致系统崩溃。


<details>
  <summary>Details</summary>
Motivation: 现有文献在分析LLM多智能体系统中不合作行为的影响方面存在明显空白，需要系统性地研究这类行为如何破坏系统稳定性。

Method: 开发了包含博弈论行为分类和多阶段模拟流程的框架，在协作资源管理场景中评估系统稳定性，使用生存时间和资源过度使用率等指标。

Result: 框架生成不合作行为的准确率达96.7%，合作智能体保持100%系统稳定性，而不合作行为可在1-7轮内导致系统崩溃。

Conclusion: 不合作智能体显著恶化集体成果，突显了设计更具韧性的多智能体系统的必要性。
...
### [150] [MiMo-Embodied: X-Embodied Foundation Model Technical Report](https://arxiv.org/abs/2511.16518)
...
Result: 在17个具身AI基准测试（任务规划、功能预测、空间理解）和12个自动驾驶基准测试（环境感知、状态预测、驾驶规划）中均创下新记录，显著优于现有开源、闭源和专业基线模型。
...
### [151] [How Robot Dogs See the Unseeable](https://arxiv.org/abs/2511.16262)
...
TL;DR: 该研究提出了一种基于动物摆动行为的合成孔径感知方法，通过机器人执行摆动运动来创建宽合成孔径，从而在遮挡环境中实现清晰的背景感知。


<details>
  <summary>Details</summary>
Motivation: 解决机器人视觉中的部分遮挡问题，传统机器人相机的小孔径和大景深使得前景障碍物和背景物体都清晰，导致遮挡物掩盖关键场景信息。

Method: 让机器人执行摆动运动，其相机描述一个宽合成孔径，通过计算整合捕获的图像合成具有极浅景深的图像，有效模糊遮挡元素同时使背景清晰聚焦。

Result: 该方法能够恢复基本场景理解，并增强大型多模态模型的高级视觉推理能力，对遮挡具有鲁棒性，计算效率高，可立即部署在任何移动机器人上。

Conclusion: 摆动运动用于合成孔径感知是复杂杂乱环境中实现高级场景理解的关键，连接了动物行为和机器人技术。
...
TL;DR: 提出了一种针对大规模发电机组的机组组合问题的新方法，通过放宽二元决策为连续变量，大幅降低求解时间，从10小时减少到不到1秒。


<details>
  <summary>Details</summary>
Motivation: 数据中心运营商越来越多地使用自有发电机组来补充电网供电，这导致大电力系统中发电机数量急剧增加，传统混合整数机组组合方法无法有效处理数千台灵活、快速循环的发电机组。

Method: 提出一种机组组合公式，通过允许发电机部分运行来放宽二元决策，使用连续求解器算法，然后采用舍入方法获得可行的机组组合方案。

Result: 在276台发电机系统中，求解时间从10小时减少到不到1秒，精度损失很小。该方法可扩展到数万台发电机，适用于北美主要互联电网规模，且大部分计算可并行化和GPU兼容。

Conclusion: 该方法为大规模发电机组组合问题提供了高效的解决方案，显著提高了计算效率，具有良好的可扩展性和未来加速潜力。
...
### [153] [Chain of Summaries: Summarization Through Iterative Questioning](https://arxiv.org/abs/2511.15719)
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM难以处理网页内容的问题，因为网页格式不友好且受限于上下文长度限制。
...
Method: 提出模态基础推理蒸馏框架，让模型学习生成基于声学特征的音频相关推理链，避免产生脱节的推理过程。
...
<details>
  <summary>Details</summary>
Motivation: 小型语言模型在判断答案正确性方面的能力相比大型语言模型尚不明确。现有的LLM-as-a-judge框架依赖与真实标签或其他答案的间接比较，难以完全自动化，对推理输出的细粒度可扩展评估支持有限。

Method: 提出JudgeBoard评估管道，在数学推理和科学/常识推理两个核心领域构建任务特定的评估排行榜。提出MAJ多智能体判断框架，利用具有不同推理特征的多个交互SLM通过协作审议来近似LLM级别的判断准确性。

Result: 实验结果显示SLM和LLM在独立判断任务中存在显著性能差距。但MAJ框架显著提高了SLM的可靠性和一致性。在MATH数据集上，使用较小模型作为骨干的MAJ表现与较大模型相当甚至更好。

Conclusion: 多智能体SLM系统在判断任务中有潜力匹配或超越LLM性能，对可扩展和高效评估具有重要意义。
...
TL;DR: LLMs在临床环境中即使获得正确证据也难以正确推理，作者使用WET指南作为测试平台，发现即使提供权威段落错误仍然存在，提出了评估推理准确性、一致性和忠实度的框架。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在检索与推理之间的差距，特别是在临床环境中需要严格遵循结构化协议的情况，确保输出与指南一致。

Method: 使用Written Exposure Therapy (WET)指南作为测试平台，评估模型对临床问题的响应，提出评估框架衡量准确性、一致性和推理忠实度。

Result: 发现即使提供权威证据，错误仍然存在，检索增强生成(RAG)可以约束输出，但安全部署需要同等严格地评估推理过程。

Conclusion: 在临床环境中部署LLMs需要同时严格评估检索和推理能力，仅凭检索增强不足以确保安全可靠的输出。
...
<details>
  <summary>Details</summary>
Motivation: 探索人工智能作为核心游戏玩法共创工具的应用，让玩家通过自然语言实现个性化创作体验。

Method: 使用监督训练的BERT模型解释玩家提示，映射到法术模板并平衡参数；Unity游戏引擎开发，Python AI后端。

Result: 开发出功能原型，能够实时生成法术，应用于以玩家创造力为核心的吸引人游戏循环。

Conclusion: 验证了AI作为直接游戏机制的有效性，为玩家提供了独特的个性化创作体验。
...
### [158] [OpenMMReasoner: Pushing the Frontiers for Multimodal Reasoning with an Open and General Recipe](https://arxiv.org/abs/2511.16334)
...
Method: 采用两阶段训练方法：1) SFT阶段使用87.4万样本的冷启动数据集进行逐步验证；2) RL阶段使用7.4万样本跨领域数据集进一步优化推理能力。

Result: 在多个多模态推理基准测试中显著超越强基线模型，证明了数据质量和训练设计对多模态推理性能的关键作用。
...
TL;DR: TOFA是一个无需训练的单次联邦视觉语言模型适配框架，通过视觉和文本双管道充分利用预训练模型的通用多模态特征，解决了现有方法在数据异构性、多模态信息利用和额外训练资源需求方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有联邦视觉语言模型适配方法需要迭代训练，导致通信成本高且易受攻击。单次联邦训练技术虽然能减少通信轮次，但在多模态信息利用、数据异构性处理和额外训练资源需求方面存在不足。
...
Conclusion: TOFA是一个高效、轻量级的单次联邦视觉语言模型适配框架，无需额外训练资源，能有效处理数据异构性并充分利用多模态信息。
...
### [160] [D-GARA: A Dynamic Benchmarking Framework for GUI Agent Robustness in Real-World Anomalies](https://arxiv.org/abs/2511.16590)
*Sen Chen,Tong Zhao,Yi Bin,Fei Ma,Wenqi Shao,Zheng Wang*

Main category: cs.AI

TL;DR: 提出了D-GARA动态基准测试框架，用于评估Android GUI代理在真实世界异常环境中的鲁棒性，填补了现有静态基准测试无法反映现实复杂性的研究空白。


<details>
  <summary>Details</summary>
Motivation: 现有GUI代理训练和评估数据集大多是静态和理想化的，无法反映真实环境中异常情况的复杂性和不可预测性，特别是各种中断事件。

Method: 构建D-GARA框架，引入多种真实世界异常类型（如权限对话框、电池警告、更新提示等），并基于该框架构建包含常用Android应用和嵌入异常的基准测试集。

Result: 实验表明，在异常丰富的环境中，最先进的GUI代理性能显著下降，凸显了对鲁棒性学习的需求。
...
### [161] [How Modality Shapes Perception and Reasoning: A Study of Error Propagation in ARC-AGI](https://arxiv.org/abs/2511.15717)
*Bo Wen,Chen Wang,Erhan Bilal*

Main category: cs.AI

TL;DR: 该论文研究了不同模态（文本和图像）如何影响模型在ARC-AGI任务中对网格特征的感知能力，发现结构化文本能精确定位稀疏特征，图像能捕捉2D形状但对分辨率敏感，结合两者可提升执行效果。


<details>
  <summary>Details</summary>
Motivation: 现有指令优先系统在ARC-AGI任务中缺乏对编码方式如何影响模型感知的原则性分析，以及如何区分指令错误和执行错误。作者假设不同模态会带来感知瓶颈，从而影响网格特征的可靠感知。

Method: 使用加权集合分歧度量和两阶段推理流程，在九种文本和图像模态中隔离感知与推理，分析不同表示方式对特征感知的影响。

Result: 结构化文本在稀疏特征上能提供精确坐标，图像能捕捉2D形状但对分辨率敏感，结合文本和图像能提升执行效果（约8个感知点；约0.20中位数相似度）。
...
### [162] [FOOTPASS: A Multi-Modal Multi-Agent Tactical Context Dataset for Play-by-Play Action Spotting in Soccer Broadcast Videos](https://arxiv.org/abs/2511.16183)
...
<details>
  <summary>Details</summary>
Motivation: 当前动作识别方法不足以构建可靠的逐场比赛数据，通常只能辅助而非完全自动化标注。战术建模、轨迹预测和表现分析等研究都需要基于比赛状态和逐场比赛数据，因此需要利用战术知识作为先验来支持基于计算机视觉的预测。

Method: 引入FOOTPASS数据集，支持开发利用计算机视觉任务输出（如跟踪、识别）和足球战术知识（包括长期战术规律）的球员中心动作识别方法。

Result: 创建了首个用于足球比赛逐场比赛动作识别的基准数据集，能够生成可靠的逐场比赛数据流。

Conclusion: FOOTPASS数据集为数据驱动的体育分析提供了重要输入，通过结合计算机视觉和战术知识，实现了更自动化和可靠的逐场比赛数据提取。
...
### [163] [Pipelined Dense Symmetric Eigenvalue Decomposition on Multi-GPU Architectures](https://arxiv.org/abs/2511.16174)
*Hansheng Wang,Ruiyi Zhan,Dajun Huang,Xingchen Liu,Qiao Li,Hancong Duan,Dingwen Tao,Guangming Tan,Shaoshuai Zhang*

Main category: cs.MS

TL;DR: 提出了一种流水线化的两阶段特征值分解算法，在8×A100平台上相比现有库实现了5.74-6.59倍的加速，具有更好的强扩展性和弱扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有多GPU库（cuSOLVERMp、MAGMA、ELPA）在大规模对称特征值问题中性能不佳，仅利用了约1.5%的峰值多GPU性能。

Method: 采用流水线化的两阶段特征值分解算法，替代传统的顺序算法，并进行大量优化。

Result: 在8×A100平台上，相比cuSOLVERMp和MAGMA基准，平均分别实现了5.74倍和6.59倍的加速，具有更好的强扩展性和弱扩展性。

Conclusion: 提出的流水线化两阶段算法显著提升了多GPU平台上大规模对称特征值分解的性能。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型的强化学习训练存在效率瓶颈，由于响应生成呈现长尾分布，少数极长响应会主导执行时间，浪费资源并增加成本。

Method: TLT系统包含两个组件：(1) 自适应草稿器，在长尾生成期间于空闲GPU上持续训练轻量级草稿模型；(2) 自适应执行引擎，维护内存高效的CUDAGraph池并自适应选择推测解码策略。

Result: 评估显示TLT实现了超过1.7倍的端到端RL训练加速，保持了模型精度，并免费获得高质量草稿模型用于高效部署。

Conclusion: TLT系统成功解决了推理型LLM强化学习训练的效率瓶颈，通过自适应推测解码实现了显著加速且无损模型性能。
...
### [165] [AccelOpt: A Self-Improving LLM Agentic System for AI Accelerator Kernel Optimization](https://arxiv.org/abs/2511.15915)
*Genghan Zhang,Shaowei Zhu,Anjiang Wei,Zhenyu Song,Allen Nie,Zhen Jia,Nandita Vijaykumar,Yida Wang,Kunle Olukotun*

Main category: cs.LG

TL;DR: AccelOpt是一个自改进的LLM代理系统，能自主优化新兴AI加速器的内核，无需专家提供的硬件特定优化知识。通过迭代生成和优化记忆库，提升内核性能，在Trainium加速器上显著提高吞吐量峰值百分比。


<details>
  <summary>Details</summary>
Motivation: 消除对专家硬件优化知识的依赖，为新兴AI加速器提供自主、成本效益高的内核优化解决方案。

Method: 使用迭代生成方法探索内核优化空间，结合优化记忆库记录先前遇到的慢-快内核对经验和洞察。

Result: 在Trainium 1上平均峰值吞吐量从49%提升到61%，在Trainium 2上从45%提升到59%。使用开源模型时，性能与Claude Sonnet 4相当但成本降低26倍。

Conclusion: AccelOpt证明能够随时间改进性能，为新兴AI加速器提供高效、经济的内核优化方案，无需专家知识。
...
### [166] [UniUltra: Interactive Parameter-Efficient SAM2 for Universal Ultrasound Segmentation](https://arxiv.org/abs/2511.15771)
...
<details>
  <summary>Details</summary>
Motivation: SAM2在自然图像上表现出色，但在超声图像上性能显著下降，需要解决参数高效微调和资源受限临床环境部署的挑战。

Method: 提出上下文-边缘混合适配器(CH-Adapter)增强细粒度感知，采用深度监督知识蒸馏(DSKD)将知识从大型图像编码器转移到超轻量编码器。

Result: UniUltra在微调阶段仅使用SAM2参数的8.91%，最终压缩模型参数减少94.08%，性能优于现有最先进方法。

Conclusion: 该框架在保持优异分割性能的同时实现了显著的参数压缩，为临床超声分割应用提供了高效实用的解决方案。
...
### [167] [Weakly Supervised Segmentation and Classification of Alpha-Synuclein Aggregates in Brightfield Midbrain Images](https://arxiv.org/abs/2511.16268)
*Erwan Dereure,Robin Louiset,Laura Parkkinen,David A Menassa,David Holcman*

Main category: eess.IV

TL;DR: 开发了一个基于弱监督分割的自动化图像处理流程，用于在帕金森病和中途路易体病的中脑组织全玻片图像中分割和分类α-突触核蛋白聚集体，能够区分路易体和神经突等主要形态，平衡准确率达到80%。


<details>
  <summary>Details</summary>
Motivation: 帕金森病与错误折叠的α-突触核蛋白聚集体（形成路易体和神经突）积累相关，这些用于病理诊断。深度学习自动分析免疫组织化学组织病理学图像为更好理解这些聚集体的空间组织提供了有前景的工具。

Method: 基于弱监督分割开发自动化图像处理流程，使用ResNet50分类器，对免疫组织化学标记变异性具有鲁棒性，能够分割和分类全玻片图像中的α-突触核蛋白聚集体。

Result: 该方法能够区分主要聚集体形态，包括路易体和神经突，平衡准确率达到80%。

Conclusion: 该框架为大规模表征明场免疫组织化学组织中α-突触核蛋白聚集体的空间分布和异质性铺平了道路，并为研究其与周围细胞（如小胶质细胞和星形胶质细胞）之间知之甚少的关系提供了工具。
...
### [168] [Green Resilience of Cyber-Physical Systems: Doctoral Dissertation](https://arxiv.org/abs/2511.16593)
*Diaeddin Rimawi*

Main category: cs.SE

TL;DR: 本文研究了在线协作AI系统(OL-CAIS)在面临干扰事件时如何平衡恢复力与绿色性能的权衡问题，提出了GResilience框架和量化指标，通过多目标优化、博弈论和强化学习等方法实现绿色恢复。


<details>
  <summary>Details</summary>
Motivation: OL-CAIS作为一类网络物理系统，在与人协作学习过程中易受干扰事件影响性能。决策者需要在恢复性能的同时限制能耗影响，这就产生了恢复力与绿色性能之间的权衡问题。

Method: 将OL-CAIS行为建模为三种操作状态：稳定、干扰和最终状态；提出GResilience框架，通过多目标优化（单智能体）、博弈论决策（双智能体）和强化学习（RL智能体）提供恢复策略；设计量化恢复力和绿色性能的测量框架。

Result: 实验表明恢复力模型能捕捉干扰期间的性能转换，GResilience策略通过缩短恢复时间、稳定性能和减少人类依赖来改善绿色恢复。RL智能体策略效果最佳，但CO2排放略有增加。容器化执行可将CO2排放减半。

Conclusion: 本研究提供了确保OL-CAIS绿色恢复的模型、指标和策略，解决了恢复力与绿色性能的平衡问题，同时观察到重复干扰后的灾难性遗忘现象，所提策略有助于维持稳定性。
...
TL;DR: Codec2Vec是首个基于离散音频编解码单元的语音表示学习框架，在SUPERB基准测试中表现优异，同时大幅降低了存储需求和训练时间。
...
Method: 采用基于离散音频编解码单元的表示学习框架，探索多种训练目标推导策略的掩码预测方法。

Result: 在SUPERB基准测试中达到与连续输入模型相当的竞争性性能，同时存储需求降低16.5倍，训练时间减少2.3倍。
...
TL;DR: 提出基于区块链的框架，通过密码学验证AI代理发起的金融交易的真实性和意图，使用DID、零知识证明和TEE技术确保安全性和隐私保护。


<details>
  <summary>Details</summary>
Motivation: AI代理越来越多地代表用户发起金融交易，但在去中心化环境中缺乏验证代理身份和交易意图的有效机制，传统支付系统的人类授权假设不再适用。

Method: 采用混合链上/链下架构，结合去中心化身份标准、可验证凭证、链上意图证明、零知识证明和可信执行环境认证，建立从用户意图到支付结果的不可变审计追踪。

Result: 定性分析表明该框架对冒充攻击、未授权交易和意图偏差具有强抵抗力，能够确保AI驱动金融生态系统的可验证信任和问责制。

Conclusion: 该工作为安全、可审计且意图感知的自主经济代理奠定了基础，实现了AI驱动金融生态系统中可验证的信任和问责。
...
TL;DR: 提出Digital Agriculture Sandbox平台，通过联邦学习、差分隐私等技术保护农场数据隐私，让农民和研究人员能在不暴露敏感信息的情况下协作分析数据。


<details>
  <summary>Details</summary>
Motivation: 数字农业产生大量有价值数据，但农民因隐私担忧不愿分享数据，限制了研究人员利用这些数据改进农业实践的能力。

Method: 开发安全的在线平台，采用联邦学习、差分隐私和数据分析方法，在保护数据隐私的同时保持数据的研究价值。

Result: 系统使农民能够以简化方式识别相似农民，无需技术知识或计算资源；研究人员能在数据不离开农民系统的情况下学习并构建有用工具。

Conclusion: 该平台在保护农场数据隐私与利用数据解决全球粮食和农业挑战之间架起了桥梁，为农民和研究人员创造了安全协作空间。
...
<details>
  <summary>Details</summary>
Motivation: 系统提示包含专有逻辑或敏感信息，容易受到提取攻击，现有防御机制要么依赖启发式方法，要么计算开销大，要么不适用于黑盒API访问的模型。

Method: 采用屏蔽附加的轻量级方法，在原始提示上添加保护性文本层，将提示强化形式化为效用约束优化问题，利用LLM作为优化器搜索可能的屏蔽层。

Result: 优化的屏蔽层显著减少了针对全面提取攻击的提示泄露，优于现有基线防御方法，且不损害模型的预期功能。
