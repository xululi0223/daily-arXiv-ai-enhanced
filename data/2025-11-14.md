...
TL;DR: Omnilingual ASR是一个可扩展的大规模自动语音识别系统，能够仅用少量数据样本支持未服务语言，覆盖1600多种语言，其中500多种是首次获得ASR支持。
...
Method: 采用70亿参数的自监督预训练学习鲁棒语音表示，引入基于LLM解码器的编码器-解码器架构实现零样本泛化，整合公共资源和社区合作收集的语音数据。

Result: 自动评估显示在低资源条件下显著优于现有系统，具有强大的泛化能力，提供从3亿到70亿参数的不同规模模型。

Conclusion: 开源模型和工具可降低研究者和社区的参与门槛，促进新的参与形式，具有积极的社会影响。
...
TL;DR: 该论文研究针对拉丁语（一种形态丰富的语言）的形态感知分词方法，发现基于形态学的分词能提升下游任务性能，特别是在领域外文本上表现更好。


<details>
  <summary>Details</summary>
Motivation: 标准分词方法通常优先考虑信息理论目标（如高压缩和低生育率），而不是语言学目标（如形态对齐），这对形态丰富的语言来说是次优的，会直接影响下游性能。

Method: 研究拉丁语的形态感知分词，利用拉丁语作为中等预训练数据资源但具有丰富词典资源的特点，开发形态引导的分词方法。

Result: 形态引导的分词在四个下游任务上提高了整体性能，特别是在领域外文本上表现最显著，显示了模型改进的泛化能力。

Conclusion: 研究表明语言资源对于改进形态复杂语言的语言建模具有实用性，对于缺乏大规模预训练数据的低资源语言，开发和整合语言资源可以作为提高语言模型性能的可行替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 探索NLP在处理大规模书面语料库中的潜力，特别是在社会科学研究中识别总统指令中的信号主题，以评估NLP工具在复杂文本分析任务中的实用性。

Method: 使用自然语言处理技术从里根到克林顿政府时期的总统指令语料库中提取主要主题，并与分析师的人工标注结果进行比较分析。

Result: NLP和人工分析师都能识别相关文档，表明NLP在大规模书面语料库研究中具有潜在效用，但两者结果存在差异，需要进一步验证NLP的有效性。

Conclusion: 虽然NLP在提取主题方面显示出潜力，但与人工分析结果的差异表明需要更多研究来评估NLP在此类应用中的有效性，特别是在快速发展的AI/ML背景下，现有工具可能已经过时。
...
TL;DR: 研究发现约10亿参数的小型语言模型在机器翻译关键错误检测任务中能达到最佳质量效率平衡，Gemma-3-1B在保持低计算成本的同时实现高检测性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在机器翻译评估方面表现出色，但其规模和成本限制了在边缘设备和隐私敏感工作流中的部署，因此需要探索小型模型的可行性。

Method: 使用多个小于20亿参数的模型，在WMT21、WMT22和SynCED-EnDe-2025数据集上进行基准测试，采用标准化提示、轻量级logit-bias校准和多数投票策略。

Result: Gemma-3-1B提供最佳质量效率权衡，在SynCED-EnDe-2025上达到MCC=0.77和F1-ERR=0.98，在MacBook Pro M4 Pro上保持400ms单样本延迟。

Conclusion: 经过轻量级校准和小样本监督的紧凑指令调优LLM能够为机器翻译提供可信赖的端侧关键错误检测，实现现实翻译流程中的私密低成本错误筛查。
...
Method: 通过中英文平行句子的谓词-论元结构分析，采用标注投影实验，分别以中文和英文作为源语言进行双向标注投影，并进行定性和定量分析。

Result: 语言迁移具有明显的不对称性，不同语言作为源语言时标注投影效果存在显著差异。

Conclusion: 在跨语言NLP应用中，源语言的选择需要谨慎考虑，任何关于跨语言NLP的科学主张都需要在深入调查语言迁移不对称性之后提出。
...
<details>
  <summary>Details</summary>
Motivation: 检索增强生成(RAG)虽然提高了事实准确性，但每次查询都检索会降低质量、增加令牌数量和延迟。需要一种智能的检索触发机制来平衡准确性和效率。

Method: 从基础模型的短无上下文草稿的前缀对数中计算轻量级不确定性分数：平均令牌熵、基于top-1/top-2对数间隙的边际信号，或少量随机前缀的小N方差，仅在分数超过阈值时触发检索。

Result: 在NQ-Open、TriviaQA和PopQA数据集上，TARG与Always-RAG相比，在保持或提高EM/F1的同时，减少了70-90%的检索次数，降低了端到端延迟，且开销接近Never-RAG。

Conclusion: 边际信号是现代指令调优LLM下的稳健默认选择，小N方差提供了保守的预算优先替代方案。TARG通过明确的预算权衡，有效提升了检索增强生成的效率边界。
...
TL;DR: 本文提出了一种解决高棉语拼写检查问题的整体方法，通过整合子词分割、命名实体识别、字形到音素转换和语言模型，实现了94.4%的最先进准确率。


<details>
  <summary>Details</summary>
Motivation: 高棉语拼写检查面临多重挑战：词典与分词模型不对齐、单词有多种书写形式、复合词灵活构成且不在词典中、专有名词因缺乏NER模型而被误判为拼写错误。现有解决方案无法充分应对这些挑战。

Method: 整合高棉语子词分割、命名实体识别、字形到音素转换和语言模型，识别潜在修正候选词并排序最优候选。

Result: 实验结果显示，该方法在高棉语拼写检查上达到94.4%的最先进准确率，优于现有解决方案。

Conclusion: 提出的整体方法有效解决了高棉语拼写检查的核心挑战，建立了相关基准数据集并将公开可用。
...
Result: 开发了一个用户友好的课程推荐系统，能够根据行业趋势和用户偏好提供定制化课程建议。
...
Method: 使用开源LLM微调课程数据，结合RAG方法从本地知识库检索相关文档，并集成多链推理减少幻觉

Result: 在HotpotQA数据集上的实验表明，微调LLM结合RAG方法在问答任务上表现强劲

Conclusion: 提出的系统能有效解决课程论坛问答问题，RAG和微调LLM的组合方法效果显著
...
TL;DR: 提出了TermGPT框架，通过多级对比学习解决LLM在专业领域术语表示不足的问题，在金融和法律领域表现出色。


<details>
  <summary>Details</summary>
Motivation: LLM的嵌入空间存在各向同性问题，导致在专业领域（如法律和金融）中术语级表示能力不足，影响下游任务性能。

Method: 构建句子图捕捉语义和结构关系，基于上下文和拓扑线索生成正负样本，设计句子级和词元级多级对比学习框架。

Result: 在金融和法律领域的术语区分任务中优于现有基线方法，构建了首个基于官方监管文件的金融术语数据集。

Conclusion: TermGPT框架有效提升了LLM在专业领域的术语表示能力，为领域特定应用提供了解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在训练LLMs进行思维链推理时存在两个问题：监督微调惩罚了同样有效的替代方案，而强化学习在信用分配和计算成本方面存在困难。

Method: InTRO利用校正因子——通过生成策略与其答案条件对应策略之间的信息差异估计的令牌级重要性权重，来进行信息丰富的下一个令牌选择，实现单次前向传播中的令牌级探索和自我反馈。

Result: 在六个数学推理基准测试中，InTRO始终优于其他基线方法，相对于基础模型将解决方案准确性提高了高达20%。其思维链更加简洁，减少了冗余。

Conclusion: InTRO不仅提高了推理准确性和简洁性，还展示了跨领域迁移能力，成功适应了数学领域之外的推理任务，表现出强大的泛化能力。
...
TL;DR: HierRouter是一种分层路由方法，通过动态组装轻量级专用语言模型来提升LLM性能，同时控制计算成本。
...
Result: 在六个基准测试中，HierRouter相比单独使用单个模型将响应质量提升了最高2.4倍，仅带来最小的额外推理成本。
...
TL;DR: EnchTable是一个新颖的框架，用于在微调后的LLM中转移和维护安全对齐，通过NTK-based安全向量蒸馏和干扰感知合并技术，在不显著影响性能的情况下确保模型安全。


<details>
  <summary>Details</summary>
Motivation: 针对LLM微调过程中安全对齐系统性退化的问题，需要一种方法在不进行大量重新训练的情况下保持模型安全性。

Method: 使用基于神经正切核(NTK)的安全向量蒸馏方法解耦安全约束与任务特定推理，结合干扰感知合并技术平衡安全性和实用性。

Result: 在11个数据集上的评估显示，EnchTable实现了更低的不安全率、更高的效用分数，并能抵抗静态和动态越狱攻击，优于供应商发布的安全模型。

Conclusion: EnchTable能够无缝集成到各种部署流程中，具有通用适用性，是解决LLM微调安全对齐问题的有效方案。
...
Method: 构建全面的预处理和筛选流程，包括面部关键点检测、唇部区域隔离稳定、多模态样本质量评估；采用课程学习策略，从高质量样本逐步扩展到困难样本；使用SigLIP编码器和Unified 3D-Resampler高效编码高帧率唇部运动。
...
TL;DR: 提出了Norm-RAG框架，通过检索增强和语义分块方法在多轮对话中进行细粒度的社会规范推理，并创建了MINDS双语数据集来评估模型性能。


<details>
  <summary>Details</summary>
Motivation: 社会规范推理具有主观性、语境依赖性和文化差异性，现有方法主要针对孤立语句或合成对话，难以捕捉真实对话的流动性和多轮特性。

Method: Norm-RAG框架建模话语级属性（沟通意图、说话者角色、人际框架、语言线索），通过语义分块方法从结构化规范文档中检索相关信息，实现可解释的规范推理。

Result: 实验表明Norm-RAG提高了规范检测和泛化能力，在文化适应性和社交智能对话系统方面表现出改进性能。

Conclusion: 该框架为多语言对话中的社会规范推理提供了有效解决方案，有助于开发更具文化敏感性的对话系统。
...
TL;DR: 本研究通过扩展LLM提示策略到646个多选题数据集，发现自动生成的KC模型性能较差且冗余严重。提出基于余弦相似度的语义合并方法，显著提升了模型性能并减少了KC数量。


<details>
  <summary>Details</summary>
Motivation: 知识组件(KC)是自适应学习系统的基础，但手动识别存在瓶颈。虽然LLM提供了自动化可能，但现有研究限于小数据集且产生冗余KC标签。

Method: 首先使用GPT-4o-mini扩展"模拟教科书"提示策略到更大数据集，然后提出基于余弦相似度的语义合并方法来减少冗余KC标签。

Result: 初始自动方法性能显著差于专家设计模型(RMSE 0.4285 vs 0.4206)，生成过多KC(569 vs 101)。使用余弦相似度阈值0.8的合并策略将KC减少到428个，RMSE提升到0.4259。
...
TL;DR: REAP方法通过子任务规划器和事实提取器模块解决多跳推理任务中的全局规划不足问题，显著提升推理准确性和可追溯性
...
Conclusion: REAP方法通过结构化子任务和事实管理，有效提升了复杂多跳推理任务的可靠性和可追溯性
...
TL;DR: 对数值声明进行事实核查时，即使最先进的LLM在受控扰动下准确率下降高达62%，没有模型在所有条件下都稳健。增加上下文长度通常会降低准确率，但加入扰动演示后大多数模型能显著恢复。
...
Result: 即使领先的专有系统在某些扰动下准确率下降高达62%，没有模型在所有条件下都稳健。增加上下文长度通常降低准确率，但用扰动演示丰富扩展上下文后，大多数模型能显著恢复。
...
TL;DR: 提出了一种名为ETC的训练无关方法，通过建模token级不确定性的动态变化来确定最佳检索时机，能够在错误传播前更早、更精确地触发检索。
...
Method: ETC方法利用熵序列的一阶和二阶差分来检测不确定性趋势，实现趋势感知的不确定性建模，无需训练即可确定最佳检索时机。

Result: 在6个QA基准测试和3个LLM骨干网络上，ETC持续优于强基线方法，同时减少了检索频率，在领域特定场景中表现尤为突出。

Conclusion: ETC是一种即插即用、模型无关的方法，可轻松集成到现有解码流程中，趋势感知的不确定性建模能够产生更有效的检索时机。
...
<details>
  <summary>Details</summary>
Motivation: 在多语言RAG中，当检索到的证据与用户查询和上下文示例的语言不同时，模型经常会出现语言漂移现象，在推理密集型解码（如思维链生成）中尤其明显。

Method: 提出了Soft Constrained Decoding (SCD)方法，这是一种轻量级、无需训练的解码策略，通过惩罚非目标语言标记来温和地引导生成朝向目标语言。

Result: 在三个多语言数据集和多种类型不同语言上的实验表明，SCD持续改善了语言对齐和任务性能。

Conclusion: SCD为多语言RAG提供了一个有效且可泛化的解决方案，能够有效缓解输出语言漂移问题。
...
TL;DR: BERTScore在金融领域存在数值敏感性不足的问题，无法有效区分语义上重要的数值差异，作者构建了FinNuE诊断数据集来验证这一局限性。


<details>
  <summary>Details</summary>
Motivation: BERTScore作为广泛使用的语义相似度评估指标，在金融领域存在严重缺陷——对数值变化不敏感，而金融文本中数值精度直接影响语义（如2%收益与20%损失的区分）。

Method: 构建FinNuE诊断数据集，通过在财报电话会议、监管文件、社交媒体和新闻文章中引入受控的数值扰动，系统评估BERTScore对数值变化的敏感性。

Result: BERTScore无法区分语义上关键的数值差异，经常为金融意义上截然不同的文本对分配高相似度分数，暴露了基于嵌入的指标在金融领域的根本局限性。

Conclusion: 研究揭示了BERTScore在金融NLP中的数值敏感性缺陷，强调了开发数值感知评估框架的必要性，为金融领域专用评估指标的发展提供了动机。
...
Method: 设计了NCERT-QA数据集，包含事实型、推理型和其他类型的问题对；使用元提示、少样本和思维链等提示技术；评估了Gemma3、Llama3.2、Nemotron-mini等开源模型和Llama-4-Scout、Deepseek-r1等高端模型。

Result: 通过多种评估指标分析了不同提示技术和LLMs在课程对齐方面的表现，识别了当前模型的优势和局限性。

Conclusion: 为LLMs在正式教育系统中的应用提供了评估框架，揭示了不同模型和提示技术在教育场景中的适用性。
...
Method: 将长输入分段为重叠块，使用参数无关的融合机制为每个块表示赋予文档结构意识，通过累积前后块的上下文向量来丰富边界表示。
...
<details>
  <summary>Details</summary>
Motivation: 语音象征性（语音形式与意义之间的非任意关联）是理解MLLM如何处理听觉信息的重要探针，旨在探索模型在认知语言学层面的表现。

Method: 使用LEX-ICON数据集（包含4种语言的8,052个真实词和2,930个伪词），通过文本（正字法和IPA）和音频输入，测量模型在25个语义维度上的层间注意力分数。

Result: 发现MLLM的语音直觉与现有语言学研究一致，且注意力模式集中在具有象征性的音素上，表明模型能够识别语音-语义关联。

Conclusion: 这项研究首次在MLLM可解释性框架下对语音象征性进行了大规模定量分析，为人工智能与认知语言学的交叉研究搭建了桥梁。
...
TL;DR: GraphIF是一个即插即用框架，通过将多轮对话建模为有向关系图，利用图提示增强LLMs的多轮指令跟随能力，显著提升了指令跟随性能。
...
Conclusion: 图结构特别适合建模多轮指令跟随，GraphIF框架有效解决了现有方法在处理复杂长距离约束方面的局限性，为增强LLMs的多轮对话能力提供了新思路。
...
TL;DR: ADI-20是阿拉伯语方言识别数据集的扩展，包含19种阿拉伯方言和现代标准阿拉伯语，共3556小时音频。研究了基于ECAPA-TDNN和Whisper的模型，发现仅用30%训练数据时F1分数仅有小幅下降。


<details>
  <summary>Details</summary>
Motivation: 扩展阿拉伯语方言识别数据集，覆盖所有阿拉伯语国家的方言，为阿拉伯语方言识别研究提供更全面的数据支持。

Method: 使用ECAPA-TDNN预训练模型微调，以及结合Whisper编码器块、注意力池化层和分类密集层的模型。研究了训练数据量和模型参数数量对性能的影响。
...
<details>
  <summary>Details</summary>
Motivation: 随着科学论文数量增长，需要系统辅助评审员评估研究声明。当前多模态LLMs在不同证据格式（表格和图表）下验证科学声明的鲁棒性仍是一个重要但未被充分探索的挑战。

Method: 设计实验评估12个多模态LLMs使用表格和图表作为证据验证科学声明的能力，改编两个现有科学论文数据集以支持多模态声明验证任务，并进行人工评估。

Result: 当前模型在基于表格的证据上表现更好，而在基于图表的证据上表现较差；人工评估显示人类在两种格式上均保持强劲表现；较小多模态LLMs（小于8B）在表格和图表任务间表现相关性弱，表明跨模态泛化能力有限。
...
<details>
  <summary>Details</summary>
Motivation: 研究如何利用大型预训练语音模型进行阿拉伯语多方言语音处理，特别是在方言识别和自动语音识别任务中的表现。
...
Method: 1. 询问基于GPT的语言模型（微软Copilot）让其揭示自身对军事应用的认知；2. 研究如何利用商业云服务（微软Azure）构建此类应用并评估可行性。

Result: 发现语言模型的摘要生成和内容生成特性直接支持多种大规模应用，其他功能也可能找到特定用途。

Conclusion: 语言模型的摘要和生成特性为军事应用提供了直接支持，其他功能也具有特定应用潜力。
...
TL;DR: 提出一种通过因果视角减少事件和领域相关偏差的方法，提升对新兴灾难事件的泛化能力，在三个灾难分类任务中显著优于基线方法。
...
Result: 方法在三个灾难分类任务中比多个基线方法提升高达+1.9% F1分数，显著改善了基于预训练语言模型的分类器性能。

Conclusion: 因果学习方法能有效缓解灾难事件领域的偏差问题，提升模型对新事件的泛化能力。
...
Result: VISTA提供了一个统一交互框架，显著降低了分析推理链的复杂度，促进了对当前语言模型能力和局限性的深入理解。

Conclusion: VISTA平台是开源的，支持自定义基准测试和本地模型的轻松集成，为多轮推理分析提供了有效的可视化解决方案。
...
TL;DR: 提出了Text2SQL-Flow框架，通过数据增强生成大规模、高质量、多样化的Text-to-SQL对，构建了包含89,544个标注样本的SQLFlow数据集，显著提升了开源和闭源LLM在Text-to-SQL任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 解决Text-to-SQL领域数据稀缺、简单和多样性不足的问题，这些限制了AI模型的性能发展。

Method: 开发了SQL感知的数据增强框架，包含六个增强维度，集成了SQL执行验证、自然语言问题生成、思维链推理追踪和数据分类的端到端流程，使用模块化数据库管理器确保跨数据库兼容性。

Result: 构建了SQLFlow数据集（89,544个样本），在开源LLM上微调和在闭源LLM上使用掩码对齐检索方法均显著提升了性能，超越了现有方法。
...
TL;DR: EffiReason-Bench是一个统一的基准测试，用于评估高效推理方法，包含三个类别：推理蓝图、动态执行和后处理优化。通过构建经过验证的CoT标注和提出E3-Score指标，在多个数据集上评估了7种方法和6个LLM，发现最优策略取决于模型规模、任务复杂度和架构。


<details>
  <summary>Details</summary>
Motivation: 现有LLM的CoT推理往往产生不必要的冗长解释，增加成本并可能降低准确性。不同效率导向方法的公平比较受到碎片化评估实践的阻碍。

Method: 构建EffiReason-Bench基准，包含三个方法类别：推理蓝图、动态执行和后处理优化。通过标准化推理结构、全面选项分析和人工验证的流程构建验证过的CoT标注。提出E3-Score指标，基于经济权衡建模提供平滑稳定的评估。

Result: 在数学、常识和逻辑领域的4个数据集上评估了7种方法和6个开源LLM（1B-70B）。实验表明没有单一方法在所有情况下都最优，最优策略取决于骨干模型规模、任务复杂度和架构。

Conclusion: 需要根据具体场景选择推理方法，没有通用的最优解决方案。EffiReason-Bench为高效推理方法提供了统一的评估框架和原则性指标。
...
TL;DR: 提出了PAL框架，通过将人设对齐作为训练目标，采用两阶段训练方法提升个性化对话生成的人设相关性和一致性。


<details>
  <summary>Details</summary>
Motivation: 主流模型依赖词级语言模型训练，往往忽视给定人设信息，生成通用回复而非个性化回复。

Method: 两阶段训练：人设感知学习和人设对齐，配备选择后生成的推理策略，在语义层面提升人设敏感性。

Result: 实验表明该框架优于许多最先进的个性化对话方法和大型语言模型。

Conclusion: PAL框架通过直接优化人设对齐目标，能生成更相关的人设对话回复。
...
Method: 两阶段预选择框架：首先基于语言可分离性分数过滤训练数据，然后使用现有选择方法细化子集

Result: 在6个基准测试和22种语言上的实验表明，LangGPS能提高现有选择方法的有效性和泛化能力，特别是在理解任务和低资源语言上

Conclusion: 语言可分离性可作为多语言环境中数据效用的新视角，支持开发更具语言意识的LLMs，也可用于多语言课程学习
...
Result: 首块延迟从约725ms大幅降低到350ms，同时在主流语音语言模型中保持竞争性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注单轮交互和对话特征，忽视了多轮通信的复杂性以及指令遵循和安全性等关键能力，且在多轮设置下评估全双工语音语言模型存在对话轮次边界模糊和推理上下文不一致的挑战。
...
TL;DR: 提出了一种本地化问答系统，通过结合语义理解和关键词精度，在无需互联网访问的情况下实现隐私保护和竞争性准确率
...
Method: 结合两种互补的检索策略（语义理解和关键词精度），在本地基础设施上运行，使用消费级硬件加速

Result: 在复杂查询（法律、科学、对话文档）上实现竞争性准确率，错误率最小，让银行、医院和律师事务所能够采用对话式文档AI而无需传输专有信息

Conclusion: 在企业AI部署中，隐私和性能不必相互排斥，可以同时实现
...
TL;DR: 本文提出了一种基于困惑度感知强化学习的方法来纠正LLMs在数学推理中的不平衡评估偏好，通过构建OPS基准量化评估行为差异，并利用GRPO算法提升评判能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注构建高质量监督微调数据来增强评判能力，但忽视了LLMs评判性能差的内在原因——不平衡的评估偏好，即LLMs倾向于将低困惑度的解决方案判断为正确。

Method: 构建OPS基准量化LLMs评估行为差异，进行基于困惑度的统计偏好分析，提出困惑度感知的强化学习算法（GRPO）来纠正评估偏好，支持LLMs探索将低困惑度判断为错误、高困惑度判断为正确的轨迹。

Result: 在构建的OPS基准和现有评判基准上的广泛实验结果表明，该方法能有效提升LLMs的评判能力。

Conclusion: 通过识别和纠正LLMs的不平衡评估偏好，提出的困惑度感知强化学习方法能显著提升多步数学推理中的评判能力，为改进LLMs的推理能力提供了新思路。
...
TL;DR: 本研究系统性地生成和评估了印度语言的多语言合成预训练数据，构建了包含540B标记的BhashaKritika数据集，探索了基于文档、角色和主题的生成方法，并开发了模块化质量评估框架。
...
Method: 使用5种不同技术为10种语言构建大规模合成数据集；探索基于文档、角色和主题的生成方法；分析提示语言和文档基础对数据质量的影响；比较英语内容翻译与本地生成；开发包含脚本检测、元数据一致性检查、n-gram重复分析和基于困惑度过滤的模块化质量评估管道。
...
Method: 结合注释模型、本体框架和LLM提取的五步方法：基础分析、注释模式开发、管道架构、集成优化和全面评估，使用三个LLM（Claude Sonnet 3.7、Llama 3.3 70B、GPT-4o-mini）构建顺序管道。
...
Conclusion: 这是首个协调LLM提取与文化遗产本体的系统方法，提供了可跨文化遗产领域和机构资源复制的框架，但需要人工监督后处理。
...
TL;DR: TruthfulRAG是首个利用知识图谱解决RAG系统中事实级知识冲突的框架，通过构建知识图谱、图检索和熵过滤机制来精确定位冲突元素，提升生成内容的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统在检索外部信息与LLM内部知识冲突时，通常只在词元或语义层面处理，导致对事实差异的理解碎片化，特别是在知识密集型任务中影响生成内容的准确性和可靠性。

Method: TruthfulRAG通过系统地从检索内容中提取三元组构建知识图谱，使用基于查询的图检索识别相关知识，并采用基于熵的过滤机制精确定位冲突元素，缓解事实不一致性。

Result: 大量实验表明，TruthfulRAG优于现有方法，有效缓解知识冲突，提高了RAG系统的鲁棒性和可信度。

Conclusion: TruthfulRAG框架通过知识图谱有效解决了RAG系统中的事实级知识冲突问题，为生成忠实准确的响应提供了可靠解决方案。
...
TL;DR: 本文认为评估基础LLM的推理能力存在方法论问题，因为基础LLM的预训练目标与推理评估的规范性质量（如正确性）存在根本性不匹配。
...
Conclusion: 呼吁重新审视依赖这些假设的现有研究，并建议未来工作要考虑到这些方法论陷阱。
...
TL;DR: 本文提出了DELICATE方法，一种结合BERT编码器和Wikidata上下文信息的神经符号方法，用于历史意大利语实体链接，并创建了ENEIDE多领域语料库。
...
Method: DELICATE方法结合BERT编码器与Wikidata上下文信息，利用时间合理性和实体类型一致性来选择知识库实体。ENEIDE语料库是从19-20世纪注释版本中半自动提取的多领域历史意大利语语料。

Result: DELICATE在历史意大利语实体链接任务中优于其他模型，即使与拥有数十亿参数的大型架构相比。其置信度分数和特征敏感性提供了比纯神经方法更可解释的结果。

Conclusion: DELICATE方法在历史意大利语实体链接任务中表现出色，提供了更可解释的结果，ENEIDE语料库为领域研究提供了重要资源。
...
TL;DR: 类比范式组织能让轻量级模型用极少数据达到大模型性能。通过认知启发的类比结构、对比学习和最小上下文线索，在仅100个英语使役/起始交替的结构化示例上训练，轻量模型（0.5M参数）F1=0.95，优于GPT-3零样本（F1=0.87）。


<details>
  <summary>Details</summary>
Motivation: 探索是否通过类比范式组织，能让轻量级模型用极少量数据达到大语言模型的性能水平，解决大模型对海量数据的依赖问题。

Method: 开发计算框架实现三个认知启发原则：类比结构、对比学习和最小上下文线索。使用结构化补全任务，让模型从类比模式中识别正确句子补全。

Result: 轻量级模型（BERT+CNN，50万参数）在仅100个英语使役/起始交替的结构化示例上训练，达到F1=0.95，显著优于GPT-3零样本（F1=0.87）。消融研究确认类比组织和对比结构提升性能。

Conclusion: 类比范式组织能实现竞争性语言规则学习，所需数据量比传统方法少几个数量级，为高效语言学习提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理模糊请求时往往隐式选择一个解释，这种意图误解会使用户感到沮丧并带来安全风险。
...
TL;DR: LLMs在状态跟踪任务中的表现分析，新一代模型（GPT-4、Llama3）在Chain of Thought机制下能有效跟踪状态，而老一代模型在初始阶段后容易失败。
...
Method: 基于三个明确定义的状态跟踪任务构建基准测试，分析不同代际LLMs（GPT-4、Llama3 vs 老一代模型）在有无Chain of Thought机制下的表现。

Result: 新一代LLMs能够有效跟踪状态，特别是在集成Chain of Thought机制时；老一代模型虽然理解任务并能在初始阶段解决，但在一定步骤后经常失败。

Conclusion: 新一代LLMs具备状态跟踪能力，Chain of Thought机制能显著提升性能，但模型代际差异明显，老一代模型在复杂状态跟踪中存在局限性。
...
TL;DR: LocalBench是首个系统评估LLMs在美国县级本地知识能力的基准，包含14,782个验证问答对，覆盖526个县。评估显示现有模型在本地知识方面存在严重不足，最佳模型在叙述式问题中仅达56.8%准确率，数值推理低于15.5%。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs评估主要关注宏观地理任务，但缺乏对超本地知识能力的理解。随着从公民平台到社区新闻等实际应用需求增长，需要能够推理邻里特定动态、文化叙事和本地治理的AI系统。

Method: 基于本地性概念框架构建LocalBench基准，整合人口普查统计、本地subreddit讨论和区域新闻等多样化来源，涵盖物理、认知和关系三个本地维度。评估了13个最先进LLMs在闭卷和网络增强设置下的表现。

Result: 模型表现存在严重局限：最佳模型在叙述式问题中准确率仅56.8%，数值推理低于15.5%。更大的模型规模和网络增强不能保证更好性能，搜索使Gemini准确率提升+13.6%，但使GPT系列下降-11.4%。

Conclusion: 结果强调了迫切需要能够支持公平、位置感知AI系统的语言模型，这些系统能够参与跨越地理和文化背景的本地社区的多样化、细粒度现实。
...
TL;DR: KPPO是一个基于知识供给的提示优化框架，通过系统性的知识整合而非单纯的能力激发来优化提示，在知识密集型任务上显著优于传统的激发式方法。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法主要关注激发模型能力，但在知识密集型任务中存在根本性局限，无法提供必要的专业知识、术语精确性和推理模式。
...
TL;DR: 提出了AdvancedIF基准测试和RIFL训练框架，通过基于评分标准的强化学习显著提升大语言模型的复杂指令跟随能力
...
Result: RIFL在AdvancedIF上实现了6.7%的绝对提升，并在公共基准上表现优异，消融研究证实了各组件有效性
...
<details>
  <summary>Details</summary>
Motivation: 物理奥林匹克竞赛问题需要精确计算、抽象推理和物理原理的复杂整合，对AI构成重大挑战，CPhO作为复杂严谨的测试平台
...
TL;DR: 研究发现语言风格重构（如恐惧、好奇等）能显著提高对大型语言模型的越狱成功率，最高可达57个百分点，并提出了风格中和预处理方法来缓解这一漏洞。
...
Result: 风格重构显著提高越狱成功率（最高+57%），恐惧、好奇和同情等风格最有效，情境化改写优于模板化变体。

Conclusion: 语言风格重构是一个被当前安全管道忽视的系统性且难以扩展的漏洞，需要新的防御措施。
...
TL;DR: 提出了一个包含75,336个问答对的对话记忆评估基准，分析了对话记忆与RAG系统的关系，发现简单全上下文方法在150次对话内比复杂RAG系统表现更好，揭示了不同对话规模下的最佳实践。


<details>
  <summary>Details</summary>
Motivation: 现有对话记忆评估基准存在统计能力不足、数据生成不一致和评估灵活性受限等问题，需要更全面的评估框架来推动对话记忆系统的发展。

Method: 构建大规模对话记忆评估基准，比较简单全上下文方法与复杂RAG系统在不同对话规模下的表现，分析对话记忆与RAG系统的异同。

Result: 简单全上下文方法在150次对话内达到70-82%准确率，而复杂RAG系统只有30-45%；长上下文在前30次对话中表现最佳，150次对话内仍可行，超过150次需要混合或RAG方法。

Conclusion: 对话记忆系统具有从小规模语料开始的独特优势，需要专门研究而非简单应用通用RAG解决方案，因为穷举搜索和完全重排在对话历史中是可行的。
...
TL;DR: 使用计算方法验证文学体裁作为形式分类而非制度分类的有效性，发现每个文学类别都有显著的形式标记，女性作者身份会缩小和模糊获得文学地位的标准。


<details>
  <summary>Details</summary>
Motivation: 探索体裁作为形式分类与制度分类的合理性，分析当代文学中体裁小说与文学小说的区别，特别关注作者性别在文学分类中的作用。

Method: 使用CONLIT数据集构建文学小说和体裁小说语料库，采用Welch's ANOVA比较叙事特征分布，使用逻辑回归建模特征对文学分类的影响，分析风格和语义向量表示。

Result: 发现每个文学类别都有统计显著的形式标记，女性作者身份会缩小获得文学地位的标准并模糊分类界限。

Conclusion: 体裁确实具有形式上的显著标记，但作者性别在文学分类中起着重要的调节作用，女性作者面临更严格的文学地位标准。
...
TL;DR: URaG是一个统一检索与生成的多模态大语言模型框架，通过利用模型早期层的广泛注意力机制作为证据选择器，有效提升长文档理解效率，减少44-56%计算开销。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在长文档理解中的两大挑战：信息干扰和二次计算成本。现有方法要么牺牲细节，要么增加系统复杂度。

Method: 提出URaG框架，引入轻量级跨模态检索模块，将早期Transformer层转换为证据选择器，识别并保留相关页面，让深层专注于相关信息。

Result: 在广泛实验中达到最先进性能，同时减少44-56%计算开销。

Conclusion: URaG证明了MLLMs内在的证据定位能力可以明确用于推理过程中的检索，实现高效的长文档理解。
...
TL;DR: DESS是一个基于DeBERTa和LSTM的双通道框架，用于改进方面情感三元组提取(ASTE)，通过增强的注意力机制更好地理解文本中的上下文和关系。


<details>
  <summary>Details</summary>
Motivation: 细粒度情感分析在ASTE任务中面临准确捕捉方面、观点和情感极性之间关系的挑战，现有方法未能充分利用先进语言模型理解复杂语言模式的潜力。

Method: 构建DeBERTa和LSTM的双通道结构，DeBERTa处理上下文关系，LSTM处理语法模式，精心设计组件间的协作机制来处理不同类型的语言信息交互。

Result: 在标准数据集上测试显示显著改进，识别方面观点对和确定情感的F1分数分别提升了4.85、8.36和2.42，DeBERTa的注意力系统特别擅长处理复杂句子结构和远距离重要词汇。

Conclusion: 通过精心整合更先进的语言模型，可以在文本情感分析方面实现实质性改进，DeBERTa的复杂注意力系统有助于更好地处理复杂语言结构。
...
TL;DR: 本文研究了使用MedGemma模型从医患对话中提取医疗指令的最佳提示策略，发现简单的单样本提示方法在验证集上表现最佳，而复杂的推理框架可能导致"过度思考"问题。


<details>
  <summary>Details</summary>
Motivation: 准确提取医患对话中的医疗指令对于减轻临床文档负担和确保患者安全至关重要。本文旨在探索不同提示策略在医疗指令提取任务中的表现。

Method: 系统评估了三种不同的提示范式：简单的单样本方法、推理导向的ReAct框架和多步骤智能体工作流，使用MedGemma模型在MEDIQA-OE-2025共享任务数据集上进行实验。

Result: 实验表明，虽然ReAct和智能体流程等复杂框架功能强大，但简单的单样本提示方法在官方验证集上取得了最高性能。

Conclusion: 在人工标注的转录本上，复杂的推理链可能导致"过度思考"并引入噪声，使得直接方法更加稳健和高效。这项工作为在不同数据条件下选择适当的临床信息提取提示策略提供了宝贵见解。
...
TL;DR: 提出了两种互补方法用于伤口护理查询应答生成：基于挖掘提示的策略和元数据引导生成方法，通过检索相似示例和预测关键元数据来提升回答质量。


<details>
  <summary>Details</summary>
Motivation: 异步远程医疗的快速发展增加了医生工作负担，需要AI系统来帮助临床医生更高效地处理患者查询，特别是在伤口护理领域。

Method: 1. 挖掘提示策略：嵌入训练数据并检索top-k相似示例作为少样本演示；2. 元数据引导生成：训练分类器预测关键元数据属性，并根据预测置信度动态调整生成输出。

Result: 挖掘提示提高了回答相关性，元数据引导生成进一步提升了临床精确度，两种方法共同展示了开发可靠高效伤口护理AI工具的有前景方向。

Conclusion: 两种互补方法为开发能够提供可靠高效伤口护理支持的AI驱动工具指明了有希望的方向。
...
TL;DR: 本文提出了一种编码器增强的因果解码器模型架构，通过考虑语言的信息熵约束来改进语言模型的训练和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于因果（下一个token预测）的大型语言模型虽然是最有效的语言压缩算法，但用它们来准确估计语言熵在计算上不可行。语言预测受到语言内在信息熵的约束，存在语言模型准确性的上限和语言压缩的下界。

Method: 引入编码器增强的因果解码器模型架构，展示如何基于每个token获得熵估计，并证明训练模型接近但不超出估计的每个token熵值可以带来更好的泛化能力。

Result: 该架构在训练效率上表现优越，即使在适度硬件上训练也能实现比因果变换器更高的压缩率。经验表明，考虑熵约束训练的因果模型比不考虑熵的模型具有更好的泛化能力。

Conclusion: 训练模型接近但不超出训练数据的熵估计值，其泛化能力必然超过训练以最小化超出该值的损失的模型。考虑语言熵约束可以提升语言模型的泛化性能。
...
Method: 使用公开数据和代码库，通过大规模预训练、通用指令微调以及与人类偏好对齐来开发模型，并针对特定任务创建专门变体。

Result: Instella在完全开源模型中达到最先进水平，与同规模领先开源权重模型性能相当，尽管使用了更少的预训练token。
...
Method: 将学生LLM作为生成器，训练判别器区分师生模型响应，形成极小极大博弈框架，判别器作为在线奖励模型与学生协同进化。

Result: GAD持续超越常用的序列级知识蒸馏方法，Qwen2.5-14B-Instruct学生模型在LMSYS-Chat自动评估中达到与GPT-5-Chat教师模型相当的水平。

Conclusion: GAD是黑盒LLM蒸馏的一种有前景且有效的范式。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型权重和激活中的异常值会导致较大的量化误差和严重的精度下降，特别是在推理任务中，误差会在长思维链中累积。现有PTQ方法要么无法充分抑制异常值，要么在推理时引入显著开销。

Method: 结合硬件高效的独立Givens旋转和通道级缩放，均衡通道间的幅度并缩小每个量化组内的动态范围，同时协同设计推理内核以充分利用GPU并行性，保持旋转和缩放操作在运行时轻量。
...
TL;DR: FedeCouple是一种联邦学习方法，通过联合学习全局和局部特征表示，使用动态知识蒸馏增强个性化分类器的泛化能力，引入锚点优化特征空间，在异构客户端数据场景下平衡全局泛化与局部适应性。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注特征空间一致性和分类个性化，但忽略了提取器的局部适应性和分类器的全局泛化性，导致组件间协调不足、耦合弱，影响整体模型性能。

Method: 联合学习全局和局部特征表示；使用动态知识蒸馏增强个性化分类器泛化；引入严格局部且不传输的锚点优化特征空间；提供非凸目标收敛性理论分析。

Result: 在五个图像分类数据集上的实验表明，FedeCouple在有效性、稳定性、可扩展性和安全性方面均优于九种基线方法，有效性实验中比最佳基线显著提升4.3%。

Conclusion: FedeCouple通过精细平衡全局泛化与局部适应性，在保护隐私的同时提升了联邦学习性能，理论分析和实验验证了其优越性。
...
TL;DR: 本文提出了ParaBench基准测试，发现现有顺序自回归方法在思考感知生成中存在性能退化问题，并提出并行多模态扩散框架MMaDA-Parallel来解决跨模态对齐问题。


<details>
  <summary>Details</summary>
Motivation: 现有顺序自回归方法在复杂任务中由于错误传播可能导致性能退化，特别是在文本和图像输出模态之间的对齐问题上。

Method: 提出并行多模态扩散框架MMaDA-Parallel，支持文本和图像在去噪过程中的连续双向交互，并通过监督微调和并行强化学习(ParaRL)进行优化。

Result: 在ParaBench基准测试中，相比最先进的Bagel模型，输出对齐度提高了6.9%，显著改善了跨模态对齐和语义一致性。

Conclusion: MMaDA-Parallel为思考感知图像合成建立了更鲁棒的范式，通过并行交互机制有效解决了跨模态对齐问题。
...
TL;DR: 提出了PriVi——一个大规模灵长类动物为中心的预训练视频数据集，包含424小时精心策划的视频，通过数据为中心的方法显著提升了灵长类行为分析的泛化能力和数据效率。


<details>
  <summary>Details</summary>
Motivation: 现有计算机视觉方法依赖人类中心预训练模型且专注于单一数据集，限制了在灵长类行为分析中的泛化能力。

Method: 构建PriVi数据集（424小时视频），在V-JEPA模型上进行灵长类特定表示预训练，使用轻量级冻结分类器评估。

Result: 在四个基准数据集上持续优于先前工作，包括完全微调的基线，并在少标签情况下表现更好。

Conclusion: 灵长类为中心的预训练显著提高了数据效率和泛化能力，是低标签应用的有前景方法。
...
TL;DR: 提出首个从声带图像自动分类发音创伤严重程度的方法，采用序数回归框架处理有序标签，并提出软标签序数回归损失函数来处理标注不确定性。


<details>
  <summary>Details</summary>
Motivation: 发音创伤严重程度评估依赖临床专家判断，成本高且可靠性差异大，需要自动化工具来支持大规模研究和改善患者护理。

Method: 采用序数回归框架处理有序标签，提出新颖的软标签序数回归损失函数来处理标注者评分分布的不确定性。

Result: 提出的软序数回归方法预测性能接近临床专家水平，同时产生良好校准的不确定性估计。

Conclusion: 该自动化工具能够实现发音创伤严重程度的大规模研究，最终改善临床理解和患者护理。
...
TL;DR: SliderEdit是一个用于基于指令的图像编辑的连续控制框架，通过全局训练的滑块实现对单个编辑指令强度的精细调节，无需为每个属性单独训练。
...
Method: SliderEdit将多部分编辑指令解耦，为每个指令训练全局滑块，使用单一低秩适配矩阵集泛化到各种编辑、属性和组合指令。

Result: 在FLUX-Kontext和Qwen-Image-Edit等先进模型上应用SliderEdit，显著提升了编辑可控性、视觉一致性和用户可操作性。

Conclusion: SliderEdit首次探索并提出了在基于指令的图像编辑模型中实现连续、细粒度指令控制的框架，为交互式、指令驱动的图像操作铺平了道路。
...
TL;DR: 将图像人群密度估计算法扩展到视频场景，通过扩散模型生成高质量密度图，结合事件驱动采样减少计算负担，在密集和稀疏场景下都能有效捕捉人群动态。
...
Method: 集成去噪概率模型生成密度图，使用窄高斯核和多输出，加入回归分支进行特征提取，基于相似度合并密度图，采用Farneback光流算法的事件驱动采样技术。

Result: 通过定性和定量评估（包括叠加图和MAE），模型在密集和稀疏场景下都能有效捕捉人群动态，采样方法显著减少帧数同时保持关键人群事件。

Conclusion: 该工作成功解决了视频人群密度估计的时序挑战，提供了一个可扩展且高效的实时人群监控框架。
...
<details>
  <summary>Details</summary>
Motivation: 解决GPS缺失环境下的室内定位问题，克服现有视觉方法如PALMS的局限性，包括智能手机LiDAR短距离和室内布局模糊性问题。
...
Conclusion: PALMS+展示了无需摄像头跟踪的鲁棒性，在无需基础设施的应用中具有潜力，为GPS缺失环境下的室内定位提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将行人视为点实体，忽略了每个人占据的物理空间，在动态拥挤环境中难以准确预测轨迹和避免碰撞。

Method: 在Social LSTM基础上引入动态占用空间损失函数，结合平均位移误差和对场景密度及个体空间占用敏感的碰撞惩罚项。

Result: 模型在五个数据集上平均降低碰撞率31%，平均位移误差和最终位移误差分别降低5%和6%，在大多数测试集上优于现有先进模型。

Conclusion: 提出的动态占用空间损失函数能有效指导模型学习避免现实碰撞，同时保持位移预测精度，在不同密度场景下均表现优异。
...
TL;DR: 本文通过将汽车摄像头污染检测视为语义分割问题，比较了流行分割方法与传统瓦片分类方法的性能优势，并修正了Woodscape数据集中的数据泄漏和标注不精确问题。
...
Method: 将污染检测视为语义分割问题，比较了流行分割方法与瓦片分类方法，并创建了新的数据子集来解决原始数据集中的数据泄漏和标注不精确问题。

Result: 尽管新数据子集规模小得多，但为分割方法提供了足够信息，在更短时间内达到了可比较的结果。

Conclusion: 语义分割方法在汽车摄像头污染检测中优于瓦片分类方法，修正后的数据集虽小但有效。
...
TL;DR: 评估8个医学和通用领域基础模型在胸部X光分析中的表现，发现医学领域预训练显著优于通用领域，但特征有效性高度任务相关，对于复杂病理分割任务，预训练特征效果有限，监督模型仍具竞争力。
...
Result: 医学领域预训练显著优于通用领域；预训练嵌入在全局分类和显著解剖结构分割中表现良好，但在复杂病理分割中效果差；发现模型使用混淆捷径进行分类；文本-图像对齐并非必需，图像专用和监督模型表现优异。

Conclusion: 医学预训练有益，但架构选择关键；预训练特征并非普遍有效，特别是在复杂定位任务中，监督模型仍是强有力的替代方案。
...
TL;DR: 提出一种通过遍历生成模型潜在空间来增强虹膜图像的方法，通过梯度引导操纵特定几何、纹理或质量相关特征，同时保持身份信息。


<details>
  <summary>Details</summary>
Motivation: 开发可靠的虹膜识别和呈现攻击检测方法需要能够捕捉虹膜特征真实变化和广泛异常的数据集，但由于虹膜纹理丰富且空间频率范围广，合成相同身份但控制特定属性的图像具有挑战性。

Method: 通过遍历生成模型的潜在空间，使用特定几何、纹理或质量相关虹膜图像特征的梯度引导，操纵所需属性同时保持身份信息。可以使用预训练GAN模型生成随机图像或真实虹膜图像，通过GAN反演将图像投影到潜在空间。

Result: 该方法能够合成相同身份但具有操纵属性的虹膜图像，可轻松扩展到任何可微分损失项定义的属性。

Conclusion: 提出的潜在空间遍历方法为虹膜图像增强提供了有效策略，能够控制特定属性同时保持身份一致性，适用于虹膜识别和呈现攻击检测的数据增强需求。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖第一帧的手动标注分割掩码，这既耗时又在面对遮挡或快速运动时性能下降。STORM旨在解决这些限制，提供无需人工标注的解决方案。

Method: 采用三阶段流程：上下文对象描述指导定位、自交叉注意力机制识别候选区域、分割模型生成精确掩码用于姿态估计。还包含自动重新注册机制，通过特征相似性监测检测跟踪失败并从严重遮挡或快速运动中恢复。
...
TL;DR: PANDA是一个用于无示例持续学习的补丁和分布感知增强框架，通过CLIP编码器识别代表性区域并移植到频繁类样本中，同时利用自适应平衡策略平滑任务间不平衡。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据流存在双重不平衡问题：数据集级分布与单个任务内的极端或反向偏斜，这种任务内和任务间的不平衡阻碍了有效学习和泛化。
...
Conclusion: PANDA框架有效解决了无示例持续学习中的双重不平衡问题，能够提升学习效果并缓解灾难性遗忘。
...
TL;DR: STS是一种轻量级的测试时适应框架，通过提取文本嵌入的光谱子空间来定义主要语义方向，并通过调整少量参数来最小化增强视图的熵，无需反向传播或修改编码器。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在零样本推理中表现优异，但在测试时域偏移下性能会下降。现有的测试时适应策略通常需要反向传播或修改模型核心组件，计算成本高。

Method: 从文本嵌入中提取光谱子空间定义语义方向，学习通过调整少量参数在光谱感知的方式下引导潜在表示，最小化增强视图的熵。

Result: STS在标准评估协议下大幅超越或与最先进的测试时适应方法相当，同时仅引入少量额外参数，推理速度提升8倍，内存占用减少12倍。

Conclusion: STS提供了一种高效轻量的测试时适应方法，无需修改模型架构或进行昂贵的反向传播，在保持性能的同时显著提升了效率。
...
Method: 基于几何基础骨干网络重建正常光照的3D高斯表示，采用跨光照蒸馏方案和专门的Lumos损失来促进光度一致性。

Result: 在真实世界数据集上实现高保真低光照3D场景恢复，具有准确几何和强泛化能力，并可扩展到过曝校正。

Conclusion: Lumos3D提供了一个高效通用的3D低光照恢复解决方案，无需场景特定训练，具有强泛化性和多功能性。
...
TL;DR: 提出了一种无需训练的跨视角图像检索框架，结合预训练视觉编码器和大型语言模型，通过地理线索提取和卫星图像查询实现街景到卫星图像的匹配。


<details>
  <summary>Details</summary>
Motivation: 现有的跨视角图像检索方法需要监督训练和特定数据集，限制了实际部署。本文旨在开发一种无需额外训练、能处理单目街景图像的检索方法。

Method: 使用预训练视觉编码器（如DINOv2）和LLM，通过基于网络的图像搜索提取地理线索，利用地理编码API生成卫星查询，并通过PCA白化特征精化进行匹配检索。
...
<details>
  <summary>Details</summary>
Motivation: 3DGS在新型视图合成方面取得了最先进的光照真实感结果，但在人-场景动画和交互方面仍未被充分探索。现有动画流水线使用网格或点云作为底层3D表示，存在局限性。

Method: 使用3DGS作为3D表示，将渲染与运动合成解耦。核心包括高斯对齐运动模块（使用不透明度线索和投影高斯结构指导人体放置和姿态对齐）和人-场景高斯细化优化（确保真实接触和导航）。

Result: 在Scannet++和SuperSplat库的场景上进行了评估，使用稀疏和密集多视角人体捕捉重建的虚拟角色。实现了几何一致的自由视点渲染和自然的人-场景交互。

Conclusion: 该框架展示了3DGS在单目视频人体动画方面的独特优势，能够实现编辑的单目RGB视频与新的动画化人类角色的几何一致自由视点渲染。
...
<details>
  <summary>Details</summary>
Motivation: 对抗性补丁攻击通过在图像中注入局部扰动来误导深度视觉模型，这些攻击可在物理世界中部署，对实际应用构成严重威胁。现有最先进方法PatchCleanser需要两轮掩码且计算复杂度高。
...
Method: 将太阳动力学天文台图像预训练的基础模型嵌入与卫星位置、傅里叶特征编码的太阳磁连接信息结合，构建神经网络场模型进行微调。

Result: 整体分类性能一般，可能由于标签粗糙、类别不平衡和预训练模型迁移性有限，但证明了基础模型嵌入用于太阳风任务的可行性。
...
TL;DR: IPCD-Net是一种用于点云本征分解的方法，直接将彩色点云分解为反照率和阴影，解决了点云非网格结构和全局光照方向的挑战。
...
Method: 提出IPCD-Net，通过逐点特征聚合处理非网格数据；引入基于投影的亮度分布和分层特征细化，通过多视图投影捕捉全局光照信息。

Result: 实验结果表明IPCD-Net能有效减少反照率中的投射阴影，提高阴影中的颜色准确性，并在纹理编辑、重光照和点云配准等应用中表现良好。

Conclusion: IPCD-Net成功实现了点云的本征分解，验证了其在真实世界中的适用性，为点云处理提供了新的解决方案。
...
TL;DR: 提出了T-DRS方法解决LVLM中ROPE导致的远程依赖建模问题，包含三个推理阶段策略来增强长距离语义信号，无需训练即可提升VQA性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在使用ROPE时面临远程依赖建模挑战，ROPE虽然能精确建模token位置，但会导致随着token距离增加而注意力衰减，损害模型记忆全局上下文的能力。

Method: 提出推理阶段的三步衰减恢复策略：语义驱动DRS通过内容感知残差放大语义重要但遥远的信号；距离感知控制DRS基于位置距离平滑调节权重来净化注意力；重新强化远程DRS巩固剩余信息性远程依赖以保持全局连贯性。
...
Result: 在三个RGB-D VSOD数据集上的广泛实验表明，SAM-DAQ在所有评估指标上持续优于最先进的方法。

Conclusion: SAM-DAQ成功将SAM2适配到RGB-D视频显著目标检测，通过深度和时序线索的集成解决了现有方法的局限性。
...
TL;DR: 提出RWKV-PCSSC，一种轻量级的点云语义场景补全网络，通过RWKV机制减少参数数量并提高内存效率，在多个数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有语义场景补全方法通常采用密集网络架构，参数数量多，导致模型复杂度和资源需求高。需要设计更轻量高效的解决方案。

Method: 使用RWKV种子生成器模块从部分点云聚合特征生成粗粒度点云，然后通过多阶段RWKV点反卷积模块逐步恢复点云特征。

Result: 相比最先进方法PointSSC，参数数量减少4.18倍，内存效率提高1.37倍，在室内外多个数据集上达到最先进性能。

Conclusion: RWKV-PCSSC通过紧凑高效的设计实现了轻量级模型表示，在保持高性能的同时显著降低了模型复杂度和资源需求。
...
TL;DR: HCC-3D提出了一种分层补偿压缩方法，通过全局结构压缩和自适应细节挖掘模块，在保持关键细节的同时将3D令牌压缩约98%，显著提升了3D视觉语言模型的效率和性能。


<details>
  <summary>Details</summary>
Motivation: 当前3D-VLMs直接将点云嵌入为3D令牌，但在LLM部分处理所有3D令牌会产生巨大计算成本，限制了实际应用。需要找到在减少计算开销的同时保持关键信息完整性的方法。

Method: 提出分层补偿压缩(HCC-3D)：1) 全局结构压缩(GSC)：使用全局查询将所有3D令牌压缩为少量关键令牌；2) 自适应细节挖掘(ADM)：通过互补评分选择性重新压缩显著但未被充分关注的特征。

Result: 实验表明HCC-3D实现了约98%的压缩率，相比之前的3D-VLMs在效率和性能上都有显著提升，达到了新的最先进水平。

Conclusion: HCC-3D通过高效压缩3D令牌同时保持关键细节，成功解决了3D-VLMs的计算瓶颈问题，在计算效率和模型性能方面都取得了重大改进。
...
<details>
  <summary>Details</summary>
Motivation: 现代检测器在航拍图像中的小目标检测表现不佳，主要因为小目标特征有限且在网络传播中易丢失，同时小目标在训练中受到不成比例的回归惩罚。

Method: 提出尺度感知中继层(SARL)和尺度自适应损失(SAL)。SARL使用跨尺度空间-通道注意力逐步丰富每层特征并加强跨层特征共享；SAL重塑基于IoU的损失函数，动态为较大目标分配较低权重。

Result: 在AI-TOD、DOTA-v2.0和VisDrone2019三个基准数据集上，该方法在YOLOv5和YOLOx基准上提升5.5%的平均精度，在真实噪声数据集AI-TOD-v2.0上达到29.0% AP。

Conclusion: 所提出的SARL和SAL方法能有效提升航拍图像中小目标检测的泛化能力和鲁棒性，与主流检测框架兼容性好。
...
TL;DR: 提出了一种基于Swin-BART编码器-解码器的医疗图像自动描述系统，通过轻量级区域注意力模块增强诊断关键区域，在ROCO数据集上实现最先进的语义保真度。
...
Result: 在ROCO数据集上显著优于基线模型：ROUGE（0.603 vs 0.356/0.255）、BERTScore（0.807 vs 0.645/0.623），在BLEU、CIDEr和METEOR指标上具有竞争力。

Conclusion: 该设计能够生成准确、临床化的描述，并提供透明的区域归因，支持在有人类参与的安全研究使用。
...
<details>
  <summary>Details</summary>
Motivation: 现有单域广义目标检测方法依赖离散数据增强或静态扰动来扩展数据多样性，但无法有效捕捉现实场景中连续渐变的域偏移特征分布动态变化。
...
TL;DR: 提出了DocWeaver多智能体流水线，自动生成大规模双语文档理解基准MosaicDoc，包含7.2万张图像和60万QA对，用于评估视觉语言模型在复杂文档理解任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要是英语中心、布局简单、任务有限，无法充分评估模型在视觉丰富文档理解(VRDU)中的能力，VRDU涉及复杂布局和密集文本。

Method: 使用DocWeaver多智能体流水线，利用大语言模型自动生成新基准MosaicDoc，该基准从报纸杂志获取，具有多样复杂布局、丰富样式和全面多任务标注。

Result: 创建了包含7.2万图像和60万QA对的大规模双语基准，评估显示现有最先进模型在处理真实世界文档复杂性方面存在明显局限性。

Conclusion: MosaicDoc为VRDU领域提供了权威基准，揭示了当前模型的不足，并为未来研究指明了方向。
...
Method: 提出线性SLDC（通过正则化最小二乘学习线性转换算子）和弱非线性SLDC（在纯线性和完全非线性变换间平衡），并结合知识蒸馏减少表示漂移。

Result: 在标准CIL基准测试中，SLDC显著提升了SeqFT性能，结合KD后达到与联合训练相当的表现。

Conclusion: SLDC通过补偿分布漂移有效解决了序列微调的关键问题，使SeqFT在类增量学习中达到接近最优性能。
...
TL;DR: 提出了一种针对行人重识别任务的对抗防御框架，通过数据平衡和双对抗自元防御来解决模型偏见和复合泛化需求两大挑战。
...
Method: 提出去偏双重不变防御框架，包括基于扩散模型的数据重采样策略来缓解模型偏见，以及包含最远负样本扩展软化的度量对抗训练方法和对抗增强的自元机制。

Result: 实验表明该方法显著优于现有的最先进防御方法。

Conclusion: 该框架有效解决了行人重识别对抗防御中的模型偏见和复合泛化需求问题，为度量学习任务的对抗鲁棒性提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统视觉图神经网络(ViGs)在图构建阶段面临的计算效率问题，平衡准确性和计算效率。

Method: 采用自适应图卷积机制，包括高效的静态轴向支架和基于特征相似性的动态门控策略(指数衰减门控)，以及混合策略：早期使用高效门控，最后阶段使用全局注意力块。

Result: AdaptViG-M达到82.6% top-1准确率，比ViG-B高0.3%，同时减少80%参数和84%GMACs；在下游任务中性能优于EfficientFormer-L7，参数减少78%。

Conclusion: AdaptViG在视觉图神经网络中实现了准确性和效率的最佳平衡，为高效视觉架构提供了新方向。
...
TL;DR: TSPE-GS通过均匀采样透射率建模像素级多模态不透明度和深度分布，解决3D高斯溅射在半透明表面重建中的问题，无需额外训练即可提升半透明几何重建质量。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在速度-质量权衡方面表现良好，但假设每个像素单一深度的前提在半透明表面重建中失效，因为多个表面可能同时可见。

Method: 提出TSPE-GS方法，均匀采样透射率建模像素级多模态分布，逐步融合截断符号距离函数，在统一框架内分别重建外部和内部表面。

Result: 在公开和自采集的半透明及不透明数据集上的广泛实验表明，TSPE-GS显著改善了半透明几何重建，同时保持对不透明场景的性能。

Conclusion: TSPE-GS通过多模态分布建模有效解决了半透明表面重建中的深度模糊问题，且可泛化到其他基于高斯的重建流程中。
...
TL;DR: 提出了一种自适应融合框架，将CLIP图像特征的幅度信息与余弦相似度结合，用于无参考图像质量评估，无需任务特定训练即可超越现有方法。
...
Method: 提取CLIP图像特征的绝对值，应用Box-Cox变换进行统计归一化以减轻语义敏感性，然后设计置信度引导的融合方案自适应地加权余弦相似度和幅度感知质量线索。

Result: 在多个基准IQA数据集上的广泛实验表明，该方法始终优于标准的基于CLIP的IQA和最先进的基线方法。

Conclusion: CLIP图像特征的幅度信息是感知质量的重要线索，通过自适应融合语义相似度和幅度感知线索，可以在无需任务特定训练的情况下显著提升NR-IQA性能。
...
TL;DR: 提出一种利用视觉语言模型自动生成伪标签来训练高效实时目标检测器的新方法，通过基于每对象协同教学策略减少VLM生成标签中的噪声。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在零样本目标检测方面具有潜力，但存在检测延迟高和幻觉预测问题，无法直接部署。手动标注成本高昂，需要自动化解决方案。

Method: 使用VLM自动生成伪标签，提出每对象协同教学训练策略，两个YOLO模型协作学习，基于对等模型的每对象损失值过滤不可靠边界框。

Result: 在KITTI数据集上，方法显著提升mAP@0.5从31.12%到46.61%，保持实时检测延迟。添加10%真实标签后进一步提升至57.97%。在ACDC和BDD100k数据集上也观察到类似性能提升。
...
TL;DR: 提出了EquS和EquS+方法，通过双采样轨迹施加等变信息，并引入时间步感知调度来提升扩散模型图像恢复的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 现有问题无关的扩散模型图像恢复方法未能充分利用扩散先验，导致性能不佳。

Method: 使用双采样轨迹施加等变信息的EquS方法，以及引入时间步感知调度(TAS)的EquS+方法，优先确定性步骤以增强确定性和采样效率。

Result: 在基准测试上显著提升了现有问题无关DMIR方法的性能，且不增加计算成本。

Conclusion: 该方法与现有问题无关DMIR方法兼容，能显著提升性能而不增加计算开销。
...
TL;DR: 提出DiVE方法，通过约束预训练模型和微调模型嵌入向量的差异向量来保持几何结构，从而在ID数据上稳健微调视觉语言模型而不损害OOD和零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于对比学习的稳健微调方法会扭曲嵌入的几何结构，这损害了视觉语言模型在OOD和零样本设置下的泛化能力。

Method: 提出差异向量均衡化(DiVE)，通过平均向量损失(AVL)和成对向量损失(PVL)来约束差异向量，分别从全局和局部保持几何结构。

Result: 实验表明DiVE有效保持了几何结构，在ID、OOD和零样本指标上都取得了强劲结果。

Conclusion: DiVE方法通过保持嵌入几何结构，成功实现了在不损害泛化能力的前提下对视觉语言模型的稳健微调。
...
<details>
  <summary>Details</summary>
Motivation: 解决当前场景文本编辑方法在低资源语言支持、合成与真实数据之间的领域差距以及缺乏合适的文本样式保持评估指标等方面的局限性。
...
Conclusion: STELLAR能够可靠地进行多语言编辑，通过提出的TAS指标实现了对文本样式保持的稳健评估，为低资源语言的场景文本编辑提供了有效解决方案。
...
TL;DR: MOBA是一种针对LiDAR 3D物体检测系统的物理可实现后门攻击框架，通过显式建模真实世界触发器的材质特性来弥合数字-物理领域差距，在先进模型上达到93.50%的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有后门攻击缺乏物理可实现性，数字触发器因忽略材质相关的LiDAR反射特性而无法在真实环境中工作，而物理构造的触发器往往未经优化，导致低效或易被检测。

Method: 1) 系统选择鲁棒触发材质（二氧化钛TiO₂）；2) 开发新型仿真流水线：使用角度无关的Oren-Nayar BRDF模型近似生成真实LiDAR强度，以及距离感知缩放机制保持空间一致性。

Result: 在先进的LiDAR和Camera-LiDAR融合模型上进行广泛实验，MOBA达到93.50%的攻击成功率，比先前方法提高超过41%。

Conclusion: MOBA揭示了一类新的物理可实现威胁，强调了在真实环境中考虑材质级属性的防御措施的迫切需求。
...
TL;DR: DBGroup是一个两阶段弱监督3D实例分割框架，使用场景级标注替代传统标注方式，通过双分支点分组和自训练方法实现高效分割
...
Result: 在实验中达到与稀疏点级监督方法相当的性能，并超越最先进的场景级监督3D语义分割方法

Conclusion: DBGroup证明了场景级标注在3D实例分割中的有效性，为减少标注成本提供了可行方案
...
<details>
  <summary>Details</summary>
Motivation: 现有Vision Transformer量化方法采用统一精度，忽略了不同组件对量化的敏感度差异，而现有的混合精度量化方法存在粒度粗、度量尺度不匹配和量化无关的位分配问题。

Method: LampQ采用层级量化实现细粒度控制和高效加速，使用类型感知的Fisher度量评估敏感度，通过整数线性规划最优分配位宽，并进行迭代更新。

Result: 在图像分类、目标检测和零样本量化等多种任务上，LampQ在量化预训练Vision Transformer时提供了最先进的性能。

Conclusion: LampQ通过层级的混合精度量化方法，有效克服了现有Vision Transformer量化方法的局限性，在各种任务上都取得了优异的量化效果。
...
TL;DR: MIRNet是一个用于医学图像分析的新框架，结合自监督预训练和图推理，在舌诊领域取得SOTA性能，并构建了最大的公开舌像数据集TongueAtlas-4K。
...
Method: 使用自监督掩码自编码器(MAE)从未标注数据学习可迁移视觉表示；采用图注意力网络(GAT)通过专家定义的结构图建模标签相关性；通过KL散度和正则化损失的约束感知优化强制执行临床先验；使用非对称损失(ASL)和提升集成缓解不平衡问题。

Result: 方法在舌诊任务上实现了最先进的性能，同时构建了包含4,000张图像和22个诊断标签的TongueAtlas-4K数据集，这是舌分析领域最大的公开数据集。

Conclusion: MIRNet框架在舌诊领域表现出色，同时能够泛化到更广泛的诊断医学成像任务中。
...
TL;DR: 提出了细粒度3D具身推理任务，要求代理根据任务指令预测3D场景中每个可操作元素的空间位置、运动类型和运动轴，并开发了AffordBot框架，结合多模态大语言模型和链式思维推理来解决该任务。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常在对象级别操作或分离处理细粒度可操作性推理，缺乏连贯的指令驱动的基础和推理能力，无法有效支持物理环境中的人机协作。

Method: AffordBot框架将多模态大语言模型与定制的链式思维推理范式相结合，通过渲染场景的环绕视图图像并将3D元素候选投影到这些视图中，形成与场景几何对齐的丰富视觉表示。推理流程包括主动感知阶段和逐步推理阶段。

Result: 在SceneFun3D数据集上的评估表明，AffordBot实现了最先进的性能，仅使用3D点云输入和多模态大语言模型就展示了强大的泛化能力和物理基础推理能力。

Conclusion: 该工作成功解决了细粒度3D具身推理任务，为物理环境中的人机协作提供了有效的解决方案，证明了多模态大语言模型在3D场景理解和推理方面的潜力。
...
TL;DR: Anomagic是一种零样本异常生成方法，无需异常样本即可生成语义一致的异常。通过跨模态提示编码整合视觉和文本线索，利用上下文信息指导基于修复的生成流程，并通过对比精炼策略增强异常与掩码的对齐，提升异常检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有异常生成方法通常需要异常样本作为参考，限制了在零样本场景下的应用。作者旨在开发一种无需异常样本即可生成语义一致异常的方法，以解决异常检测中数据稀缺的问题。

Method: 提出Anomagic方法：1）跨模态提示编码方案统一视觉和文本线索；2）基于修复的生成流程利用上下文信息；3）对比精炼策略强制合成异常与掩码的精确对齐；4）构建AnomVerse数据集（12,987个异常-掩码-描述三元组）用于训练。

Result: 实验表明，Anomagic在AnomVerse上训练后，能够生成比现有方法更真实、更多样化的异常，显著提升了下游异常检测任务的性能。该方法还能通过用户定义提示为任何正常类别图像生成异常。

Conclusion: Anomagic建立了一个通用的异常生成基础模型，能够零样本生成语义一致的异常，为异常检测任务提供了有效的解决方案，并展现了在任意图像上生成异常的灵活性。
...
TL;DR: 提出DGFusion双引导多模态3D目标检测方法，通过难度感知实例配对解决远距离、小目标和遮挡目标检测难题
...
Method: 基于双引导范式，结合点云引导图像和图像引导点云，使用难度感知实例配对器生成难易实例对，通过双引导模块实现多模态特征融合

Result: 在nuScenes数据集上比基线方法提升1.0% mAP、0.8% NDS和1.3%平均召回率，在距离、尺寸、可见性和小规模训练场景下对硬实例检测具有鲁棒性

Conclusion: DGFusion通过双引导范式和难度感知实例配对有效解决了硬实例检测问题，提升了自动驾驶感知系统的安全性
...
Result: 在重建精度和生成质量方面达到最先进性能，实现优异的表面平滑度和几何灵活性，支持前所未有的2048^3超高分辨率。

Conclusion: 基于UDF的LoG-VAE框架为复杂拓扑结构的3D内容生成提供了鲁棒高效的解决方案，突破了传统方法的分辨率限制。
...
TL;DR: 提出了一种用于可见光-红外目标检测的频率域融合变换器FreDFT，通过频率域注意力机制和混合尺度频率特征融合来解决模态间信息不平衡问题，在多个数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有可见光-红外目标检测方法存在模态间信息不平衡问题，导致跨模态融合不充分，检测性能下降。同时，大多数方法在空间域使用变换器，忽略了在频率域挖掘互补信息的优势。

Method: 提出FreDFT方法，包含：1）多模态频率域注意力(MFDA)挖掘模态间互补信息；2）频率域前馈层(FDFFL)通过混合尺度频率特征融合增强多模态特征；3）跨模态全局建模模块(CGMM)在空间和通道维度进行像素级跨模态特征交互；4）局部特征增强模块(LFEM)使用多种卷积层和通道混洗来增强局部特征表示。
...
TL;DR: MuSc-V2是一个零样本异常分类和分割框架，利用正常图像块在2D外观和3D形状上的相似性，通过相互评分机制和多模态融合来检测异常。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽略了正常图像块在工业产品中通常能找到许多相似块（不仅在2D外观上，还在3D形状上），而异常则保持多样化和孤立的关键特性。

Method: 提出MuSc-V2框架，包括迭代点分组改进3D表示、相似性邻域聚合融合2D/3D特征、相互评分机制让样本相互评分、跨模态异常增强融合2D和3D分数、以及约束邻域重评分抑制误分类。

Result: 在MVTec 3D-AD数据集上AP提升23.7%，在Eyecandies数据集上提升19.3%，超越了之前的零样本基准，甚至优于大多数少样本方法。

Conclusion: MuSc-V2框架能够灵活处理完整数据集和较小子集，在不同产品线上具有一致的鲁棒性能，为工业异常检测提供了有效的零样本解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前图像筛选研究稀缺，多模态大语言模型在图像美学推理方面表现不佳，主要由于缺乏数据和模型推理能力弱。

Method: 收集了包含128k样本的全面数据集，评估四个美学方面；提出了HCM-GRPO方法，在GRPO框架中引入困难案例挖掘策略和动态比例准确率奖励。

Result: 实验表明，即使是GPT4o和Qwen-VL-Max等最先进的闭源模型在图像美学推理上也表现类似随机猜测，而使用HCM-GRPO的小模型能够超越开源和闭源大模型。

Conclusion: 通过构建高质量数据集和开发HCM-GRPO方法，显著提升了多模态大语言模型的图像美学推理能力，证明了小模型也能在特定任务上超越大模型。
...
TL;DR: 提出了AV-ConfuseBench基准测试，发现MLLMs在音频缺失的视听混淆场景中存在视觉主导推理问题，并提出RL-CoMM方法通过强化学习协作多模态模型来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 研究多模态大语言模型能否识别视觉存在但音频缺失的混淆对象，即"视听混淆"场景中的音频判别能力。

Method: 提出RL-CoMM方法：1）引入大型音频语言模型作为参考模型生成纯音频推理；2）设计逐步推理奖励函数让MLLMs自我改进视听推理；3）引入答案中心置信度优化减少异构推理差异的不确定性。

Result: 在视听问答和视听幻觉任务上，RL-CoMM相比基线模型在有限训练数据下准确率提升10~30%。

Conclusion: MLLMs在视听混淆场景中存在视觉主导推理问题，RL-CoMM通过强化学习协作多模态模型有效提升了音频缺失场景下的判别能力。
...
TL;DR: 提出GaussMedAct框架，通过多元高斯编码和混合空间编码，在医疗动作评估中实现92.1%的Top-1准确率，比ST-GCN基线提升5.9%，且仅需10%的计算量。
...
Conclusion: GaussMedAct框架通过自适应时空表示学习显著提升了医疗动作分析的性能，在精度和效率方面都表现出色。
...
TL;DR: CABIN是一个半监督学习框架，通过感知-行动-校正的闭环过程解决高光谱图像分类中置信度误导的问题。它估计认知不确定性识别模糊区域，采用不确定性引导的双重采样策略，并通过细粒度动态分配策略校正噪声监督。


<details>
  <summary>Details</summary>
Motivation: 在高光谱图像分类中，仅依赖置信度会产生误导，模型容易将高预测分数误认为正确性而缺乏不确定性意识。这导致确认偏误，特别是在稀疏标注或类别不平衡的情况下，模型会过度拟合自信的错误并无法泛化。

Method: CABIN框架包含三个核心组件：1）感知意识：估计认知不确定性识别可能出错的模糊区域；2）行动策略：采用不确定性引导的双重采样策略，选择不确定样本进行探索，同时将自信样本作为稳定伪标签以减少偏误；3）校正机制：引入细粒度动态分配策略，将伪标签数据分为可靠、模糊和噪声子集，并应用定制损失来增强泛化能力。

Result: 实验结果表明，多种最先进的方法在集成CABIN后都受益，提高了标注效率和性能。

Conclusion: CABIN通过认知感知的行为驱动学习，有效解决了高光谱图像分类中置信度误导的问题，提升了模型在稀疏标注和类别不平衡情况下的泛化能力。
...
TL;DR: VLF-MSC提出了一种基于视觉语言特征的多模态语义通信系统，通过传输单一的紧凑视觉语言表示来同时支持图像和文本生成，提高了频谱效率和对信道噪声的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的语义通信技术通常单独处理每种模态，需要特定模态的流或重传，导致频谱效率低下。本文旨在开发一个统一的系统，通过单一表示同时支持图像和文本生成。
...
TL;DR: GlobalDiff是一个基于扩散模型的框架，首次直接在全局关节旋转空间中进行语音驱动运动生成，通过多级约束方案解决层次误差累积问题，在标准基准测试中性能提升46.0%。


<details>
  <summary>Details</summary>
Motivation: 现有生成方法通常在局部关节旋转上操作，这些旋转基于骨架结构分层定义，导致生成过程中累积误差，表现为末端效应器的不稳定和不合理运动。

Method: 提出GlobalDiff扩散框架，直接在全局关节旋转空间操作；引入多级约束方案：关节结构约束使用虚拟锚点捕捉细粒度方向，骨架结构约束保持骨骼间角度一致性，时间结构约束使用多尺度变分编码器对齐生成运动与真实时间模式。

Result: 在标准语音驱动运动基准测试中，GlobalDiff生成平滑准确的运动，在多个说话者身份下相比当前最优方法性能提升46.0%。

Conclusion: 直接在全局旋转空间操作并引入多级约束，能有效解决层次误差累积问题，显著提升语音驱动运动生成的质量和稳定性。
...
TL;DR: SUGAR是一个结合LLMs和人体骨架进行动作分类和描述的新范式，通过视觉-运动知识监督骨架学习，生成离散表示供LLM理解。


<details>
  <summary>Details</summary>
Motivation: 探索如何让LLMs理解骨架数据并区分不同动作，解决LLMs作为识别器时的骨架理解和动作区分问题。

Method: 使用大规模视频模型生成视觉和运动信息作为先验知识，监督骨架学习生成离散表示，通过Temporal Query Projection模块建模长序列骨架信号，最后用预训练LLM理解这些表示。

Result: 在多个骨架动作分类基准测试中表现有效，在零样本场景下比基于线性方法更具通用性。

Conclusion: SUGAR成功地将LLMs与骨架数据结合，为动作识别提供了新的有效方法，特别是在零样本场景下表现出色。
...
TL;DR: MTAttack是首个针对大型视觉语言模型的多目标后门攻击框架，通过在潜在空间中联合优化多个触发器，实现准确的多个触发-目标映射，显著提升了攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有后门攻击主要针对单目标攻击，而多目标后门攻击在现实应用中威胁更大，但由于不同触发器间的特征干扰，实现准确的多触发-目标映射具有挑战性。

Method: 提出MTAttack框架，包含代理空间划分约束和触发器原型锚定约束两种优化方法，在潜在空间中联合优化多个触发器，确保每个触发器独立地将干净图像映射到唯一的代理类别并保持可分离性。
...
TL;DR: 提出了RobIA框架，用于立体深度估计中的持续测试时适应，通过动态专家路由和鲁棒教师模型解决领域偏移问题


<details>
  <summary>Details</summary>
Motivation: 真实环境中的立体深度估计面临动态领域偏移、稀疏监督和高成本标注等挑战，现有TTA方法假设静态目标域且采用输入不变策略，限制了在持续变化环境中的有效性

Method: RobIA框架包含两个核心组件：(1) Attend-and-Excite Mixture-of-Experts(AttEx-MoE)，通过轻量级自注意力机制动态路由输入到冻结专家；(2) Robust AdaptBN Teacher，基于PEFT的教师模型提供密集伪监督，补充稀疏手工标注

Result: 大量实验表明，RobIA在动态目标域上实现了优越的自适应性能，同时保持了计算效率

Conclusion: RobIA通过实例感知的灵活适应策略和广泛的监督覆盖，在领域偏移下提高了泛化能力，为立体深度估计的持续测试时适应提供了有效解决方案
...
TL;DR: 提出CACMI框架，通过跨模态帧聚合和上下文感知特征增强，显式建模视频事件序列的时序连贯性和视觉上下文语义，在密集视频描述任务中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖帧级或碎片化视频特征的隐式建模，无法捕捉事件序列的时序连贯性和视觉上下文的全面语义。
...
Conclusion: 提出的显式时序-语义建模框架CACMI能够有效捕捉视频事件序列的时序连贯性和视觉上下文语义，显著提升密集视频描述性能。
...
<details>
  <summary>Details</summary>
Motivation: 调查当前领先文本到图像模型在逻辑组合方面的根本缺陷，揭示模型在单个原语上表现良好但在组合时性能崩溃的问题。

Method: 通过分析三个核心逻辑原语（否定、计数、空间关系）的组合表现，追踪失败原因到训练数据缺乏显式否定、连续注意力架构不适合离散逻辑、评估指标偏好视觉合理性而非约束满足。

Result: 模型在单个原语上准确但在组合时性能急剧下降，暴露严重干扰问题，现有解决方案和简单扩展无法弥合这一差距。
...
TL;DR: 提出split-layer方法，通过将MLP层分成多个并行分支并用Hadamard积整合输出，构建高维多项式空间，显著提升隐式神经表示(INR)的表征能力，同时避免计算开销的二次增长。


<details>
  <summary>Details</summary>
Motivation: 传统MLP架构中低维特征空间限制了INR的表征能力，而加宽MLP会导致计算和内存成本的二次增长，需要一种既能提升表征能力又避免过高计算开销的方法。

Method: 提出split-layer重构MLP结构，将每层分成多个并行分支，通过Hadamard乘积整合分支输出，构建高维多项式空间来增强特征空间维度。

Result: 在2D图像拟合、2D CT重建、3D形状表示和5D新视角合成等多个任务中，split-layer显著提升了INR性能，超越了现有方法。

Conclusion: split-layer通过构建高维多项式空间有效提升了INR的表征能力，在多个任务中表现出优越性能，同时避免了计算开销的二次增长。
...
TL;DR: 提出了一种双机制协作优化框架，通过结构公平解耦和全局分布对齐，在保持检测精度的同时提升深度伪造检测模型的公平性。


<details>
  <summary>Details</summary>
Motivation: 解决深度伪造检测模型在不同人口统计群体（如性别、种族）上的偏见问题，避免系统性误判加剧数字鸿沟和社会不平等。当前公平性增强检测器往往以牺牲检测精度为代价。
...
Conclusion: 提出的双机制协作优化框架能够有效平衡检测精度和公平性，为可信赖的深度伪造检测模型部署提供了解决方案。
...
TL;DR: 本文提出了一种基于生成视角的生成增强对齐方法GEA，通过扩散生成图像作为中间语义表示来弥合文本与视觉模式之间的差距，提升文本到图像人物检索的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像人物检索方法存在文本查询无法准确反映图像内容、跨模态对齐困难以及过拟合等问题，文本与图像之间的模态差距进一步加剧了这些挑战。

Method: GEA包含两个并行模块：文本引导标记增强(TGTE)使用扩散生成图像作为中间语义表示来丰富文本语义；生成中间融合(GIF)结合生成图像、原始图像和文本特征的交叉注意力生成统一表示，并通过三元组对齐损失优化。

Result: 在CUHK-PEDES、RSTPReid和ICFG-PEDES三个公开数据集上的实验结果表明，该方法具有有效性。

Conclusion: 从生成视角提出的GEA方法能够有效解决文本到图像人物检索中的跨模态对齐问题，通过生成中间语义表示弥合模态差距。
...
Result: InterIR在多退化恢复中表现优异，同时在单退化任务中保持高度竞争力。
...
TL;DR: 提出CephRes-MHNet多头残差卷积网络，用于从2D侧位头颅X光片中准确定位头影测量标志点，在Aariz数据集上达到1.23mm平均径向误差和85.5%的检测成功率。
...
Method: 集成残差编码、双重注意力机制和多头解码器的卷积网络架构，增强上下文推理和解剖精度。

Result: 在1000张X光片数据集上，平均径向误差1.23mm，2.0mm阈值下检测成功率85.5%，优于所有基线模型，参数量仅为最强基线的25%。
...
TL;DR: 评估NASA和IBM的Prithvi-EO-2.0地理空间基础模型在小沙岛海岸线划定中的应用，使用少量训练数据即可获得高精度结果。


<details>
  <summary>Details</summary>
Motivation: 探索Prithvi地理空间基础模型在海岸线监测中的迁移学习能力，特别是在数据稀缺地区支持海岸监测的潜力。

Method: 收集并标注了225张马尔代夫岛屿多光谱卫星图像数据集，对Prithvi模型的300M和600M参数版本进行微调，训练集规模从5到181张图像不等。
...
TL;DR: VISTA是一个基于递归目标条件Transformer的多智能体轨迹预测模型，通过结合长期意图与历史运动、灵活的社会交互建模以及可解释的社交影响模式，显著提高了预测精度并大幅减少了轨迹碰撞。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时捕捉智能体的长期目标和细粒度社会交互，导致预测的多智能体轨迹不真实。需要开发能够联合建模长期意图和社会互动的预测框架。

Method: 提出VISTA模型，包含：(1)交叉注意力融合模块整合长期意图与历史运动；(2)社交令牌注意力机制实现灵活的跨智能体交互建模；(3)成对注意力图在推理时提供可解释的社交影响模式。将单智能体目标条件预测转化为连贯的多智能体预测框架。

Result: 在高密度MADRAS基准和SDD数据集上，VISTA实现了最先进的精度，并大幅减少碰撞率。在MADRAS上将平均碰撞率从2.14%降至0.03%，在SDD上实现零碰撞，同时改善了ADE、FDE和minFDE指标。
...
TL;DR: 提出LiNeXt——一种轻量级的非扩散网络，用于快速准确的点云补全，相比扩散模型实现了199.8倍加速和50.7%的Chamfer距离减少。


<details>
  <summary>Details</summary>
Motivation: 解决现有扩散模型在3D LiDAR场景补全中多步迭代采样带来的计算开销问题，提升实时性。

Method: 使用噪声到粗粒度模块单次去噪，避免多步迭代；采用距离感知选择重复策略生成均匀分布噪声点云；通过细化模块进行精确补全。

Result: 在SemanticKITTI数据集上，相比LiDiff实现199.8倍推理加速，Chamfer距离减少50.7%，参数量仅需6.1%。

Conclusion: LiNeXt在效率和效果上均优于扩散模型，适用于实时场景补全。
...
TL;DR: 提出了HeatV2X框架，通过异构适应方法解决V2X协作感知中的多模态异构性和可扩展性问题，在降低训练成本的同时提升感知性能。


<details>
  <summary>Details</summary>
Motivation: 现有V2X协作感知框架面临两个关键挑战：(1)参与代理本质上是多模态和异构的，(2)协作框架必须能够扩展以适应新代理。前者需要有效的跨代理特征对齐来减轻异构性损失，后者使得全参数训练不切实际。

Method: 首先基于异构图注意力训练高性能代理作为协作学习基础，然后设计局部异构微调和全局协作微调：局部使用Hetero-Aware适配器提取模态特定差异，全局使用Multi-Cognitive适配器增强跨代理协作和融合潜力。
...
TL;DR: 提出单阶段训练框架，通过跨模态和单模态的下一帧预测增强泛化能力，结合窗口级注意力机制检测局部伪影，实现强泛化性和精确时间定位。


<details>
  <summary>Details</summary>
Motivation: 现有多模态深度伪造检测方法需要预训练，主要关注音频-视觉不一致性，可能忽略单模态伪影，在保持音频-视觉对齐的伪造上表现不佳。

Method: 单阶段训练框架，包含跨模态和单模态的下一帧预测，使用窗口级注意力机制捕捉预测帧与实际帧之间的差异，检测每帧周围的局部伪影。

Result: 在多个基准数据集上评估，模型表现出强泛化能力和精确的时间定位性能。

Conclusion: 提出的方法能够有效检测完全伪造视频并准确定位部分伪造样本中的深度伪造片段，解决了现有方法的局限性。
...
Method: 使用预训练视觉定位模型生成文本条件候选管，通过管状条件重建和时空约束进行细化，设计了时间、空间和时空三个重建策略，并引入空间和时间提案之间的相互约束。
...
TL;DR: 本文构建了首个包含细粒度子分数和扣分标注的空中滑雪AQA数据集，并提出JudgeMind方法模拟专业裁判的评分思维，通过分阶段评分、阶段感知特征增强融合和基于知识的等级感知解码器来提升动作质量评估的性能和可靠性。
...
Conclusion: 通过构建细粒度标注数据集和模拟裁判评分思维的JudgeMind方法，显著提升了动作质量评估的性能和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 传统面部情感识别缺乏可解释性，现有视觉语言模型存在幻觉推理（生成看似合理但不准确的解释）和情感推理与识别不对齐的问题。

Method: 三阶段框架：1) 指令微调建立基本情感推理能力；2) 强化训练使用情感和动作单元标签作为奖励信号，对齐生成推理与预测情感；3) 数据合成管道迭代扩展训练数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖特征选择机制或区域提议策略来定位判别性区域，但这些方法往往无法全面捕捉判别性线索，同时引入大量类别无关的冗余。

Method: 提出语义感知聚合模块(SAAM)，利用多尺度上下文线索动态构建token间的加权超图，通过超图卷积捕获高阶语义依赖，将token特征逐步聚合成紧凑的区域级表示。同时引入双曲层次对比损失(HHCL)，在非欧几里得嵌入空间中强制执行层次语义约束。
...
Method: PROPA框架整合MCTS与GRPO生成过程级奖励，通过交错GRPO更新和SFT来解决冷启动问题，并训练过程奖励模型来指导推理时的搜索。

Result: 在7个基准测试和4个VLM骨干网络上，PROPA始终优于SFT和RLVR基线方法，在域内任务上提升达17.0%，在域外任务上提升达21.0%。

Conclusion: PROPA为视觉推理任务建立了强大的推理和泛化能力，是当前最先进的视觉语言模型推理优化方法。
...
TL;DR: RUDDER是一个低开销框架，通过单次前向传播提取视觉证据向量，并基于贝叶斯自适应门控进行令牌级校正，有效减少大型视觉语言模型的对象幻觉问题，同时保持计算效率。
...
TL;DR: 提出GRAM框架，通过混合专家架构和测试时自适应方法，解决卫星图像贫民窟分割中的区域泛化问题，无需目标区域标注数据即可实现鲁棒分割。


<details>
  <summary>Details</summary>
Motivation: 卫星贫民窟分割对全球城市贫困估算很重要，但非正式定居点的形态异质性导致模型难以泛化到未见区域，需要解决区域适应性问题。

Method: 构建百万级卫星图像数据集，采用混合专家架构学习区域特定特征和通用特征，通过测试时自适应利用预测一致性过滤不可靠伪标签。

Result: GRAM在非洲城市等低资源设置中优于现有方法，为全球贫民窟测绘提供可扩展且标签高效的解决方案。
...
TL;DR: LLaViT让大语言模型同时作为视觉编码器，通过三个关键改进解决LLaVA架构中视觉特征整合不足的问题，在多项基准测试中显著优于LLaVA基线方法。
...
Result: 在广泛的LLM上进行的控制实验表明，LLaViT在多项基准测试中显著优于LLaVA基线方法，甚至超过参数数量翻倍的模型。
...
TL;DR: 本文研究了不同参数规模的SmolVLM2模型（500M和2.2B）在盲人和低视力用户视频描述任务中的表现，提出了两个专门针对BLV可访问性评估的新框架，并在智能手机上测试了不同精度变体的实际性能。
...
Method: 在两个多样化数据集（AVCaps户外和Charades室内）上评估SmolVLM2变体；引入两个新评估框架：多上下文BLV框架和导航辅助框架；系统评估四种提示设计策略；在智能手机上部署模型并评估FP32和INT8精度变体。

Result: 通过系统评估揭示了模型规模对可访问性描述质量的影响，并测试了在资源受限移动设备上的实际性能约束。

Conclusion: 研究为开发更适合BLV用户需求的轻量级视觉语言模型提供了重要见解，特别是在移动设备部署方面的可行性评估。
...
TL;DR: 提出了基于图像分割的细粒度行人检测错误分类和评估指标，改进了现有性能基准的不足，在CityPersons数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前行人检测性能基准存在缺陷，无法对DNN进行真实性能评估。图像分割提供细粒度街道场景信息，可用于自动区分不同类型的检测错误。

Method: 提出8种不同的行人检测错误类别和相应评估指标，使用新指标比较不同骨干网络的简化APD版本。

Result: 实现了更细粒度和鲁棒的模型比较方法，特别是在安全关键性能方面。在CityPersons-reasonable数据集上达到SOTA（无需额外训练数据）。

Conclusion: 基于图像分割的错误分类方法为行人检测提供了更精细和安全的性能评估框架，简单架构也能取得优异性能。
...
TL;DR: 提出CLIP4VI-ReID网络，通过文本语义生成、红外特征嵌入和高层语义对齐三个模块，解决可见光-红外人员重识别任务中的模态差异问题。


<details>
  <summary>Details</summary>
Motivation: 可见光图像和红外图像在物理特性上存在巨大差异，导致跨模态人员重识别面临挑战。需要一种能够有效对齐两种模态特征的方法。

Method: 1) TSG模块为可见光图像生成文本语义；2) IFE模块利用文本语义校正红外图像特征嵌入；3) HSA模块优化高层语义对齐，确保文本语义仅包含身份相关信息。

Result: 在多个VI-ReID数据集上取得了优于现有最先进方法的性能。

Conclusion: 该方法通过文本作为桥梁，实现了可见光-红外模态的间接对齐，有效提升了跨模态人员重识别的性能。
...
TL;DR: 提出了一种结合景深监督和多视图一致性监督的3D高斯泼溅框架，解决了极端深度变化场景中的深度估计问题。
...
Method: 采用两个核心组件：1) 景深监督使用度量恢复的单目深度估计器生成深度先验，通过散焦卷积合成物理准确的散焦图像，并通过景深损失增强几何一致性；2) 多视图一致性监督使用LoFTR半稠密特征匹配最小化跨视图几何误差，并通过可靠匹配点的最小二乘优化强制深度一致性。

Result: 在Waymo Open Dataset上比最先进方法PSNR提高了0.8 dB，在远场和近场区域都实现了优异的深度保真度。
...
TL;DR: 提出DSANet网络，通过粗粒度和细粒度两个层面显式分离异常与正常特征，提升弱监督视频异常检测的分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法倾向于检测最显著响应片段，忽视挖掘与异常分离的多样正常模式，且因外观相似易产生类别混淆，导致细粒度分类结果不理想。

Method: 在粗粒度层面引入自引导正常性建模分支，通过学习正常原型重构输入视频特征；在细粒度层面提出解耦对比语义对齐机制，将视频分解为事件中心和背景中心组件，应用视觉-语言对比学习。

Result: 在XD-Violence和UCF-Crime两个标准基准测试上，DSANet超越了现有最先进方法。
...
Method: 使用vMF分布建模对象表示的方向特征，结合傅里叶变换进行幅度和相位扰动，在CLIP引导管道中增强特征多样性和结构一致性。

Result: 在多样化天气驾驶基准测试中，该方法超越了现有最先进方法。

Conclusion: 该方法不仅保留了CLIP的语义对齐优势，还通过vMF分布和傅里叶增强显著提升了跨域泛化能力。
...
TL;DR: DermAI是一个基于智能手机的轻量级应用，能够在常规咨询中实时捕获、注释和分类皮肤病变，通过设备端质量检查和本地模型适应解决AI皮肤病学中的偏见数据集和有限验证问题。


<details>
  <summary>Details</summary>
Motivation: AI皮肤病学的应用受到偏见数据集、图像质量变化和有限验证的限制，需要开发能够适应不同肤色、种族和来源设备的解决方案。

Method: 开发DermAI应用，在设备端进行质量检查，使用包含广泛肤色、种族和来源设备的临床数据集，并通过本地数据微调模型。

Result: 在初步实验中，基于公共数据集训练的模型无法泛化到DermAI样本，但使用本地数据微调后性能得到改善。
...
<details>
  <summary>Details</summary>
Motivation: 地理空间基础模型在预训练数据不足的环境中表现不可靠，需要开发能够识别和减轻这种不可靠性的方法。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界机器人导航需要开放词汇泛化和低训练开销，现有零样本方法构建显式3D场景图时会将丰富的视觉观察压缩为纯文本关系，导致构建成本高、视觉证据丢失和词汇受限。

Method: 引入M3DSG多模态3D场景图，用动态分配图像替换文本关系边；构建MSGNav系统，包含关键子图选择、自适应词汇更新、闭环推理模块；针对最后一英里问题提出基于可见度的视点决策模块。

Result: 在GOAT-Bench和HM3D-OVON数据集上实现了最先进的性能表现。

Conclusion: MSGNav通过多模态场景图和专门模块有效解决了零样本导航中的关键挑战，为现实世界机器人部署提供了可行的解决方案。
...
TL;DR: 现有防御方法（如Anti-DreamBooth）在防止AI个性化生成中的身份泄露方面存在两个关键缺陷：可感知的伪影和易被简单过滤器去除的脆弱性。


<details>
  <summary>Details</summary>
Motivation: DreamBooth等个性化AI应用存在面部身份泄露的隐私风险，现有防御机制存在明显不足，需要更有效的保护方案。

Method: 提出了AntiDB_Purify评估框架，系统评估现有防御方法在传统图像过滤器和对抗性净化威胁下的表现。

Result: 结果显示当前所有防御方法在净化威胁下都失去保护效果，无法有效防止身份泄露。

Conclusion: 当前防御方法提供虚假安全感，迫切需要开发更不可感知且鲁棒的保护机制来保障用户身份安全。
...
<details>
  <summary>Details</summary>
Motivation: 现实环境中的背景杂乱、光照变化和遮挡等问题对车道检测构成挑战，数据驱动方法需要大量数据收集和标注工作。需要利用上下文和全局信息来改进车道检测。

Method: 提出SAMIRO方法，结合空间注意力互信息正则化和预训练模型作为Oracle，具有即插即用特性，可集成到各种先进车道检测方法中。

Result: 在CULane、Tusimple和LLAMAS等主要基准测试上进行广泛实验，结果显示SAMIRO在不同模型和数据集上都能持续提升性能。

Conclusion: SAMIRO方法有效提升了车道检测性能，具有通用性和实用性，代码将在发表后公开。
...
TL;DR: 提出基于物理信息的Transformer-VAE架构，用于从Sentinel-2数据反演植被生物物理参数，仅使用模拟数据训练即可达到与使用真实图像训练的最先进方法相当的性能。
...
Method: 将PROSAIL辐射传输模型作为可微分物理解码器集成到Transformer-VAE架构中，确保推断的潜在变量对应物理上合理的叶片和冠层属性，完全使用模拟数据进行训练。

Result: 在真实世界数据集(FRM4Veg和BelSAR)上反演叶面积指数(LAI)和冠层叶绿素含量(CCC)，准确性与使用真实Sentinel-2数据训练的模型相当。

Conclusion: 该方法无需实地标签或真实图像校准，为全球植被监测提供了一种经济高效的自监督解决方案，展示了物理模型与先进深度网络结合如何改进RTM反演。
...
Conclusion: MonkeyOCR v1.5通过统一的视觉语言框架和创新的表格处理技术，有效解决了复杂文档解析的挑战，为文档智能应用提供了可靠的解决方案。
...
TL;DR: 提出结合YOLOMS目标检测与LLM的集成框架，用于风力涡轮机故障智能分析和诊断，提高故障检测准确性和维护决策支持能力。
...
Result: 在真实数据集上实现90.6%的故障检测准确率，维护报告平均准确率达到89%。
...
TL;DR: 提出GT++算法和3DFETUS深度学习模型，用于自动定位3D胎儿超声中的标准面部平面，解决传统方法因胎儿运动、方向变异和操作者依赖性导致的挑战。
...
Method: 1) GT++算法：通过标注的解剖标志从3D超声体积中估计标准面部平面；2) 3DFETUS深度学习模型：自动化和标准化3D胎儿超声体积中的平面定位。

Result: 平均平移误差4.13毫米，平均旋转误差7.93度/平面，优于其他最先进方法。临床评估证实两种方法的有效性，在平面估计精度方面显示出统计学显著改进。

Conclusion: GT++和3DFETUS方法能够有效解决3D胎儿超声中标准面部平面定位的挑战，提供更准确和一致的平面估计，具有临床实用价值。
...
Method: 使用TimeSformer架构的视频分类器，采用五折交叉验证和严格的主题分区策略防止数据泄漏。

Result: TimeSformer模型能够区分惊厥和正常活动，平均F1分数达到97%。

Conclusion: RodEpil数据集和基线代码已公开发布，为临床前癫痫研究的非侵入性视频监测提供了重要资源。
...
TL;DR: 提出了一种基于语义分割的组织学信息切片方法（HIT），通过提取腺体作为生物学意义输入补丁，提高了多实例学习模型的准确性和可解释性，同时简化了计算。


<details>
  <summary>Details</summary>
Motivation: 传统数字病理学流程使用网格切片方法忽略了组织结构，引入了不相关信息并限制了可解释性。

Method: 使用语义分割从全切片图像中提取腺体作为生物学意义输入补丁，用于多实例学习和表型分析。

Result: 在ProMPT队列的137个样本上训练，腺体级别Dice得分为0.83±0.17；从ICGC-C和TCGA-PRAD队列的760个WSI中提取38万个腺体，将检测上皮-间质转化和MYC相关基因拷贝数变异的多实例学习模型AUC提高了10%，并识别出15个腺体簇，其中多个与癌症复发、致癌突变和高Gleason评分相关。

Conclusion: HIT通过聚焦于生物学意义结构进行特征提取，提高了多实例学习预测的准确性和可解释性，同时简化了计算。
...
Result: 框架提供即用配置、合理的对抗训练默认设置，以及用于日志记录、验证和大场景推理的内置钩子。
...
TL;DR: 本文提出了一种全自动方法，通过CT成像生物标志物筛查2型糖尿病，特别关注胰腺表面小叶化(PSL)的作用，并开发了多变量预测模型。


<details>
  <summary>Details</summary>
Motivation: 2型糖尿病早期检测至关重要，但胰腺表面小叶化在T2DM患者中的作用尚未充分研究，需要开发自动化筛查方法。

Method: 使用四种深度学习模型在584名患者内部数据集上分割胰腺，自动检测PSL，并训练多变量模型预测T2DM。

Result: 糖尿病患者PSL显著更高(4.26 vs 3.19, p=0.01)；PancAP模型达到最高Dice分数0.79；多变量模型预测T2DM的AUC为0.90，灵敏度66.7%，特异性91.9%。
...
Method: 使用轻量级预测器分析令牌嵌入、交互和跨层注意力动态，实现上下文感知的令牌重要性推断和稀疏化。

Result: 相比标准ViT实现了高达40%的计算效率提升，同时保持甚至提高了准确率。

Conclusion: SPOT框架能够有效识别和消除冗余令牌，显著提升ViT的计算效率，且设计灵活可适应不同资源约束。
...
TL;DR: 提出可学习总变差(LTV)框架，将展开的TV求解器与数据驱动的LambdaNet结合，预测逐像素正则化图，实现端到端训练和空间自适应平滑。


<details>
  <summary>Details</summary>
Motivation: 传统总变差(TV)在图像去噪和边缘保持方面表现良好，但对lambda参数的依赖限制了其效率和使用效果。

Method: 耦合展开的TV求解器与LambdaNet网络，预测逐像素正则化图，实现端到端训练，在均匀区域强平滑，在解剖边界附近弱平滑。
...
Method: 1) 语义引导的双重视觉剪枝器(SD-Pruner)：指令驱动剪枝器提取全局动作线索和局部语义锚点；空间聚合剪枝器将几何丰富特征压缩为任务自适应令牌。2) 语义互补层次融合器(SH-Fuser)：融合SigLIP和DINOv2中的密集补丁和稀疏令牌。3) 语义条件动作耦合器(SA-Coupler)：替代传统的观测到自由度方法。

Result: 在仿真和真实世界任务中，SemanticVLA在LIBERO基准测试上比OpenVLA成功率提高21.1%，同时训练成本和推理延迟分别降低3.0倍和2.7倍。

Conclusion: SemanticVLA通过语义对齐的稀疏化和增强，显著提升了VLA模型在机器人操作中的性能和效率，为实际部署提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有4D神经渲染方法要么整体建模动态场景，要么分别建模场景和人体，但前者忽略不同组件的运动特性，后者缺乏组件间信息交换，导致重建不完整和边界伪影。
...
Conclusion: StM策略通过引入信息映射机制有效解决了分别建模组件间的协调问题，在人体-场景交互重建方面取得了优越性能。
...
TL;DR: 提出了一个评估文本到图像模型多样性的框架，包括人类评估模板、精心设计的提示集和基于二项式检验的模型比较方法。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在生成质量上有所进步，但输出往往缺乏多样性，产生同质化的结果，需要建立系统的多样性评估方法。

Method: 通过评估个体概念及其变化因素来系统评估多样性，包括：1）新颖的人类评估模板；2）涵盖多样化概念及其变化因素的提示集；3）基于二项式检验的模型比较方法。

Result: 能够对不同图像嵌入进行严格比较，并基于人类标注对T2I模型进行多样性排序，识别模型在特定类别中的困难。

Conclusion: 该研究为T2I模型多样性和度量开发提供了稳健的方法论和见解，为改进模型多样性铺平了道路。
...
TL;DR: 本文提出代码到风格图像生成任务，仅用数值风格代码就能生成新颖且一致的视觉风格图像，并开发了首个开源方法CoTyle。


<details>
  <summary>Details</summary>
Motivation: 现有生成方法依赖冗长文本提示、参考图像或参数微调，存在风格一致性差、创意有限和风格表示复杂的问题，需要更简单高效的风格控制方法。

Method: 训练离散风格码本提取风格嵌入，作为文本到图像扩散模型的条件；训练自回归风格生成器建模风格嵌入分布，合成新风格嵌入；推理时数值代码映射为风格嵌入指导图像生成。

Result: CoTyle成功将数值代码转化为风格控制器，验证了"一个风格值一个代码"的理念，实现了简单、多样且可复现的风格生成。

Conclusion: 该方法为风格控制提供了前所未有的简单性和多样性，从最小输入解锁了广阔的可复现风格空间。
...
TL;DR: OmniVGGT是一个能够有效利用任意数量几何模态（如深度、相机内外参）的3D基础模型框架，通过GeoAdapter和随机多模态融合机制，在保持推理速度的同时提升多种视觉任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的3D基础模型大多只使用RGB输入，忽略了容易获得的几何线索（如相机参数、深度图等），这限制了模型的性能提升。
...
Conclusion: OmniVGGT通过有效利用几何模态显著提升了3D基础模型的性能，且具有实际应用价值，特别是在机器人视觉任务中表现优异。
...
Result: M&M-3D在定位任务上比2D投影和3D切片方法提升11-54%，在分类任务上提升3-10%。在低数据情况下，比复杂3D推理变体在定位上提升20-47%，在分类上提升2-10%，在高数据情况下性能相当。在BCS-DBT基准测试中，分类性能提升4%，定位性能提升10%。

Conclusion: M&M-3D是一种有效的解决方案，能够在保持参数效率的同时实现3D推理，解决了DBT数据稀缺的问题，并在多个任务上显著优于现有方法。
...
TL;DR: 提出了一种新颖的多任务全局-局部OBIA-Mamba（MSOM）方法，用于增强Sentinel-2影像的土地利用/覆盖分类，通过结合超像素作为Mamba令牌、双分支CNN-Mamba架构和多任务优化框架，显著提高了分类精度和细节保留。


<details>
  <summary>Details</summary>
Motivation: Sentinel-2影像的土地利用/覆盖分类面临空间异质性、上下文信息和特征模糊性等关键数据挑战，需要开发更有效的分类方法来应对这些困难。

Method: 1. 设计了基于对象的图像分析（OBIA）Mamba模型，使用超像素作为Mamba令牌以减少冗余计算；2. 构建了全局-局部双分支CNN-Mamba架构，联合建模局部空间细节和全局上下文信息；3. 设计了多任务优化框架，采用双重损失函数平衡局部精度和全局一致性。

Result: 在加拿大阿尔伯塔省的Sentinel-2影像上进行测试，与多种先进分类方法相比，所提方法获得了更高的分类精度和更精细的细节保留。

Conclusion: MSOM方法通过结合OBIA、Mamba模型和多任务学习，有效解决了Sentinel-2影像分类中的关键挑战，在精度和细节方面均优于现有最先进方法。
...
TL;DR: 提出了Latent Upscaler Adapter (LUA)，一种轻量级模块，在VAE解码前直接在生成器的潜在代码上执行超分辨率，避免了传统图像超分辨率方法的延迟和伪影问题。


<details>
  <summary>Details</summary>
Motivation: 扩散模型难以扩展到超出训练分辨率，直接高分辨率采样缓慢且昂贵，而事后图像超分辨率会引入伪影和额外延迟。

Method: LUA作为即插即用组件集成，无需修改基础模型或添加额外的扩散阶段，通过潜在空间中的单次前向传递实现高分辨率合成。使用共享的Swin风格骨干网络和特定尺度的像素洗牌头支持2倍和4倍缩放。

Result: 与像素空间超分辨率相比，解码和上采样时间降低近3倍（1024像素生成仅增加+0.42秒，而相同SwinIR架构的像素空间SR需要1.87秒），同时保持可比的感知质量。
...
TL;DR: Depth Anything 3 (DA3) 是一个从任意数量视觉输入预测空间一致几何的模型，无需已知相机位姿，使用简单Transformer骨干和单一深度射线预测目标，在几何精度和相机位姿估计上大幅超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 追求最小化建模，证明简单架构即可实现高质量几何预测，无需复杂多任务学习或专门化架构。

Method: 使用单一普通Transformer（如DINO编码器）作为骨干，采用单一深度射线预测目标，通过师生训练范式进行训练。

Result: 在新建的视觉几何基准上，DA3在所有任务上达到新SOTA，相机位姿精度平均提升44.3%，几何精度提升25.1%，且在单目深度估计上超越DA2。

Conclusion: DA3证明了简单架构在几何预测任务中的有效性，为视觉几何建模提供了新的基准和方向。
...
<details>
  <summary>Details</summary>
Motivation: 在多选题设置中，结果奖励强化学习面临一个被忽视的障碍：即使推理链错误但猜对选项的轨迹与真正正确推理的轨迹获得相同奖励，这会影响模型训练效果。

Method: SCS方法对每个问题：(i)引入小的视觉扰动，(ii)对初始轨迹进行重复截断和重采样，通过轨迹间的一致性产生可微的一致性分数，在策略更新时降低不可靠轨迹的权重。
...
TL;DR: 该论文评估了Ksurf方法在云资源编排中的表现，Ksurf是一种针对高变异性云数据的方差最小化估计方法，在Drone编排器中作为上下文多臂老虎机目标函数模型使用时，显著降低了延迟方差和资源使用。


<details>
  <summary>Details</summary>
Motivation: 云数据中心中容器基础设施的资源编排和配置参数搜索面临巨大挑战，包括大型配置搜索空间和云环境不确定性。云环境中虚拟机数量变化带来的工作负载和资源指标变异性增加了编排决策的非线性和噪声，降低了准确性。

Method: 使用Ksurf作为上下文多臂老虎机目标函数模型，在Drone编排器中应用于云场景的资源编排任务。Ksurf是一种针对高变异性云数据的方差最小化估计方法。

Result: Ksurf实现了显著更低的延迟方差：p95降低41%，p99降低47%；CPU使用率降低4%；主节点内存使用减少7MB；在VarBench Kubernetes基准测试中平均工作pod数量节省7%成本。

Conclusion: Ksurf方法在高变异性云环境中能够有效优化资源估计和编排，显著降低延迟方差和资源使用成本。
...
TL;DR: 本文研究了雾-云计算网络中事务调度问题，提出了在常数倍维网络中的近似算法，最小化调度总成本。


<details>
  <summary>Details</summary>
Motivation: 在分布式系统中，事务调度对于高效分配共享资源至关重要。雾-云计算模型中事务和共享对象可以在网络中移动，需要设计高效的调度策略。

Method: 针对单对象和多对象访问场景，分别设计了集中式和分布式调度算法。单对象算法提供O(log n·log D)近似比，多对象算法提供O(k·log n·log D)近似比。

Result: 提出的算法在常数倍维网络中实现了理论保证的近似比，且分布式版本无需全局事务知识。

Conclusion: 该工作为移动事务和对象的调度问题提供了有效的解决方案，适用于实际网络环境。
...
TL;DR: GPU系统在数据中心中面临性能变异问题，特别是在运行LLM训练时，热不平衡导致的"Lit Silicon效应"会造成节点级性能下降。本文提出了检测和缓解技术，通过功率管理优化可实现6%性能提升和4%功耗改善。


<details>
  <summary>Details</summary>
Motivation: GPU系统在现代数据中心中广泛应用，但存在性能变异问题，特别是在多GPU节点中运行LLM训练时，热不平衡与并发计算通信(C3)技术结合会导致性能下降，影响数据中心效率。

Method: 分析单节点多GPU系统的LLM训练性能，识别热诱导的Lit Silicon效应，建立性能与功耗分析模型，设计检测和缓解技术，评估三种功率管理方案：GPU热设计功耗优化、节点级GPU功率封顶、节点级CPU功率调整。

Result: 在两个AMD Instinct MI300X GPU系统和两个LLM训练框架上的实验显示，解决方案可实现高达6%的性能提升和4%的功耗改善，有望为数据中心节省数亿美元成本。

Conclusion: Lit Silicon效应是多GPU节点中热不平衡导致的重要性能问题，提出的检测和缓解技术成本极低，可轻松作为新的节点级功率管理层在数据中心中部署。
...
TL;DR: MoFa是一个新的预训练性能建模框架，统一了多维优化特征和容错机制，通过增强的成本模型和基于历史集群可靠性数据的容错模型，实现高精度的性能预测和优化调优。


<details>
  <summary>Details</summary>
Motivation: 随着LLM规模从数十亿扩展到数万亿参数，分布式预训练需要在大规模集群上进行。传统手动调优成本高昂，现有性能建模方法无法全面考虑优化特征和容错机制的开销。

Method: 提出MoFa框架，包含增强的成本模型来准确捕捉关键优化效果，并集成基于历史集群可靠性数据的容错模型。开发了基于MoFa的调优系统来探索最优预训练性能和潜在瓶颈。

Result: 广泛的建模评估表明MoFa在各种场景下都能实现高预测精度。调优实验系统地揭示了不同配置下影响预训练性能的关键因素。
...
TL;DR: CacheX通过在虚拟机内使用驱逐集探测缓存抽象，无需硬件或虚拟机管理器支持，实现了LLC竞争感知的任务调度和虚拟颜色感知的页面缓存管理，有效提升了公共云虚拟机中各种工作负载的缓存利用率。


<details>
  <summary>Details</summary>
Motivation: 在公共云虚拟机中，由于无法了解和控制CPU缓存的配置细节（如缓存分区、共享情况），以及无法通过页面放置策略影响缓存使用，导致基于缓存的优化通常无效。

Method: 提出CacheX解决方案，使用驱逐集在虚拟机内探测准确且细粒度的缓存抽象，并开发了两种新技术：LLC竞争感知的任务调度和虚拟颜色感知的页面缓存管理。

Result: 在x86 Linux内核中实现的CacheX评估表明，它能够有效提高公共云虚拟机中各种工作负载的缓存利用率。

Conclusion: CacheX提供了一种无需硬件或虚拟机管理器支持的缓存抽象探测方法，能够显著改善云虚拟机中的缓存性能优化效果。
...
TL;DR: MO-HAN是一种轻量级边缘服务器选择方法，融合延迟预测与自适应可靠性，通过滞后切换机制平衡预测延迟和可靠性，在降低端到端延迟的同时减少近50%的切换次数。


<details>
  <summary>Details</summary>
Motivation: 多服务器架构中的动态网络拥塞对延迟敏感的嵌入式应用选择合适的边缘服务器构成挑战，需要一种轻量级解决方案。

Method: 使用被动测量（到达率、利用率、负载大小）和指数调制有理延迟模型，计算平衡预测延迟和可靠性的分数，仅在预期收益有意义时进行切换。

Result: MO-HAN在降低平均延迟和尾部延迟方面持续优于静态和公平分布基线，同时相比纯机会选择减少了近50%的切换次数。

Conclusion: MO-HAN无需侵入式检测或重型学习基础设施，适用于资源受限的嵌入式设备，实现了显著的性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 传统稀疏矩阵排序算法选择依赖暴力搜索或经验知识，无法适应不同稀疏矩阵结构，需要自动化智能选择方法

Method: 使用监督学习模型学习矩阵特征与常用重排序算法之间的关联，实现自动选择最优算法

Result: 在Florida稀疏矩阵数据集上实验表明，模型能准确预测最优重排序算法，相比单独使用AMD算法减少55.37%求解时间

Conclusion: 基于监督学习的算法选择模型能有效提升稀疏矩阵求解效率，实现自动化智能算法选择
...
TL;DR: 本文提出了一种现代工作负载调度器分类的新方法，描述了操作系统进程调度器、集群系统作业调度器和大数据调度器三类调度器，并讨论了它们从早期采用到现代实现的演变过程。
...
Method: 通过描述三类调度器（操作系统进程调度器、集群系统作业调度器、大数据调度器）的演变过程，分析其算法使用和特性。

Result: 提出了一个全面的调度器分类框架，揭示了不同类别调度器之间的差异及其按时间顺序的发展轨迹。

Conclusion: 本地系统和分布式系统的调度策略设计在关注点上存在相似性，这为跨系统的调度算法设计提供了有价值的见解。
...
TL;DR: FastGraph是一种专为低维空间（2-10维）图构建优化的GPU加速k近邻算法，相比现有方法提速20-40倍，内存开销几乎为零。


<details>
  <summary>Details</summary>
Motivation: 现有图神经网络（GNN）在图构建阶段存在性能瓶颈，特别是在低维空间中，需要更高效的k近邻算法来加速GNN工作流程。

Method: 采用GPU驻留的箱式分区方法，支持完整梯度流和自适应参数调整，提升计算和内存效率。

Result: 在维度小于10时，相比FAISS、ANNOY和SCANN等先进库，FastGraph实现20-40倍加速，内存开销几乎为零。

Conclusion: FastGraph显著提升了GNN工作流程性能，特别适用于高能物理粒子聚类、视觉目标跟踪和图聚类等计算密集型低维应用。
...
TL;DR: STAGE是一个框架，能够合成高保真度的执行轨迹来准确建模LLM工作负载，支持全面的并行化策略，可扩展到32K GPU规模，保持张量级精度。
...
Method: 引入Symbolic Tensor grAph GEnerator (STAGE)框架，通过合成高保真度执行轨迹来建模LLM工作负载，支持全面的并行化策略。
...
TL;DR: 提出了一种结合历史轨迹搜索和最短路径计算的FPGA实时轨迹k-匿名化方法，相比仅使用最短路径的方法，能更好地保护隐私并保持行为准确性。
...
Method: 开发了新型FPGA硬件架构，集成并行历史轨迹搜索与传统最短路径查找，使用自定义定点计数模块准确权衡历史数据贡献。
...
Result: AssertMiner在生成模块高质量断言方面优于AssertLLM和Spec2Assertion方法，集成后能显著提升结构覆盖率和错误检测能力。

Conclusion: AssertMiner能够实现更全面和高效的验证过程，解决了模块级断言生成的局限性。
...
TL;DR: 本文回顾了AI与计算机架构的协同演化，分析了GPU、ASIC和FPGA等主流AI加速器架构的设计理念、性能权衡，以及数据流优化、内存层次、稀疏性和量化等核心性能原则，并展望了存内计算和神经形态计算等新兴技术。


<details>
  <summary>Details</summary>
Motivation: 随着AI模型（特别是深度神经网络）复杂度的增加，巨大的计算需求已使传统架构达到极限，需要专门研究AI加速器架构来满足这些需求。

Method: 通过结构化回顾和分析主流AI加速器架构（GPU、ASIC、FPGA），分解其设计哲学、关键特征和性能权衡，并分析数据流优化、内存层次、稀疏性和量化等核心原则。

Result: 通过行业标准基准的定量性能数据，展示了AI加速器架构的全面图景，确认了硬件-软件协同设计对计算未来发展的必要性。
...
TL;DR: 该论文研究了数据中心和超级计算机网络中两个关键方面：在重负载下防止性能退化的拥塞控制，以及在空闲时节省能源的功耗管理，并探讨了这两者之间的相互作用。


<details>
  <summary>Details</summary>
Motivation: 随着数据中心和超级计算机在现代生活中的重要性日益增长，这些大规模复杂系统的网络互连可能成为整个基础设施的瓶颈。需要先进的技术和工程来确保系统正常运行，而不仅仅是依靠原始资源。

Method: 探索网络的两个方面：拥塞控制（防止重负载下的性能退化）和功耗管理（在空闲时节省能源），并研究这两者之间的相互作用关系。

Result: 论文分析了拥塞控制和功耗管理在网络性能优化中的重要性，以及它们之间的潜在冲突和协同效应。

Conclusion: 在设计和优化大规模计算系统的网络时，需要同时考虑拥塞控制和功耗管理这两个关键因素，并理解它们之间的相互作用，以实现整体系统性能的最优化。
...
TL;DR: 本文提出了一种用于毫米波大规模MIMO系统的波束空间数据检测算法CSPADE及其VLSI架构，相比天线域均衡器可节省高达66%的功耗，并实现了最高的吞吐量和更好的能效。


<details>
  <summary>Details</summary>
Motivation: 大规模MIMO和毫米波通信在部署时面临基带处理硬件成本和功耗过高的问题，波束空间处理利用毫米波频段的信道稀疏性来降低基带处理复杂度。

Method: 提出了复杂稀疏自适应均衡器(CSPADE)算法，并设计了相应的VLSI架构，包括全并行实现和基于顺序MAC的架构。

Result: 在22nm FDSOI工艺中的VLSI实现结果显示，全并行CSPADE相比天线域均衡节省54%功耗，MAC架构节省66%功耗，同时实现了现有大规模MIMO数据检测器中最高的吞吐量和更好的能效。

Conclusion: 波束空间处理结合CSPADE算法和优化的VLSI架构能够显著降低大规模MIMO毫米波系统的功耗，同时保持高性能。
...
### [189] [VEDA: 3D Molecular Generation via Variance-Exploding Diffusion with Annealing](https://arxiv.org/abs/2511.09568)
*Peining Zhang,Jinbo Bi,Minghu Song*

Main category: physics.chem-ph

TL;DR: VEDA是一个统一的SE(3)-等变框架，结合方差爆炸扩散和退火技术，能高效生成构象准确的3D分子结构，在100步采样内达到最先进的化学有效性，同时显著降低弛豫能量。


<details>
  <summary>Details</summary>
Motivation: 解决扩散模型在3D分子生成中采样效率与构象准确性之间的基本权衡问题。流基模型虽快但几何不准确，去噪扩散模型准确但采样慢。

Method: 提出VEDA框架：1）VE调度实现类似模拟退火的噪声注入；2）新颖的预处理方案协调SE(3)-等变网络与残差扩散目标；3）基于arcsin的调度器在关键信噪比区间集中采样。

Result: 在QM9和GEOM-DRUGS数据集上，VEDA匹配流基模型的采样效率，仅用100步采样达到最先进的价态稳定性和有效性。生成结构非常稳定，中位弛豫能量仅1.72 kcal/mol，显著低于SemlaFlow的32.3 kcal/mol。

Conclusion: VEDA证明将VE扩散与SE(3)-等变架构进行原则性整合，可以同时实现高化学准确性和计算效率。
...
### [190] [Simulating Misinformation Propagation in Social Networks using Large Language Models](https://arxiv.org/abs/2511.10384)
*Raj Gaurav Maurya,Vaibhav Shukla,Raj Abhijit Dandekar,Rajat Dandekar,Sreedath Panat*

Main category: cs.SI

TL;DR: 该研究通过LLM模拟具有认知偏见和意识形态的用户代理，构建审计-节点框架来追踪虚假信息在网络中的传播演化，发现身份和意识形态驱动的代理会加速虚假信息传播，而专家型代理能保持事实稳定性。


<details>
  <summary>Details</summary>
Motivation: 研究社交媒体上虚假信息传播的认知机制，特别是基于惊讶、情感和身份认同的推理如何通过人类认知偏见被放大。

Method: 使用LLM创建模拟用户偏见的合成代理，建立审计-节点框架，让新闻文章在代理网络中传播重写，通过问答式审计器追踪事实保真度，定义虚假信息指数和传播率来量化事实退化。

Result: 实验显示身份和意识形态型代理是虚假信息加速器，尤其在政治、营销和技术领域；专家型代理能保持事实稳定；早期失真出现后，异构代理互动会迅速升级为宣传级失真。

Conclusion: LLM既能模拟人类偏见，又能作为审计器追踪信息保真度，该框架为研究和缓解数字生态系统中的虚假信息传播提供了可解释的实证方法。
...
TL;DR: 提出了首个可大规模并行化的证明数搜索算法，在1024个CPU核心上实现332.9倍加速，应用于Sprouts游戏验证，性能提升4个数量级，验证了42个新位置的Sprouts猜想。


<details>
  <summary>Details</summary>
Motivation: 现有并行证明数搜索算法在多CPU核心上扩展性差，随着大规模计算集群的普及，需要开发能够高效利用大量CPU的并行算法。

Method: 使用两级并行化架构和工作者间信息共享，结合Grundy数减少游戏树规模，实现大规模并行证明数搜索。

Result: 在1024个核心上实现332.9倍加速，性能比现有最佳Sprouts求解器GLOP快4个数量级，生成复杂度高1000倍的证明，验证了42个新位置的Sprouts猜想。

Conclusion: 成功开发了首个可大规模并行化的证明数搜索算法，显著提升了游戏求解能力，为Sprouts猜想提供了新的验证结果。
...
### [192] [Enhancing the Medical Context-Awareness Ability of LLMs via Multifaceted Self-Refinement Learning](https://arxiv.org/abs/2511.10067)
*Yuxuan Zhou,Yubin Wang,Bin Wang,Chen Ning,Xien Liu,Ji Wu,Jianye Hao*

Main category: cs.AI

TL;DR: MuSeR是一种数据驱动方法，通过自我评估和精炼提升LLM在医疗领域的上下文感知能力，在HealthBench基准测试中显著提升性能并创造新的SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在真实医疗场景中表现不佳，缺乏对关键细节（如用户身份、病史、风险因素）的识别能力，需要更强的上下文感知能力来提供安全、有帮助且情境适当的响应。

Method: 提出多层面自我精炼(MuSeR)方法：1)属性条件查询生成器模拟多样化用户情境；2)LLM响应查询并沿决策制定、沟通和安全三个关键层面进行自我评估；3)精炼响应以更好对齐各层面要求；4)使用查询和精炼响应进行监督微调。

Result: 在HealthBench数据集上显著提升LLM性能，特别是在上下文感知维度。通过知识蒸馏，较小的骨干LLM(Qwen3-32B)超越其教师模型，在HealthBench上达到63.8%，其困难子集达到43.1%，创下开源LLM新SOTA。

Conclusion: MuSeR方法有效增强了LLM在医疗领域的上下文感知能力，通过自我评估和精炼机制显著提升了模型在真实医疗场景中的表现，为医疗AI应用提供了重要技术支撑。
...
### [193] [ProgRAG: Hallucination-Resistant Progressive Retrieval and Reasoning over Knowledge Graphs](https://arxiv.org/abs/2511.10240)
*Minbae Park,Hyemin Yang,Jeonghyun Kim,Kunsoo Park,Hyunjoon Kim*

Main category: cs.AI

TL;DR: ProgRAG是一个多跳知识图谱问答框架，通过将复杂问题分解为子问题，逐步扩展推理路径，使用不确定性感知剪枝优化证据检索，从而提升多跳KGQA的性能和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有KG增强的LLMs方法存在检索不准确、推理失败等问题，特别是面对长输入上下文时相关信息被淹没，且难以捕捉不同问题类型所需的丰富逻辑方向。此外，LLMs直接检索证据并自我评估证据充分性往往导致过早或不正确的推理。
...
### [194] [FactGuard: Event-Centric and Commonsense-Guided Fake News Detection](https://arxiv.org/abs/2511.10281)
...
### [195] [EgoEMS: A High-Fidelity Multimodal Egocentric Dataset for Cognitive Assistance in Emergency Medical Services](https://arxiv.org/abs/2511.09894)
...
TL;DR: EgoEMS是首个端到端、高保真、多模态、多人的EMS数据集，包含233个模拟紧急场景中超过20小时的自我中心视角数据，用于开发AI认知助手支持急救医疗服务。
...
Result: 创建了EgoEMS数据集，包含超过20小时的真实EMS活动数据，强调真实性，包含反映真实世界紧急动态的响应者-患者互动。
...
### [196] [Learning to Pose Problems: Reasoning-Driven and Solver-Adaptive Data Synthesis for Large Reasoning Models](https://arxiv.org/abs/2511.09907)
*Yongxian Wei,Yilin Zhao,Li Shen,Xinrui Chen,Runxi Cheng,Sinan Du,Hao Yu,Gang Liu,Jiahong Yan,Chun Yuan,Dian Li*

Main category: cs.AI

TL;DR: 开发了一个能够明确推理来规划问题方向并适应求解器能力的问题生成器，通过构建相关问题对、添加问题设计思维链，并利用求解器反馈来校准难度，在10个数学和通用推理基准上平均提升2.5%性能。
...
Method: 构建相关问题对，用推理模型生成中间问题设计思维链来引导问题生成策略，将求解器对合成问题的反馈作为奖励信号，使生成器能够校准难度并在求解器能力边界附近生成补充问题。

Result: 在10个数学和通用推理基准上平均提升2.5%性能，并能泛化到语言和视觉语言模型。通过协同进化进一步获得0.7%性能增益。

Conclusion: 该方法通过明确推理和难度适应实现了高质量问题生成，支持生成器与求解器的协同进化，显著提升了推理模型的性能。
...
### [197] [Querying Labeled Time Series Data with Scenario Programs](https://arxiv.org/abs/2511.10627)
*Edward Kim,Devan Shanker,Varun Bharadwaj,Hongbeen Park,Jinkyu Kim,Hazem Torfah,Daniel J Fremont,Sanjit A Seshia*

Main category: cs.AI

TL;DR: 提出了一种验证模拟故障场景在真实世界中重现性的方法，通过将Scenic场景程序与真实传感器数据匹配来识别真实数据中的相应场景。


<details>
  <summary>Details</summary>
Motivation: 解决仿真测试中发现的AV故障场景是否能在真实世界中重现的问题，弥合仿真与现实之间的差距。

Method: 定义了标记时间序列传感器数据与抽象场景的匹配形式化方法，开发了基于Scenic概率编程语言的场景查询算法。

Result: 实验显示该算法比最先进的商业视觉大语言模型更准确，查询速度快几个数量级，且能随查询时间序列数据长度扩展。

Conclusion: 该方法能有效验证仿真故障场景在真实世界中的重现性，为仿真测试提供可靠验证手段。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体在多智能体系统中广泛应用，其可靠性影响尚未充分探索。本文旨在研究用LLM智能体替代传统智能体是否能有效提升MAS的可靠性，特别是从拜占庭容错角度进行量化分析。

Method: 设计了CP-WBFT（基于置信度探针的加权拜占庭容错共识机制），利用LLM固有的反思和判别能力，采用基于探针的加权信息流传输方法来提升LLM智能体的可靠性。

Result: 在极端拜占庭条件下（85.7%故障率），CP-WBFT在不同网络拓扑结构下均表现出优越性能，在数学推理和安全评估任务中保持强可靠性，显著超越传统方法。

Conclusion: LLM智能体在处理错误信息时具有更强的怀疑性，CP-WBFT机制有效利用了LLM的这一特性，在不同拓扑结构下都能显著提升多智能体系统的可靠性和稳定性。
...
### [199] [Revisit to the Bai-Galbraith signature scheme](https://arxiv.org/abs/2511.09582)
*Banhirup Sengupta,Peenal Gupta,Souvik Sengupta*

Main category: cs.CR

TL;DR: 本文介绍了Bai-Galbraith签名方案，这是一种基于LWE的格基签名方案，与Dilithium的主要区别在于没有公钥压缩。


<details>
  <summary>Details</summary>
Motivation: 提出一种与NIST批准的Dilithium签名方案不同的格基签名方案，探索无公钥压缩的替代设计。

Method: 基于学习误差问题(LWE)构建格基签名方案，采用Bai-Galbraith在BG14中提出的方法，不使用公钥压缩技术。

Result: 描述了一个完整的格基签名方案，该方案在结构上与Dilithium不同，但同样基于LWE问题。

Conclusion: Bai-Galbraith签名方案提供了一种无公钥压缩的格基签名替代方案，为后量子密码学提供了更多选择。
...
### [200] [Pk-IOTA: Blockchain empowered Programmable Data Plane to secure OPC UA communications in Industry 4.0](https://arxiv.org/abs/2511.10248)
*Rinieri Lorenzo,Gori Giacomo,Melis Andrea,Girau Roberto,Prandini Marco,Callegati Franco*

Main category: cs.CR

TL;DR: Pk-IOTA是一个自动化解决方案，通过集成可编程数据平面交换机进行网络内证书验证，并利用IOTA Tangle实现去中心化证书分发，以保护OPC UA通信安全。


<details>
  <summary>Details</summary>
Motivation: OPC UA协议作为工业4.0机器间通信的事实标准，虽然具备强大的安全特性，但在实际部署中仍面临证书管理复杂性和工业设备安全功能实现不一致的挑战。

Method: 集成可编程数据平面交换机进行网络内证书验证，并利用IOTA Tangle实现去中心化证书分发。

Result: 在代表真实工业场景的物理测试平台上评估显示，Pk-IOTA引入的开销极小，同时为OPC UA证书管理提供了可扩展且防篡改的机制。
...
### [201] [Trapped by Their Own Light: Deployable and Stealth Retroreflective Patch Attacks on Traffic Sign Recognition Systems](https://arxiv.org/abs/2511.10050)
*Go Tsuruoka,Takami Sato,Qi Alfred Chen,Kazuki Nomoto,Ryunosuke Kobayashi,Yuna Tanaka,Tatsuya Mori*

Main category: cs.CR

TL;DR: 提出了一种新型对抗性攻击方法ARP，利用逆反射材料在车灯照射下激活攻击，结合了贴片攻击的高部署性和激光投影的隐蔽性，在动态场景中达到93.4%的成功率，并提出相应的防御方法DPR Shield。


<details>
  <summary>Details</summary>
Motivation: 现有的交通标志识别系统对抗攻击方法存在视觉可检测性或实施限制的问题，表明TSR系统中存在未被探索的漏洞表面。

Method: 开发逆反射模拟方法并使用黑盒优化来最大化攻击效果，通过逆反射材料仅在受害者车灯照射下激活来实现隐蔽攻击。

Result: ARP在35米动态场景中达到≥93.4%的成功率，在真实世界条件下对商业TSR系统达到≥60%的成功率，隐蔽性评分比之前的贴片攻击高≥1.9%。

Conclusion: ARP攻击在保持与良性标志几乎相同的隐蔽性的同时实现了高攻击成功率，提出的DPR Shield防御方法对停车标志和限速标志的微棱镜贴片攻击达到≥75%的防御成功率。
...
<details>
  <summary>Details</summary>
Motivation: 训练大型语言模型受到设备内存有限和设备间通信成本高的限制。传统管道并行虽然缓解了内存压力，但激活通信开销随序列长度线性增长，在长上下文训练中效率低下。现有的权重传递方法存在冗余的点对点传输和节点内带宽利用不足的问题。
...
Conclusion: TawPipe通过拓扑感知的权重管道并行方法，显著减少了跨节点通信流量，在大型语言模型训练中提供了优越的性能和可扩展性。
...
### [203] [SMoFi: Step-wise Momentum Fusion for Split Federated Learning on Heterogeneous Data](https://arxiv.org/abs/2511.09828)
...
<details>
  <summary>Details</summary>
Motivation: 解决联邦学习系统中数据异构性导致的梯度发散问题，该问题会降低全局模型的收敛速度和准确性。

Method: 设计了Step-wise Momentum Fusion框架，通过同步服务器端优化器的动量缓冲区，并采用staleness-aware对齐机制来控制训练过程中的梯度发散。

Result: 在多个真实数据集上的验证表明，SMoFi能持续提升全局模型准确性（最高7.1%）和收敛速度（最高10.25倍），在更多客户端和更深模型时效果更显著。

Conclusion: SMoFi特别适用于资源受限环境下的模型训练，能有效应对数据异构性挑战，提升联邦学习系统的性能。
...
### [204] [Unlocking Dynamic Inter-Client Spatial Dependencies: A Federated Spatio-Temporal Graph Learning Method for Traffic Flow Forecasting](https://arxiv.org/abs/2511.10434)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界交通数据分布在多个利益相关方之间，存在数据本地化约束，现有方法主要处理静态依赖关系，忽略了动态特性，导致性能不佳。
...
Conclusion: FedSTGD能够有效建模动态的客户端间空间依赖关系，在联邦学习环境中实现高性能的交通时间序列预测，同时保持数据隐私。
...
### [205] [Probability-Biased Attention over Directed Bipartite Graphs for Long-Tail ICD Coding](https://arxiv.org/abs/2511.09559)
*Tianlei Chen,Yuxiao Chen,Yang Li,Feifei Wang*

Main category: cs.LG

TL;DR: 本文提出了一种基于有向二分图编码器的自动ICD编码方法，通过建模常见与罕见疾病代码间的细粒度共现关系，利用条件概率引导信息从常见代码流向罕见代码，从而改善长尾分布下的分类性能。


<details>
  <summary>Details</summary>
Motivation: 自动ICD编码面临标签空间大（1万-2万个代码）和长尾分布的挑战，许多罕见代码缺乏足够的训练数据。需要一种能够利用常见代码信息来增强罕见代码表示的方法。

Method: 构建有向二分图编码器，将常见和罕见代码作为不相交节点集，边从常见代码指向罕见代码。基于条件概率定义连接权重，通过共现编码将这种偏置注入注意力模块。使用大语言模型生成代码描述来丰富初始嵌入。

Result: 在三个自动ICD编码基准数据集上的实验表明，该方法实现了最先进的性能，特别是在长尾分类关键指标Macro-F1上取得了显著改进。

Conclusion: 通过建模代码间的统计共现关系，利用常见代码的共病信息来增强罕见代码表示，有效解决了ICD编码中的长尾分布问题。
...
### [206] [OutSafe-Bench: A Benchmark for Multimodal Offensive Content Detection in Large Language Models](https://arxiv.org/abs/2511.10287)
*Yuping Yan,Yuhan Xie,Yuanshuai Li,Yingchao Yu,Lingjuan Lyu,Yaochu Jin*

Main category: cs.LG

TL;DR: OutSafe-Bench是首个为多模态时代设计的最全面的内容安全评估测试套件，包含超过18,000个双语文本提示、4,500张图像、450个音频和450个视频，涵盖9个关键内容风险类别，并提出了MCRS和FairScore评估框架。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型被集成到日常工具和智能代理中，对其可能输出不安全内容的担忧日益增长，包括有毒语言、偏见图像、隐私侵犯和有害错误信息等。现有安全基准在模态覆盖和性能评估方面存在严重不足。

Method: 构建大规模多模态数据集，提出多维交叉风险评分(MCRS)来建模不同风险类别间的重叠和相关性，并设计FairScore可解释自动化多评审加权聚合框架，选择表现最佳的模型作为自适应评审团。

Result: 对9个最先进的多模态大语言模型进行评估，发现了持续且显著的安全漏洞，突显了在多模态大语言模型中建立强大安全防护的紧迫需求。

Conclusion: OutSafe-Bench填补了多模态内容安全评估的空白，揭示了当前多模态大语言模型在安全防护方面的严重不足，为未来模型的安全改进提供了重要基准和评估方法。
...
### [207] [AgentEvolver: Towards Efficient Self-Evolving Agent System](https://arxiv.org/abs/2511.10395)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的自主代理开发方法成本高、效率低，需要手动构建任务数据集和强化学习管道，导致数据构建成本过高、探索效率低和样本利用率差。

Method: 引入三种协同机制：自提问（好奇心驱动任务生成）、自导航（经验重用和混合策略指导提高探索效率）、自归因（基于贡献度分配差异化奖励提高样本效率）。
...
### [208] [Impact of Layer Norm on Memorization and Generalization in Transformers](https://arxiv.org/abs/2511.10566)
...
<details>
  <summary>Details</summary>
Motivation: 研究LayerNorm在Pre-和Post-LayerNorm transformers中对学习和记忆化的影响，目前这些影响尚不清楚。

Method: 通过分析13个模型在6个视觉和语言数据集上的表现，研究LayerNorm参数对学习和记忆化的影响，特别关注早期层LayerNorm的作用。

Result: LayerNorm是Pre-LayerNorm模型稳定学习的关键因素，而在Post-LayerNorm模型中主要影响记忆化。移除LayerNorm参数在Pre-LayerNorm中加剧记忆化并破坏学习，在Post-LayerNorm中能缓解记忆化。

Conclusion: LayerNorm在transformer中塑造记忆化和学习的作用因架构而异，为理解LayerNorm在transformer中的角色提供了新视角。
...
### [209] [Towards Emotionally Intelligent and Responsible Reinforcement Learning](https://arxiv.org/abs/2511.10573)
*Garapati Keerthana,Manik Gupta*

Main category: cs.LG

TL;DR: 提出了一个负责任强化学习框架，将情感理解和伦理约束整合到序列决策过程中，用于医疗和行为支持领域的个性化系统。


<details>
  <summary>Details</summary>
Motivation: 传统基于规则或参与度最大化的个性化决策系统忽视了用户的情感背景和伦理约束，可能导致推荐不敏感或不安全的干预措施，特别是在心理健康、物质使用障碍等敏感领域。

Method: 将个性化建模为约束马尔可夫决策过程，引入多目标奖励函数平衡短期行为参与度和长期用户福祉，定义情感感知的状态表示，并可通过任何强化学习算法实现。

Result: 提出了一个概念框架，将同理心和责任操作化到机器学习策略优化中，连接了安全强化学习、情感计算和负责任AI。

Conclusion: 该框架为行为健康、教育和数字治疗等领域的人本系统提供了方法基础，旨在启动关于情感感知和可信赖个性化系统的伦理对齐强化学习方法讨论。
...
### [210] [PRISM: Diversifying Dataset Distillation by Decoupling Architectural Priors](https://arxiv.org/abs/2511.09905)
...
Method: PRISM框架在合成过程中解耦架构先验，将logit匹配和正则化目标分别用不同教师架构监督：主模型用于logits，随机子集用于批归一化对齐。还引入了可扩展的跨类批次形成方案进行快速并行合成。

Result: 在ImageNet-1K上，PRISM在低中IPC机制下持续且可重复地优于单教师方法（如SRe2L）和近期多教师变体（如G-VBSM）。生成的数据显示显著更丰富的类内多样性，特征间余弦相似度明显下降。

Conclusion: PRISM通过多教师架构解耦有效解决了单教师偏差问题，在数据集蒸馏中实现了更好的性能和多样性，为大规模数据集蒸馏提供了有效解决方案。
...
### [211] [eXIAA: eXplainable Injections for Adversarial Attack](https://arxiv.org/abs/2511.10088)
...
Method: 基于模型预测和解释的单步攻击方法，无需访问模型或其权重，仅需预测结果和解释输出。

Result: 攻击能够在不改变预测概率的情况下显著改变解释结果，通过平均绝对差和结构相似性指数验证了攻击效果。

Conclusion: 该方法揭示了当前可解释性方法的严重脆弱性，对其在安全关键应用中的可靠性提出了质疑。
...
### [212] [How does My Model Fail? Automatic Identification and Interpretation of Physical Plausibility Failure Modes with Matryoshka Transcoders](https://arxiv.org/abs/2511.10094)
...
<details>
  <summary>Details</summary>
Motivation: 现有生成模型虽然能产生逼真输出，但仍存在物理合理性错误，且这些错误难以被现有评估方法检测，缺乏自动识别和解释物理错误模式的框架。

Method: 扩展Matryoshka表示学习范式到转码器架构，在物理合理性分类器的中间表示上训练，利用多模态模型进行解释，实现多粒度层次稀疏特征学习。

Result: 相比现有方法，实现了更好的特征相关性和特征准确性，识别了多样化的物理相关失败模式，无需手动特征工程。

Conclusion: 为八个最先进生成模型的分析提供了有价值的见解，揭示了这些模型如何未能遵循物理约束，为进一步模型改进铺平了道路。
...
### [213] [Intrinsic Dimensionality as a Model-Free Measure of Class Imbalance](https://arxiv.org/abs/2511.10475)
*Çağrı Eser,Zeynep Sonat Baltacı,Emre Akbaş,Sinan Kalkan*

Main category: cs.LG

TL;DR: 提出使用数据内在维度(ID)作为不平衡分类任务的新度量方法，替代传统的基于样本数量的不平衡度量，能更好地反映类别学习难度差异。


<details>
  <summary>Details</summary>
Motivation: 传统基于样本数量的不平衡度量忽略了冗余样本和类别学习难度差异，而基于训练损失和不确定性的复杂度量需要训练模型。

Method: 使用数据内在维度(ID)作为易于计算、无需模型的度量方法，可无缝整合到各种不平衡缓解方法中。

Result: 在五个不同不平衡比率的数据集上，ID方法始终优于基于样本数量的重加权和重采样技术，且与样本数量结合能进一步提升性能。

Conclusion: 数据内在维度是有效的模型无关不平衡度量方法，能显著改善不平衡分类任务的性能。
...
### [214] [On The Performance of Prefix-Sum Parallel Kalman Filters and Smoothers on GPUs](https://arxiv.org/abs/2511.10363)
*Simo Särkkä,Ángel F. García-Fernández*

Main category: stat.CO

TL;DR: 本文对基于GPU的并行时间卡尔曼滤波器和平滑器进行实验评估，比较不同并行扫描算法，并提出新型并行时间双滤波器平滑器。


<details>
  <summary>Details</summary>
Motivation: 评估并行时间卡尔曼滤波器和平滑器在GPU上的性能，探索高效的并行扫描算法以实现时间维度的并行化。

Method: 通过仿真计算所需操作数，并在实际GPU硬件上测量算法运行时间；提出新型并行时间双滤波器平滑器；提供Metal和CUDA的Julia代码实现。

Result: 对不同的并行扫描算法进行了性能评估，包括操作数计算和实际运行时间测量。
...
### [215] [Efficient Automated Diagnosis of Retinopathy of Prematurity by Customize CNN Models](https://arxiv.org/abs/2511.10023)
*Farzan Saeedi,Sanaz Keshvari,Nasser Shoeibi*

Main category: eess.IV

TL;DR: 本文通过深度学习方法改进ROP诊断，定制CNN模型在准确率和F1分数上优于预训练模型，投票系统进一步提升性能，并验证了在临床环境中的部署可行性。


<details>
  <summary>Details</summary>
Motivation: 旨在提高早产儿视网膜病变(ROP)诊断的精确性和效率，解决现有方法在计算成本和诊断准确性方面的挑战。

Method: 采用定制化的CNN架构，结合数据集管理、预处理策略，并实施投票系统来集成多个模型的预测结果。

Result: 定制CNN模型在准确率和F1分数上显著优于预训练模型，投票系统进一步提升了诊断性能，同时减轻了计算负担。

Conclusion: 深度学习模型能有效提升ROP诊断的精确度和效率，定制化CNN结合投票系统在临床环境中具有实际应用价值。
...
### [216] [dHPR: A Distributed Halpern Peaceman--Rachford Method for Non-smooth Distributed Optimization Problems](https://arxiv.org/abs/2511.10069)
...
Method: 利用对称Gauss-Seidel分解，有效解耦目标函数和一致性约束中的线性算子，实现分散式实现和快速收敛。

Result: 在分布式LASSO、组LASSO和L1正则化逻辑回归问题上进行了全面数值实验，证明了dHPR的优越性能。

Conclusion: dHPR方法能够有效解决分布式非光滑优化问题，具有理论保证的快速收敛性和实际应用的优越性能。
...
TL;DR: Music Flamingo是一个专为音乐理解设计的大型音频-语言模型，通过多阶段数据标注、增强的Audio Flamingo 3骨干网络和强化学习训练，在10多个音乐理解基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 当前音频-语言模型在音乐理解方面存在挑战，主要由于音乐具有动态、分层和信息密集的特性，高质量音乐数据和标注稀缺，导致现有模型只能生成简短描述、回答浅层问题，且在不同音乐文化中泛化能力有限。

Method: 1) 构建MF-Skills大规模数据集，包含丰富的描述和问答对；2) 在增强的Audio Flamingo 3骨干网络上微调；3) 引入后训练方法：先在基于音乐理论的MF-Think数据集上进行冷启动，然后使用GRPO强化学习配合定制奖励。

Result: 在10多个音乐理解和推理基准测试中达到最先进水平，展示了从表层识别到分层、类人音乐感知的能力提升。

Conclusion: Music Flamingo为社区提供了基准和基础，推动下一代模型能够像人类一样有意义地理解音乐，标志着音乐理解从表层识别向分层感知的转变。
